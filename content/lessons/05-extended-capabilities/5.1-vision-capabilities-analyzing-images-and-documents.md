...
### Example 1: Competitor social post analysis

**Scenario:** A café owner screenshots a competitor's Instagram post to understand their promotional strategy.

**Prompt:**
```
I've uploaded a screenshot of an Instagram post from a competitor café in our suburb. They posted this on a Tuesday morning — it appears to be a promotion for a midweek lunch deal with a food photo and caption.

Please analyse this post and tell me:
1. What the offer is — exactly what are they promoting and at what price if visible
2. The tone of the caption — is it casual, professional, humorous, urgent?
3. What call to action they used (if any)
4. Whether the image and caption feel consistent with each other
5. One thing this post does well and one thing I could do differently when I run a similar promotion

Respond in dot points. Keep each point to one or two sentences.
```

**Why it works:** The prompt tells Claude the platform, the day, and the likely content before asking anything. The five-part question structure ensures Claude covers every angle the café owner actually cares about. The format instruction (dot points, one to two sentences) means the response is ready to share without further editing.

---

### Example 2: Supplier invoice extraction

**Scenario:** A small retailer uploads a scanned invoice from a wholesale supplier and needs the key figures extracted.

**Prompt:**
```
I've uploaded a scan of a wholesale invoice from one of our clothing suppliers. This is for stock we received last Thursday. The scan quality is decent but some text may be slightly skewed.

Please extract the following:
- Supplier name and ABN (if visible)
- Invoice number and date
- Each line item: product name or SKU, quantity, unit price, and line total
- Subtotal, GST amount, and total payable
- Payment due date (if shown)

Present this as a simple table. If any field is unclear or you're not confident in a reading, mark it with [check] so I know to verify against the original.
```

**Why it works:** The prompt sets expectations about scan quality, asks for specific fields in a defined format, and includes the critical instruction to flag uncertain readings. That last point prevents Claude from silently guessing at a number that might be wrong, which is a real risk with financial documents.

---

### Example 3: Whiteboard photo transcription

**Scenario:** A small business owner photographs the whiteboard from a team planning session and wants the content structured.

**Prompt:**
```
I've uploaded a photo of our office whiteboard from this morning's planning session. The whiteboard has notes from our team discussion about Q3 priorities — there are some bullet points, a rough list of projects, and some arrows connecting ideas. Handwriting varies (two different people wrote on it).

Please:
1. Transcribe all the text you can read — use your best interpretation for any unclear words
2. Organise the content into logical groups if you can see a clear structure (e.g. separate project names from action items)
3. Flag any words or sections where you're genuinely unsure what they say — mark these [unclear]

Output as plain text, not a table. I'll use this as meeting notes.
```

**Why it works:** The prompt acknowledges likely challenges upfront (two different handwriting styles, arrows and connectors that imply relationships). It asks for transcription and organisation as separate tasks, and sets a realistic expectation — "best interpretation" for unclear words — rather than expecting perfect accuracy.

---

### Example 4: Design layout review

**Scenario:** A beauty salon owner uploads a draft flyer for a spring promotion and wants a quick layout critique before sending it to print.

**Prompt:**
```
I've uploaded an image of a draft flyer we've designed for our spring promotion — it's an A5 sized print flyer for our beauty salon. The promotion is 20% off facial treatments throughout October. Our target audience is women aged 30–55 in our local area.

Please review this as someone who would pick it up in a shop:
1. Is the core offer (20% off facials) immediately clear, or does the reader have to search for it?
2. Is there a clear call to action — do they know what to do next?
3. Is there anything on the flyer that feels cluttered, hard to read, or off-brand for a beauty business?
4. One specific suggestion to improve the layout before we print

Be direct. We're printing 500 copies so I'd rather hear the problems now.
```
...
### Example 5: Scanned client form — key terms extraction

**Scenario:** A bookkeeper receives a scanned engagement letter from a new client and needs the key terms captured before the client signs.

**Prompt:**
```
I've uploaded a scanned engagement letter from a new client — this is the agreement they received from their previous accountant and they want me to review it before formally disengaging from that firm.

Please read through this document and extract:
1. The parties named in the agreement (client name and firm name)
2. The services covered by the engagement
3. The fee structure — fixed, hourly, or retainer — and any specific amounts mentioned
4. The notice period required to end the engagement
5. Any clauses about data or file ownership when the engagement ends

This is not a legal review — I just need a structured summary of these five points so I can have an informed conversation with the client. If any section is unclear or appears to be cut off in the scan, note that.
```
...
**Brief:** For this deliverable you will use Claude's vision capability on an image from your actual business or your industry. The image should be something you genuinely encounter at work — a competitor's ad, an invoice, a product photo, a screenshot of a report, a scanned document, a design draft, a photo from a site or event. The only rule: it must be a real image, not a stock photo, a test image, or something you pulled off Google to practice with.

You will write a two-part vision prompt — describing what you are sending, then stating exactly what you need Claude to do — and submit your prompt alongside Claude's response and an honest assessment of how accurate and useful the output was.
...
**What to submit:**
1. A description of the image: what it is, where it came from, and why you chose it for this exercise (2–4 sentences)
2. The full prompt you sent to Claude with the image — exactly as you typed it
3. Claude's full response — exactly as it came back, no edits
4. An 80-word accuracy note: what Claude got right, what it missed or got wrong, and one thing you would do differently in the prompt next time
...
**Length guide:** Item 1 is 2–4 sentences. Item 4 is 80 words. Items 2 and 3 will vary depending on your image and task — there is no minimum or maximum. The task determines the length.
...
- **Constraint clarity (25 pts)** — Does the prompt specify what Claude should extract, analyse, or produce — not just "what do you see?" or "what do you think?" The best prompts name specific outputs (a list of line items, a three-point summary, a dot-point critique) and, where relevant, the output format. Bonus if the prompt asks Claude to flag uncertainty — this is a mark of a student who has understood how vision errors work.

- **Outcome focus (25 pts)** — Does the 80-word accuracy note identify specific things Claude got right or wrong, not a general impression? "It was pretty accurate" scores zero here. "Claude read the invoice total correctly but misread the GST as $43 when the actual figure is $34 — I noticed because the numbers look similar in the scan" scores full marks. The note should show the student actually compared Claude's output to the original image.
...
The image students choose for this deliverable naturally reflects their industry: a tradie might upload a photo from a job site or a scan of a supplier docket; a salon or clinic owner might upload a draft promotional graphic; a retailer might screenshot a competitor's product listing or upload a photo of their own product display; a professional services firm might upload a scanned client document or a screenshot from a financial dashboard; a café or hospitality business might use a competitor's social post or a photo of their own menu board. Because the image is entirely student-chosen, no industry adjustment is needed to the lesson — the brief simply needs students to pick something real from their working context, and their industry does the rest. The worked examples span retail, food service, trades, professional services, and design to signal that the lesson applies broadly.

---

## Source disagreement

None. Title sourced from DOCX (renumbered to module-local 5.1).
