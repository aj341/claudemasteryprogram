...
Think about the last four weeks. You've sent Claude a prompt to write a client status update. Then two weeks later, you wrote a different prompt for a different client status update. Then again last week, another one. Each time, you started from scratch — typed out the context, the tone instruction, the format, the constraints. Each time took two or three minutes. Each time produced a slightly different result because you wrote the prompt slightly differently.
...
A prompt template is a prompt with placeholders for the parts that vary. The structure, the role instruction, the format specification, the tone constraints — those are fixed. The details specific to this particular instance of the task — a client's name, the engagement, the date, the specific deliverable — are left as named blanks.

Here's a simple example. Say you regularly write internal engagement status updates. Without a template, you'd write something like this each time:

```
I'm a project lead at a 12-person IT support firm in Auckland.
We're wrapping up a network refresh engagement for a logistics client.
We're on track, but the access details from the client are two weeks
...
```
I'm a project lead at [FIRM NAME], a [TEAM SIZE]-person [FIRM TYPE]
...
Not every task deserves a template. If you do something once, the effort of building a template isn't worth it. Templates earn their keep on tasks you do repeatedly — weekly, fortnightly, or whenever a particular trigger occurs (a new client onboards, an engagement closes, a complaint comes in).

Go back to the task map you built in Lesson 1.4. Look for tasks with these characteristics:

**Same structure every time.** The output always follows the same format: a status update, a client email of a certain type, a meeting recap, an engagement summary, a social media post. The content changes but the shape doesn't.
...
Common candidates in AU/NZ professional services: client status emails, scopes of work for new engagements, follow-up emails after a discovery meeting, social media captions for a recurring content type, internal team briefings, onboarding messages for new clients, supplier inquiry emails, monthly reports to a business partner or board.
...
Now read through it and ask: which parts of this prompt would change if I ran this same task for a different client, engagement, or situation next time?

Those parts become your variables. Put them in brackets and give each one a name: `[CLIENT NAME]`, `[ENGAGEMENT STATUS]`, `[KEY UPDATE]`, `[WORD COUNT]`. Be descriptive in the bracket label — future you should know exactly what to put there without thinking.
...
**Role** — usually fixed. Claude's role in this task doesn't change based on who the client is. "You are writing from the perspective of a consultant communicating with a client" stays the same every time.

**Context** — partly fixed, partly variable. Your firm background is fixed (and can come from your context document). The situation-specific details — the client's name, the engagement, what happened this week — are variables.
...
**Format** — almost always fixed. Word count, layout, tone, what to include or exclude — these should be consistent for the same task type. If your client status emails are always under 200 words and never use bullet points, that's fixed in the template.
...
Templates stop working when your business context changes and the template doesn't. You change your fee structure and the template still mentions the old one. You shift your tone after client feedback and the template still has the old voice instruction. You add a new service line and the template doesn't know about it.
...
- Something about your firm changed — a new service, a different client type, a rebranded voice — and the template still describes the old situation.
