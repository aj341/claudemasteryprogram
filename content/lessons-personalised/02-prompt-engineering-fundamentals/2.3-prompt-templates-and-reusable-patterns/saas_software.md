...
Think about the last four weeks. You've sent Claude a prompt to write a customer renewal email. Then two weeks later, you wrote a different prompt for a different renewal email. Then again last week, another one. Each time, you started from scratch — typed out the context, the tone instruction, the format, the constraints. Each time took two or three minutes. Each time produced a slightly different result because you wrote the prompt slightly differently.
...
A prompt template is a prompt with placeholders for the parts that vary. The structure, the role instruction, the format specification, the tone constraints — those are fixed. The details specific to this particular instance of the task — a customer's name, the topic, the date, the specific feature — are left as named blanks.

Here's a simple example. Say you regularly write internal product status updates. Without a template, you'd write something like this each time:

```
I'm a co-founder at a 14-person vertical SaaS in Auckland.
We're wrapping up the discovery phase for a new bulk-import feature.
We're on track, but the engineering scoping from our two-person backend
team is two days behind. Write a short internal status update for the
team, under 150 words, direct tone, no jargon.
```
...
```
I'm a co-founder at [PRODUCT NAME], a [TEAM SIZE]-person [SAAS CATEGORY]
in [CITY]. I'm writing a weekly internal status update for the
[FEATURE OR PROJECT NAME] workstream.

Current status: [ON TRACK / AT RISK / DELAYED — choose one]
Key update this week: [ONE SENTENCE — what shipped or changed]
Blocker or watch item: [ONE SENTENCE — or write "None this week"]
Next milestone: [DATE AND WHAT IT IS]

Write the status update in a direct but warm tone, under 150 words,
...
That consistency is the point. When you use the same template for the same task type, you train your own expectations alongside Claude's inputs. You know what you're going to get. Your team knows what format to expect in Slack. The task becomes predictable in a good way.

### Finding the tasks worth templating

Not every task deserves a template. If you do something once, the effort of building a template isn't worth it. Templates earn their keep on tasks you do repeatedly — weekly, fortnightly, or whenever a particular trigger occurs (a new account onboards, a deal closes, a P1 incident happens).

Go back to the task map you built in Lesson 1.4. Look for tasks with these characteristics:

**Same structure every time.** The output always follows the same format: a status update, a customer email of a certain type, a release note, a meeting agenda, a LinkedIn post. The content changes but the shape doesn't.

**Same audience or same purpose.** You're always writing to customers, or to your team, or to a specific platform. The audience doesn't shift from instance to instance.
...
Common candidates in small AU/NZ SaaS teams: trial-expiry emails, demo follow-ups, incident post-mortems, release notes, meeting recaps after sales calls, social media captions for a recurring content type, internal team briefings, onboarding messages for new accounts, churn-risk outreach, monthly investor or board updates.

Look at your task map, {{firstName}}. Underline three tasks that fit these criteria. Those are your starting points.
...
Now read through it and ask: which parts of this prompt would change if I ran this same task for a different customer, feature, or situation next time?

Those parts become your variables. Put them in brackets and give each one a name: `[CUSTOMER NAME]`, `[ACCOUNT STATUS]`, `[KEY UPDATE]`, `[WORD COUNT]`. Be descriptive in the bracket label — future you should know exactly what to put there without thinking.

Everything you didn't mark as a variable stays fixed. That's the template structure.

Then do one more pass. Look at the fixed parts and ask: is this instruction specific enough that Claude would follow it correctly if I gave it brand-new context? If a constraint is vague — "write it professionally" — sharpen it now: "write it in a direct but warm tone; avoid SaaS jargon and filler phrases like 'we're thrilled to announce' or 'circling back.'"
...
**Role** — usually fixed. Claude's role in this task doesn't change based on which customer it is. "You are writing as the founder of a small B2B SaaS to a paying customer" stays the same every time.

**Context** — partly fixed, partly variable. Your product background is fixed (and can come from your context document). The situation-specific details — the customer's name, the feature, what happened this week — are variables.
...
**Format** — almost always fixed. Word count, layout, tone, what to include or exclude — these should be consistent for the same task type. If your renewal emails are always under 200 words and never use bullet points, that's fixed in the template.
...
A template with eight brackets to fill in — `[CUSTOMER NAME]`, `[CATEGORY]`, `[FEATURE TYPE]`, `[TIMELINE]`, `[MAIN OUTCOME]`, `[TONE]`, `[LENGTH]`, `[SPECIFIC CONSTRAINT]` — isn't really saving you time. You're still doing the thinking you'd do writing the prompt from scratch, just in a form with boxes to fill.
...
The storage medium matters less than the access habit. A Notion page you open every morning beats a beautiful Notion database you set up thoughtfully and visit once a week. A pinned note in your team Slack beats an elaborate folder structure you have to navigate.

A few options that work for different small SaaS teams:
...
**A single text document.** One file called `prompt-templates.md` on your desktop or in your cloud drive. No folders, no categories — just a running list of templates, each with a short heading. Ctrl+F to find the one you need.

**Notion or similar.** A database with one page per template works well if you're already in Notion and it's your default workspace. Add a tag for the task type (customer comms, release, internal, marketing) so you can filter fast.

**A pinned document in your team chat.** If you share these with teammates, Slack pinned messages get them there without another app.
...
Templates stop working when your business context changes and the template doesn't. You change your pricing structure and the template still mentions the old one. You shift your tone after customer feedback and the template still has the old voice instruction. You add a new tier and the template doesn't know about it.
...
- Something about your product changed — a new feature, a different ICP, a rebranded voice — and the template still describes the old situation.

When any of those happen, open the template and update it. This takes two minutes. The fix is almost always: add the correction you've been making manually into the fixed part of the template, or update the context to reflect how your product has changed.
