...
```
I'm {{firstName}}, the practice manager at a 6-clinician allied health practice
called {{businessName}} in Auckland. We're wrapping up a 12-week return-to-work
program for an injured warehouse client. We're on track, but the case-conference
report from the insurer is two weeks late. Write a short internal status update
for the clinical team, under 150 words, professional tone, no jargon.
```
...
```
I'm the practice manager at [PRACTICE NAME], a [TEAM SIZE]-clinician [PRACTICE TYPE]
in [CITY]. I'm writing a weekly internal status update for the
[CASE OR PROGRAM NAME] case.
...
Common candidates in AU/NZ allied health practices: client appointment-reminder emails, treatment-plan summaries, follow-up emails after first sessions, social media captions for a recurring content type, internal clinical-team briefings, intake or welcome messages for new clients, supplier or referrer enquiry emails, monthly reports to a practice owner or partner.
...
**A single text document.** One file called `prompt-templates.txt` (or `.md`) on {{firstName}}'s desktop or in your cloud drive. No folders, no categories — just a running list of templates, each with a short heading. Ctrl+F to find the one you need.
