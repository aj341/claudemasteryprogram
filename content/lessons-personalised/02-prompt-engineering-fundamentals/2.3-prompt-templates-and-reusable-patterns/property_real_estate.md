...
Think about the last four weeks. You've sent Claude a prompt to write a vendor update after Saturday's open home. Then a fortnight later, you wrote a different prompt for a different vendor on a different listing. Then again last weekend, another one. Each time, you started from scratch — typed out the context, the tone instruction, the format, the constraints. Each time took two or three minutes. Each time produced a slightly different result because you wrote the prompt slightly differently.
...
This lesson is about building those templates for the tasks that matter most in your agency or property management business, storing them somewhere you'll use them, and keeping them current.

### What a prompt template actually is

A prompt template is a prompt with placeholders for the parts that vary. The structure, the role instruction, the format specification, the tone constraints — those are fixed. The details specific to this particular instance of the task — a vendor's name, the property address, the open home numbers, the offer count — are left as named blanks.

Here's a simple example. Say you regularly write Saturday vendor updates after the OFI. Without a template, you'd write something like this each time:

```
I'm a sales agent at a 12-person agency in Newcastle.
We've just run an open home for a 3-bedroom house listed at $920k.
Eight groups came through, two requested second inspections, no offers
yet. Write a vendor update under 200 words, warm but direct, no jargon.
```
...
```
I'm a sales agent at [AGENCY NAME], a [TEAM SIZE]-person agency in [SUBURB
OR REGION]. I'm writing a Saturday vendor update for [PROPERTY ADDRESS]
listed at [PRICE GUIDE].

Today's OFI: [NUMBER OF GROUPS] groups through, [NUMBER] second-inspection
requests, [NUMBER] written offers — current top offer [OFFER OR "none yet"].
Buyer feedback theme: [ONE SENTENCE — what most buyers commented on]
Recommendation for next week: [ONE SENTENCE — adjust price, more marketing,
hold the line, etc.]

Write the vendor update in a warm but direct tone, under 200 words, no
jargon. Use plain paragraph format — no bullet points.
```

You fill in the brackets, you send it, and you get a consistently structured update every Saturday afternoon. The variable parts change; the structure and tone instruction stay fixed.

That consistency is the point. When you use the same template for the same task type, {{firstName}}, you train your own expectations alongside Claude's inputs. You know what you're going to get. Your vendors know what shape their weekly note will take. The task becomes predictable in a good way.

### Finding the tasks worth templating

Not every task deserves a template. If you do something once, the effort of building a template isn't worth it. Templates earn their keep on tasks you do repeatedly — weekly, fortnightly, or whenever a particular trigger occurs (a new listing comes on, a campaign closes, an offer comes in, a tenant gives notice).

Go back to the task map you built in Lesson 1.4. Look for tasks with these characteristics:

**Same structure every time.** The output always follows the same format: a vendor update, a buyer follow-up, a listing description, a property manager's monthly statement note, a strata agenda. The content changes but the shape doesn't.

**Same audience or same purpose.** You're always writing to vendors, or to landlords, or to a specific portal. The audience doesn't shift from instance to instance.
...
Common candidates in AU property businesses: weekly vendor updates, post-OFI buyer follow-ups, listing descriptions, owner reports for managed properties, lease renewal letters, prospecting emails to potential sellers, EOM property statement covers, condition report summaries for landlords, settlement update emails, strata committee agenda items.
...
Now read through it and ask: which parts of this prompt would change if I ran this same task for a different vendor, listing, or owner next time?

Those parts become your variables. Put them in brackets and give each one a name: `[VENDOR NAME]`, `[CAMPAIGN STATUS]`, `[BUYER FEEDBACK]`, `[WORD COUNT]`. Be descriptive in the bracket label — future you should know exactly what to put there without thinking.

Everything you didn't mark as a variable stays fixed. That's the template structure.

Then do one more pass. Look at the fixed parts and ask: is this instruction specific enough that Claude would follow it correctly if I gave it brand-new property details? If a constraint is vague — "write it professionally" — sharpen it now: "write it in a professional but warm tone; avoid agent jargon and filler phrases like 'we are pleased to advise' or 'as discussed.'"
...
**Role** — usually fixed. Claude's role in this task doesn't change based on which vendor it's for. "You are writing from the perspective of a sales agent communicating with a vendor mid-campaign" stays the same every time.

**Context** — partly fixed, partly variable. Your agency background is fixed (and can come from your context document). The situation-specific details — the vendor's name, the property, what happened at the OFI — are variables.
...
**Format** — almost always fixed. Word count, layout, tone, what to include or exclude — these should be consistent for the same task type. If your vendor updates are always under 200 words and never use bullet points, that's fixed in the template.
...
A template with eight brackets to fill in — `[VENDOR NAME]`, `[SUBURB]`, `[PROPERTY TYPE]`, `[CAMPAIGN WEEK]`, `[KEY OUTCOME]`, `[TONE]`, `[LENGTH]`, `[SPECIFIC CONSTRAINT]` — isn't really saving you time. You're still doing the thinking you'd do writing the prompt from scratch, just in a form with boxes to fill.
...
The most useful template library is the one you actually use. Which means it has to be somewhere you can reach in ten seconds, in the same moment you sit down to write the Saturday vendor updates or send a property manager a tenant letter.

The storage medium matters less than the access habit. A Google Doc you open every Saturday morning beats a Notion database you set up thoughtfully and visit once a month. A pinned note on your phone beats an elaborate folder structure you have to navigate between inspections.

A few options that work for different agents:
...
**Notion or similar.** A database with one page per template works well if you're already in Notion and it's your default workspace. Add a tag for the task type (vendor comms, buyer comms, PM, prospecting) so you can filter fast.

**A pinned document in your team chat.** If you share these with your sales team or PM team, Slack or Teams pinned messages get them there without another app.
...
Templates stop working when your business context changes and the template doesn't. You change your fee structure and the template still mentions the old one. You shift your tone after a vendor complaint and the template still has the old voice instruction. You take on a new market segment — say a development project alongside the residential rent roll — and the template doesn't know about it.
...
- Something about your business changed — a new service line, a different price bracket, a rebranded voice — and the template still describes the old situation.

When any of those happen, open the template and update it. This takes two minutes. The fix is almost always: add the correction you've been making manually into the fixed part of the template, or update the context to reflect how {{businessName}} has changed.

A template library that gets small updates when needed stays useful for years. One that never gets touched becomes a liability — you send it, you edit the output, you forget the fix, you edit again next campaign. The library should make your work faster, not add a maintenance problem. The rule is simple: if you corrected the output in the same way twice, update the template.
