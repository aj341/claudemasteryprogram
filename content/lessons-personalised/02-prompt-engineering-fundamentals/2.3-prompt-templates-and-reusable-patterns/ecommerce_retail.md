...
Think about the last four weeks. You've sent Claude a prompt to write a customer reply about a delayed order. Then two weeks later, you wrote a different prompt for a different customer reply about a different delayed order. Then again last week, another one. Each time, you started from scratch — typed out the context, the tone instruction, the format, the constraints. Each time took two or three minutes. Each time produced a slightly different result because you wrote the prompt slightly differently.
...
This lesson is about building those templates for the tasks that matter most in {{businessName}}, storing them somewhere you'll use them, and keeping them current.

### What a prompt template actually is

A prompt template is a prompt with placeholders for the parts that vary. The structure, the role instruction, the format specification, the tone constraints — those are fixed. The details specific to this particular instance of the task — a customer's name, the SKU, the delay length, the freight company — are left as named blanks.

Here's a simple example. Say you regularly write restock-alert emails when popular SKUs come back in. Without a template, you'd write something like this each time:

```
I run a small homewares brand on Shopify, based in regional Victoria.
Our linen tea towels in the rust colourway are about to come back into stock
after being sold out for six weeks. About 240 customers signed up to the
back-in-stock alert. Write a short email to that list, under 150 words,
warm tone, no fake urgency.
```
...
```
I run [BRAND NAME], a [BRAND DESCRIPTION] selling on Shopify.
Our [PRODUCT NAME] in [VARIANT/COLOURWAY] is about to come back into stock
after being sold out for [DURATION].

Back-in-stock list size: [NUMBER OF CUSTOMERS]
Restock quantity available: [UNITS]
Date stock goes live in store: [DATE]
Anything new about this batch (price change, slight variation, etc.): [ONE LINE — or "nothing new"]

Write a back-in-stock email to that list. Warm tone, plain language, no fake urgency,
no shouty caps. Under 150 words. End with a single link to the product page —
I'll add the URL myself.
```
...
That consistency is the point. When you use the same template for the same task type, you train your own expectations alongside Claude's inputs. You know what you're going to get. Your customers learn what to expect from your restock emails. The task becomes predictable in a good way.

### Finding the tasks worth templating

Not every task deserves a template. If you do something once, the effort of building a template isn't worth it. Templates earn their keep on tasks you do repeatedly — weekly, fortnightly, or whenever a particular trigger occurs (a top SKU sells out, a wholesale enquiry lands in your inbox, a customer asks about a missing parcel).

Go back to the task map you built in Lesson 1.4. Look for tasks with these characteristics:

**Same structure every time.** The output always follows the same format: a "where's my order" reply, a back-in-stock email, a returns acknowledgement, a wholesale first-reply, a product-launch announcement. The content changes but the shape doesn't.

**Same audience or same purpose.** You're always writing to existing customers, or to wholesale buyers, or to a specific platform. The audience doesn't shift from instance to instance.
...
Common candidates in AU retail: customer-service replies (where's my order, returns, damaged stock, sizing questions), restock-alert emails, new-arrival announcements, wholesale first-reply emails, post-purchase thank-you emails, EOFY and Black Friday campaign emails, supplier follow-up emails, social captions for product launches, and the standard reply when a customer asks about ingredients, materials, or care instructions.
...
Find a prompt you've sent that you were happy with — one where Claude returned something you actually used with minimal editing. Copy that prompt into a blank document.

Now read through it and ask: which parts of this prompt would change if I ran this same task for a different SKU, customer, or campaign next time?

Those parts become your variables. Put them in brackets and give each one a name: `[SKU NAME]`, `[CUSTOMER NAME]`, `[DELAY DAYS]`, `[FREIGHT COMPANY]`. Be descriptive in the bracket label — future you should know exactly what to put there without thinking.

Everything you didn't mark as a variable stays fixed. That's the template structure.

Then do one more pass. Look at the fixed parts and ask: is this instruction specific enough that Claude would follow it correctly if I gave it brand-new context? If a constraint is vague — "write it warmly" — sharpen it now: "write it in a warm, plain-language tone; avoid shouty caps and avoid filler phrases like 'thank you for your patience'."
...
**Role** — usually fixed. Claude's role in this task doesn't change based on which customer or which SKU is in front of you. "You are writing as the founder of a small Australian retail brand communicating with a customer" stays the same every time.

**Context** — partly fixed, partly variable. Your brand background is fixed (and can come from your context document). The situation-specific details — the customer, the SKU, the delay length, what happened with this particular order — are variables.

**Task** — fixed structure, variable specifics. "Write a [TYPE OF EMAIL]" is fixed. The details inside the task instruction might include variables: "covering [TOPIC]" or "for [AUDIENCE]."

**Format** — almost always fixed. Word count, layout, tone, what to include or exclude — these should be consistent for the same task type. If your "where's my order" replies are always under 120 words and never include a tracking link (because you'll add it yourself), that's fixed in the template.
...
A template with eight brackets to fill in — `[CUSTOMER NAME]`, `[SKU]`, `[ORDER DATE]`, `[DELAY DAYS]`, `[FREIGHT COMPANY]`, `[TONE]`, `[LENGTH]`, `[SPECIFIC OFFER]` — isn't really saving you time. You're still doing the thinking you'd do writing the prompt from scratch, just in a form with boxes to fill.

**Better approach:** Keep variables to the information that genuinely changes from instance to instance. Everything that stays the same for this task type — tone, length, format, who you are, how you write to customers — gets fixed in the template. Aim for two to four variables maximum on most task templates. If you find yourself with more than five, review which ones you can decide once and fix permanently.
...
A few options that work for different retailers:
...
**Notion or similar.** A database with one page per template works well if you're already in Notion and it's your default workspace. Add a tag for the task type (customer service, wholesale, social, campaign, supplier) so you can filter fast.

**A pinned document in your team chat.** If you share these with casuals or a part-time customer-service person, Slack or WhatsApp pinned messages get them there without another app.
...
Templates stop working when your retail context changes and the template doesn't. You change your free-shipping threshold and the template still mentions the old one. You shift your tone after a customer survey and the template still has the old voice. You add a new shipping carrier and the template doesn't know about it.
...
- Claude's output needs more editing than it used to. The output is still okay, but you're making the same corrections each time — the same line about freight, the same edit to the sign-off.
...
- Something about your business changed — a new SKU range, a different customer-service tone, a rebrand — and the template still describes the old situation.

When any of those happen, open the template and update it. This takes two minutes. The fix is almost always: add the correction you've been making manually into the fixed part of the template, or update the context to reflect how your store has changed.
