...
Think about the last four weeks at {{businessName}}. You've sent Claude a prompt to write a BAS reminder to one client. Then two weeks later, you wrote a different prompt for a different client's BAS reminder. Then again last week, a year-end client letter. Each time, you started from scratch — typed out the context, the tone instruction, the format, the constraints. Each time took two or three minutes. Each time produced a slightly different result because you wrote the prompt slightly differently.
...
This lesson is about building those templates for the tasks that matter most in your practice, storing them somewhere you'll use them, and keeping them current.

### What a prompt template actually is

A prompt template is a prompt with placeholders for the parts that vary. The structure, the role instruction, the format specification, the tone constraints — those are fixed. The details specific to this particular instance of the task — a client's name, the BAS period, the lodgement date, the specific outstanding item — are left as named blanks.

Here's a simple example. Say you regularly write quarterly BAS reminder emails to clients. Without a template, you'd write something like this each time:

```
I'm a BAS agent at a 3-person bookkeeping firm in Adelaide. I need to
remind a hospitality client that their Q2 BAS is due in 10 days and we're
still missing two months of supplier invoices. Write a short reminder
email, under 150 words, warm but firm tone, no jargon.
```
...
```
I'm a [BAS / TAX] agent at [PRACTICE NAME], a [TEAM SIZE]-person
[ACCOUNTING / BOOKKEEPING] practice in [CITY]. I'm sending a quarterly
BAS reminder to [CLIENT NAME], who runs a [CLIENT INDUSTRY] business.

BAS period: [QUARTER AND YEAR]
Lodgement due: [DATE]
Items still outstanding from client: [SHORT LIST OR "None"]
Relationship note: [ONE SENTENCE — e.g. "Long-term client, normally responsive"]

Write the reminder in a warm but firm tone, under 150 words, no jargon.
Plain paragraph format — no bullet points unless the outstanding list is
...
Not every task deserves a template. If you do something once, the effort of building a template isn't worth it. Templates earn their keep on tasks you do repeatedly — weekly, fortnightly, quarterly, or whenever a particular trigger occurs (a new client onboards, a BAS quarter ends, an ATO bulletin lands).

Go back to the task map you built in Lesson 1.4. Look for tasks with these characteristics:

**Same structure every time.** The output always follows the same format: a BAS reminder, a fee proposal of a certain type, an engagement letter, a meeting agenda, a year-end client checklist. The content changes but the shape doesn't.

**Same audience or same purpose.** You're always writing to clients, or to your team, or to the ATO. The audience doesn't shift from instance to instance.
...
Common candidates in AU practices: BAS reminder emails, fee proposals for a standard engagement type, lodgement extension requests to the ATO, scope-clarification emails, quarterly catch-up agendas, year-end client letters, onboarding checklists for new clients, supplier inquiry emails, monthly partner board updates.
...
Now read through it and ask: which parts of this prompt would change if I ran this same task for a different client, period, or situation next time?

Those parts become your variables. Put them in brackets and give each one a name: `[CLIENT NAME]`, `[BAS QUARTER]`, `[OUTSTANDING ITEMS]`, `[WORD COUNT]`. Be descriptive in the bracket label — future you should know exactly what to put there without thinking.

Everything you didn't mark as a variable stays fixed. That's the template structure.

Then do one more pass. Look at the fixed parts and ask: is this instruction specific enough that Claude would follow it correctly if I gave it brand-new context? If a constraint is vague — "write it professionally" — sharpen it now: "write it in a warm but firm tone; avoid corporate jargon and filler phrases like 'please do not hesitate to contact me.'"
...
**Role** — usually fixed. Claude's role in this task doesn't change based on which client this is. "You are writing from the perspective of an Australian accountant communicating with a small business client" stays the same every time.

**Context** — partly fixed, partly variable. Your practice background is fixed (and can come from your context document). The situation-specific details — the client's name, the BAS period, what's outstanding this week — are variables.
...
**Format** — almost always fixed. Word count, layout, tone, what to include or exclude — these should be consistent for the same task type. If your BAS reminders are always under 150 words and never use bullet points unless absolutely necessary, that's fixed in the template.
...
A template with eight brackets to fill in — `[CLIENT NAME]`, `[INDUSTRY]`, `[ENGAGEMENT TYPE]`, `[TIMELINE]`, `[MAIN OUTCOME]`, `[TONE]`, `[LENGTH]`, `[SPECIFIC CONSTRAINT]` — isn't really saving you time. You're still doing the thinking you'd do writing the prompt from scratch, just in a form with boxes to fill.
...
**Notion or similar.** A database with one page per template works well if you're already in Notion and it's your default workspace. Add a tag for the task type (client comms, ATO comms, internal, fee proposals) so you can filter fast.
...
Templates stop working when your practice context changes and the template doesn't. You change your fee structure and the template still mentions the old one. You shift your tone after client feedback and the template still has the old voice instruction. You add a new service line (say, advisory) and the template doesn't know about it.
...
- Something about your practice changed — a new service, a different client type, a rebranded voice, a new partner — and the template still describes the old situation.

When any of those happen, open the template and update it. This takes two minutes. The fix is almost always: add the correction you've been making manually into the fixed part of the template, or update the context to reflect how your practice has changed.
