...
Think about the last four weeks. You've sent Claude a prompt to write a quote follow-up for a homeowner. Then two weeks later, you wrote a different prompt for a different quote follow-up. Then again last week, another one. Each time, you started from scratch — typed out the context, the tone instruction, the format, the constraints. Each time took two or three minutes, usually in the ute between jobs. Each time produced a slightly different result because you wrote the prompt slightly differently.

That's a pattern you're repeating on a task Claude could handle consistently if you gave it a consistent input.

A prompt template is the solution. It's a prompt with deliberate blank spaces — variables — that you fill in with the details that change from job to job. Everything that stays the same across every instance of that task is already written. You only type what's new.

This lesson is about building those templates for the tasks that matter most at {{businessName}}, storing them somewhere you'll use them, and keeping them current.

### What a prompt template actually is

A prompt template is a prompt with placeholders for the parts that vary. The structure, the role instruction, the format specification, the tone constraints — those are fixed. The details specific to this particular instance of the task — a customer's name, the job, the date, the specific scope item — are left as named blanks.

Here's a simple example. Say you regularly write customer updates after a day on site. Without a template, you'd write something like this each time:

```
I run a 5-person residential building business in outer Brisbane.
We're partway through a kitchen and laundry reno for a homeowner.
We're on track, but the cabinet supplier is two weeks behind on the
custom doors. Write a short update for the homeowner, under 150 words,
warm but professional tone, no jargon.
```
...
```
I run [BUSINESS NAME], a [TEAM SIZE]-person [TRADE] business in [AREA].
I'm writing a weekly customer update for the [JOB DESCRIPTION] at
[CUSTOMER NAME]'s place.

Current status: [ON TRACK / AT RISK / DELAYED — choose one]
Key update this week: [ONE SENTENCE — what we did or what changed]
Watch item: [ONE SENTENCE — supplier delay, weather, variation — or "None this week"]
Next milestone: [DATE AND WHAT IT IS]

Write the update in a warm but direct tone, under 150 words, no jargon.
Use plain paragraph format — no bullet points.
```
...
That consistency is the point. When you use the same template for the same task type, you train your own expectations alongside Claude's inputs. You know what you're going to get. Your customer knows what format to expect each Friday. The task becomes predictable in a good way.

### Finding the tasks worth templating

Not every task deserves a template. If you do something once, the effort of building a template isn't worth it. Templates earn their keep on tasks you do repeatedly — weekly, fortnightly, or whenever a particular trigger occurs (a new job kicks off, a job wraps, a defect comes in).

Go back to the task map you built in Lesson 1.4. Look for tasks with these characteristics:

**Same structure every time.** The output always follows the same format: a customer update, a quote follow-up of a certain type, a variation order, a site-visit note, a Friday SMS to the homeowner. The content changes but the shape doesn't.

**Same audience or same purpose.** You're always writing to homeowners, or to builders you sub for, or to the property manager. The audience doesn't shift from instance to instance.
...
Common candidates in trades businesses: weekly customer updates, quote follow-ups, end-of-job wrap emails, variation order summaries, defect-liability responses, supplier back-order notes to customers, SMS reminders before a site visit, hazard reports for the builder, monthly job-list updates to the office.
...
Now read through it and ask: which parts of this prompt would change if I ran this same task for a different customer, job, or situation next time?

Those parts become your variables. Put them in brackets and give each one a name: `[CUSTOMER NAME]`, `[JOB STATUS]`, `[KEY UPDATE]`, `[WORD COUNT]`. Be descriptive in the bracket label — future you should know exactly what to put there without thinking when you're writing this in the cab at 6pm.

Everything you didn't mark as a variable stays fixed. That's the template structure.

Then do one more pass. Look at the fixed parts and ask: is this instruction specific enough that Claude would follow it correctly if I gave it brand-new context? If a constraint is vague — "write it professionally" — sharpen it now: "write it in a warm but direct tone; avoid corporate jargon and filler phrases like 'please do not hesitate to contact us.'"
...
**Role** — usually fixed. Claude's role in this task doesn't change based on who the customer is. "You are writing from the perspective of a tradie communicating with a homeowner" stays the same every time.

**Context** — partly fixed, partly variable. Your business background is fixed (and can come from your context document). The situation-specific details — the customer's name, the job, what happened on site this week — are variables.
...
**Format** — almost always fixed. Word count, layout, tone, what to include or exclude — these should be consistent for the same task type. If your customer updates are always under 200 words and never use bullet points, that's fixed in the template.
...
A template with eight brackets to fill in — `[CUSTOMER NAME]`, `[TRADE]`, `[JOB TYPE]`, `[TIMELINE]`, `[MAIN OUTCOME]`, `[TONE]`, `[LENGTH]`, `[SPECIFIC CONSTRAINT]` — isn't really saving you time. You're still doing the thinking you'd do writing the prompt from scratch, just in a form with boxes to fill.
...
The most useful template library is the one you actually use. Which means it has to be somewhere you can reach in ten seconds, in the same moment you sit down to do the task — usually in the cab of the ute or at the kitchen bench at 7pm.
...
**A single text document.** One file called `prompt-templates.txt` (or `.md`) on your phone or in your cloud drive. No folders, no categories — just a running list of templates, each with a short heading. Search for the one you need.

**Notion or similar.** A database with one page per template works well if you're already in Notion and it's your default workspace. Add a tag for the task type (customer comms, builder comms, internal, supplier) so you can filter fast.

**A pinned document in your team chat.** If you share these with the apprentice or office staff, WhatsApp or Teams pinned messages get them there without another app.
...
Templates stop working when your business context changes and the template doesn't. You change your hourly rate and the template still mentions the old one. You shift your tone after customer feedback and the template still has the old voice instruction. You add a new service line — solar batteries, hot-water replacements, split-system installs — and the template doesn't know about it.
...
- Something about the business changed — a new service, a different customer type, a rebranded voice — and the template still describes the old situation.

When any of those happen, open the template and update it. This takes two minutes. The fix is almost always: add the correction you've been making manually into the fixed part of the template, or update the context to reflect how the business has changed.
