...
Most people read a Claude response and ask: "Is this good?" That question is too broad to be useful. A response can be well-written, intelligent, and entirely wrong for your purpose. A response can be rough and exactly right for a quick file note. "Good" is too vague to act on.
...
**Checkpoint 1: Did Claude address the right task?** Sometimes a vague task instruction produces a response to the task Claude inferred rather than the task you meant. If you asked Claude to "review this scope letter," did it give editorial feedback, or did it summarise the scope letter instead?

**Checkpoint 2: Is the format right?** Is this a list when you needed prose? Big-4 audit formal when you needed conversational? Two pages when you needed one? Format mismatches are the easiest failures to spot — and usually the easiest to fix.

**Checkpoint 3: Is the scope right?** Did Claude go narrower than you needed — covering half the BAS reminder content? Or broader — adding lodgement caveats and TPB considerations you didn't ask for? Scope drift in both directions is common.

**Checkpoint 4: Is the content accurate and relevant?** Did Claude invent information you didn't provide — a section number, an ATO rate, a date? Did it make assumptions about your client, your engagement, or your fee structure that aren't true? Accuracy problems are usually a missing-context failure in disguise.
...
The response is reasonable, but it doesn't know enough about your situation to be useful. The language doesn't match the voice at {{businessName}}. The suggestions are generic rather than specific to AU practice work. The advice ignores something about your client that would have changed the answer completely. Claude isn't wrong — it just didn't have enough information to be right for you.
...
The content of the response is actually correct, but the shape is wrong. You needed bullet points, you got paragraphs. You needed a 100-word BAS reminder, you got 400 words. You needed a table, you got a list. You needed a professional client letter, you got something that reads like a Slack message.
...
Under-scoped: Claude wrote three sentences about a topic you needed two paragraphs on. You need to go back and be explicit about the depth you want. "Cover this in detail, including at least three specific examples from the FY scenario I described." That tells Claude the scope you intended.

Over-scoped: Claude produced a ten-point analysis when you asked for a single recommendation on entity structure. You wanted a decision, not a deliberation. "Give me one recommendation. One. Not a list of options — a specific recommendation with one sentence of reasoning." Explicit scope constraints prevent Claude from defaulting to comprehensiveness when you need decisiveness.
...
This matters more than it seems. If you don't know what fixed it, you can't apply the same fix to the next similar prompt. You're back to starting from scratch each time, rather than building a library of patterns that work for your practice.
...
Not every task needs a perfect output. A draft internal email to your two staff, a quick summary of a software vendor's terms for your own file, a rough agenda for a partner catch-up tomorrow — these tasks do not require four rounds of iteration. They require a result that's good enough to serve the purpose. Once you have that, stop.
...
**Ask: is the remaining gap something Claude can close, or something you'll have to add yourself anyway?** Some tasks need human judgment that Claude can't replicate — you know the politics of this long-running client relationship, or the history behind this particular Div 7A treatment, or the specific nuance of your firm's voice that you haven't documented anywhere. At that point, iteration won't close the gap. Take the closest output and edit it yourself.

**Ask: what's the actual cost of a small imperfection here?** For something going to a top-10 client or to the ATO, zero tolerance for imperfection is reasonable. For an internal process note that three people will read once, "mostly right" is entirely sufficient.
...
table: left column is the outstanding item, right column is the responsible
...
```
The scope is too broad. You've covered six client scenarios when I only
need the one I described — a new sole-trader client who hasn't signed
the engagement letter yet. Remove the sections on existing clients and
FY rollovers entirely. Focus everything on the first onboarding scenario.
```
