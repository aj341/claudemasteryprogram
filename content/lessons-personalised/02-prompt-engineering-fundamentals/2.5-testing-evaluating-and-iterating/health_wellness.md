...
Sometimes that prediction — yours, {{firstName}}'s, anyone's — is right on the first attempt. More often it's partially right. Occasionally it misses in ways that are genuinely instructive.
...
```
Reformat this as a table with two columns: home-exercise action and clinician owner.
...
the one I described — a new client who hasn't booked their first appointment
yet. Remove the sections on returning clients and re-referrals entirely.
Focus everything on the first-session scenario.
```
