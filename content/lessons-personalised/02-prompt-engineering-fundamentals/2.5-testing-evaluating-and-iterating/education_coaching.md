...
The fix is always the same: go back to the prompt and add the specific information that was missing, {{firstName}}. Not more context in general — the precise detail that would have produced a different response. If Claude wrote a formal email when your relationship with this parent is casual, add: "We've worked with this family for three years, communication is informal, sign off with first names." That's the missing context. Add it.
...
A useful heuristic: if you're on Version 4 of a prompt for a task at {{businessName}} that would have taken you ten minutes to do manually, you should stop iterating and either use Version 3 or do the task without Claude. The goal is to save time, not to prove Claude can be perfect.
