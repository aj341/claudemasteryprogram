...
**Checkpoint 1: Did Claude address the right task?** Sometimes a vague task instruction produces a response to the task Claude inferred rather than the task you meant. If you asked Claude to "review this product description," did it give editorial feedback, or did it just paraphrase the description back to you?

**Checkpoint 2: Is the format right?** Is this a list when you needed prose? Formal when you needed warm and chatty? A 250-word email when you needed a 90-word reply? Format mismatches are the easiest failures to spot — and usually the easiest to fix.
...
**Checkpoint 4: Is the content accurate and relevant?** Did Claude invent stock-on-hand numbers, prices, or dispatch dates you didn't provide? Did it make assumptions about your customer, your range, or your situation that aren't true? Accuracy problems are usually a missing-context failure in disguise.
...
The response is reasonable, but it doesn't know enough about your situation to be useful. The language doesn't match your brand voice. The product description sounds like every other Shopify store. The advice ignores something about your range that would have changed the answer completely. Claude isn't wrong — it just didn't have enough information to be right for you.

The fix is always the same: go back to the prompt and add the specific information that was missing. Not more context in general — the precise detail that would have produced a different response. If Claude wrote a formal customer email when your brand voice is warm and chatty, add: "We write to customers like a friend writing back, first-name basis, sign off with my first name only." That's the missing context. Add it.

**Failure type 2: Wrong format**

The content of the response is actually correct, but the shape is wrong. You needed a bulleted FAQ, you got paragraphs. You needed a 90-word post-purchase email, you got 320 words. You needed a table, you got a list. You needed a warm customer-facing tone, you got something that reads like a corporate press release.

The fix is a format instruction. These don't need to be long:

```
Reformat this as a table with two columns: returns reason and recommended next step.
...
Under-scoped: Claude wrote three sentences about a product launch you needed two paragraphs on. You need to go back and be explicit about the depth you want. "Cover this in detail, including at least three specific product features and one line on the launch offer." That tells Claude the scope you intended.

Over-scoped: Claude produced a ten-point campaign analysis when you asked for a single subject line. You wanted a deliverable, not a deliberation. "Give me one subject line. One. Not a list of options — a specific subject line with one sentence of reasoning." Explicit scope constraints prevent Claude from defaulting to comprehensiveness when you need decisiveness.
...
This matters more than it seems. If you don't know what fixed it, you can't apply the same fix to the next similar prompt. You're back to starting from scratch each time, rather than building a library of patterns that work for your store.
...
Not every task needs a perfect output. A draft internal note to your two casuals, a quick summary of a supplier's shipping terms for your own reference, a rough agenda for tomorrow's stocktake meeting — these tasks do not require four rounds of iteration. They require a result that's good enough to serve the purpose. Once you have that, stop.
...
**Ask: is the remaining gap something Claude can close, or something you'll have to add yourself anyway?** Some tasks need human judgment that Claude can't replicate — you know the politics of this stockist relationship, or the history behind this particular customer complaint, or the specific nuance of your brand voice that you haven't documented anywhere. At that point, iteration won't close the gap. Take the closest output and edit it yourself.

**Ask: what's the actual cost of a small imperfection here?** For a Black Friday email blast going to your full list, zero tolerance for imperfection is reasonable. For an internal restock note that two people will read once, "mostly right" is entirely sufficient.
...
This gives Claude nothing to work with, {{firstName}}. It knows the output wasn't satisfactory, but it doesn't know what specifically missed the mark. Claude will produce a variation on the same response — usually slightly different in surface features, not fundamentally improved. You'll get Version 2 of the same problem.
...
table: left column is the returns reason, right column is our standard
response.
```
...
```
The scope is too broad. You've written for both retail and wholesale customers
when I only need the retail version — a customer who placed a $95 order direct
through Shopify, not a stockist who placed a wholesale order. Remove the
wholesale sections entirely. Focus everything on the retail customer scenario.
```
