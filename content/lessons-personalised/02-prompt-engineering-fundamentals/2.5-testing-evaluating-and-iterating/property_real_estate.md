...
The agents who get consistently good results from Claude are not the ones who write better first prompts. They're the ones who treat the first response as information. They read it, compare it to what they actually needed for that vendor or that lease renewal, identify specifically where it fell short, and make a targeted change. One change. Then they read again.

That cycle — write, evaluate, one targeted change, read again — is the skill this lesson teaches. It's not glamorous. But it's what separates the agencies where Claude is genuinely useful from the ones where it's hit and miss.
...
Most people read a Claude response and ask: "Is this good?" That question is too broad to be useful. A vendor update can be well-written, intelligent, and entirely wrong for that vendor. A buyer follow-up can be rough and exactly right. "Good" is too vague to act on.
...
Work through these checkpoints when you read a response, {{firstName}}:

**Checkpoint 1: Did Claude address the right task?** Sometimes a vague task instruction produces a response to the task Claude inferred rather than the task you meant. If you asked Claude to "review this contract clause," did it give you a plain-English explanation, or did it summarise the whole contract instead?

**Checkpoint 2: Is the format right?** Is this a list when you needed prose? Formal when you needed warm? Two pages when you needed half a page? Format mismatches are the easiest failures to spot — and usually the easiest to fix.

**Checkpoint 3: Is the scope right?** Did Claude go narrower than you needed — covering half the campaign? Or broader — adding sections and caveats and considerations you didn't ask for? Scope drift in both directions is common.

**Checkpoint 4: Is the content accurate and relevant?** Did Claude invent buyer feedback you didn't provide? Did it make assumptions about your office, your vendors, or the property that aren't true? Accuracy problems are usually a missing-context failure in disguise.
...
The response is reasonable, but it doesn't know enough about your situation to be useful. The language doesn't match how you write to vendors. The suggestions are generic rather than specific to a regional NSW market or a Sydney apartment campaign. The advice ignores something about the property — the heritage overlay, the strata levies, the recent flood event — that would have changed the answer completely. Claude isn't wrong — it just didn't have enough information to be right for you.

The fix is always the same: go back to the prompt and add the specific information that was missing. Not more context in general — the precise detail that would have produced a different response. If Claude wrote a formal vendor update when your relationship with this vendor is informal, add: "We've sold two properties for this vendor before, communication is on a first-name basis, sign off with my first name only." That's the missing context. Add it.

**Failure type 2: Wrong format**

The content of the response is actually correct, but the shape is wrong. You needed bullet points, you got paragraphs. You needed a 150-word vendor SMS-friendly summary, you got 400 words of polished prose. You needed a comparison table of two offers, you got a list. You needed a polished campaign report, you got something that reads like a chat message.

The fix is a format instruction. These don't need to be long:

```
Reformat this as a table with two columns: offer detail and how it compares.
...
Under-scoped: Claude wrote three sentences about the marketing campaign when you needed two paragraphs covering portal performance, signboard enquiries, and the print run. You need to go back and be explicit about the depth you want. "Cover this in detail, including specific numbers from the scenario I described — portal hits, enquiry counts, and group attendance at each OFI." That tells Claude the scope you intended.

Over-scoped: Claude produced a ten-point analysis when you asked for a recommendation on the price guide. You wanted a decision, not a deliberation. "Give me one recommendation. One. Not a list of options — a specific price guide adjustment with one sentence of reasoning I can take to the vendor." Explicit scope constraints prevent Claude from defaulting to comprehensiveness when you need decisiveness.
...
This matters more than it seems. If you don't know what fixed it, you can't apply the same fix to the next vendor update or the next buyer follow-up. You're back to starting from scratch each time, rather than building a library of patterns that work.
...
Not every task needs a perfect output. A draft internal note to your sales team of three, a quick summary of a strata report for your own reference, a rough agenda for a Monday morning meeting tomorrow — these tasks do not require four rounds of iteration. They require a result that's good enough to serve the purpose. Once you have that, stop.
...
**Ask: is the remaining gap something Claude can close, or something you'll have to add yourself anyway?** Some tasks need human judgment that Claude can't replicate — you know the politics of this vendor relationship, or the history behind why this landlord parted ways with their last PM, or the specific nuance of how {{businessName}} talks to first-home buyers that you haven't documented anywhere. At that point, iteration won't close the gap. Take the closest output and edit it yourself.

**Ask: what's the actual cost of a small imperfection here?** For a prestige listing presentation going to a $5m vendor, zero tolerance for imperfection is reasonable. For an internal handover note that two property managers will read once, "mostly right" is entirely sufficient.
...
A useful heuristic: if you're on Version 4 of a prompt for a task that would have taken you ten minutes to write yourself, you should stop iterating and either use Version 3 or do the task without Claude. The goal is to save time, not to prove Claude can be perfect.
...
table: left column is the offer feature, right column is the implication
for the vendor.
```
...
```
The scope is too broad. You've covered six campaign scenarios when I only
need the one I described — a private treaty campaign in week three with
no offers yet. Remove the sections on auction campaigns and post-auction
negotiations entirely. Focus everything on the no-offers-week-three scenario.
```
