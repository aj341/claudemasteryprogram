...
This technique earns its place on complex, multi-variable decisions. Think: deciding which billing tool to migrate to based on cost, integration depth, and migration risk. Reviewing a security questionnaire reply that has several possible interpretations. Evaluating whether to take on a customer based on contract value, support load, and ICP fit. These are tasks where the answer is not obvious, where several factors pull in different directions, and where you want to see the working — not just the output.

Chain-of-thought prompting is overkill for simple tasks. If you want a release-note headline, you don't need Claude to reason through it. If you want three LinkedIn caption options, showing its working adds noise, not value. The signal that you need chain-of-thought is when you find yourself reading a Claude answer and thinking "how did it get there?" — or when the task involves trade-offs and you want to understand what Claude is weighing.

```
I'm deciding between two analytics vendors for our 12-person SaaS. Here are the key facts:

Vendor A: $120/month, 30-day onboarding, 3 years in market, two existing customers in B2B SaaS at our scale.
Vendor B: $260/month, 5-day onboarding, 8 years in market, specialist in product-led growth funnels.

Think through this step by step — consider cost, time-to-value, market track record, and what matters most for a small SaaS trying to lift trial-to-paid conversion — then give me a clear recommendation with a one-line reason.
```
...
This technique is especially useful when you have a distinctive voice or format that's difficult to specify through instructions alone. Writing your own outbound emails in a particular style. Describing features in a way that's specific to your product. Writing release notes that match a tone your customers already know. If you spend thirty minutes trying to describe the style in words — and Claude still doesn't quite get it — try showing it two examples instead.
...
```
Here are two examples of how I write demo follow-up emails for my SaaS:

Example 1:
Hi Sarah, thanks for the time on Thursday. The two things you flagged as deal-breakers — multi-currency and a Xero export — are both on the roadmap for Q3. Happy to share the actual specs if that would help. Either way, no rush.

Example 2:
Hi Marcus, good chat on Monday. You mentioned your finance lead wants to see how the audit log handles role changes — I've attached a 60-second Loom showing exactly that. If the timing's not right yet, completely fine — just shout when it is.

Now write a follow-up for this situation: I demoed our quoting tool to a 9-person consultancy founder called James three weeks ago. He hasn't replied. Plan was $69/month annual. Same tone and style as the examples above.
```

Claude now has two reference points for what the voice looks like in practice. "No rush" tone, first name only, short sentences, an offer to share something useful rather than a hard follow-up. The instruction "same tone and style as the examples above" locks that in.

### Technique 3: Role prompting with specificity

You've probably seen the advice to tell Claude to "act as an expert" before asking a question. This is a weak version of role prompting. An "expert" is too generic to change anything meaningful about the output. Expert in what? From what category? With what priorities?
...
Weak: "You are a SaaS expert."
Stronger: "You are the head of customer success at a 25-person vertical SaaS in healthtech. You're reviewing the churn-and-expansion numbers at the end of a quarter where gross retention slipped from 96% to 91%."
...
The most effective role prompts for small SaaS tasks tend to be:

- A specific professional reviewing a specific type of document ("You are a security reviewer reading a SaaS vendor's SOC 2 readiness assessment for a small AU fintech")
- A specific customer type responding to a specific piece of communication ("You are the non-technical co-founder of a 4-person consultancy who just received this product launch email — read it and tell me what stands out, what confuses you, and what you'd expect to hear next")
- A specific internal voice evaluating a decision ("You are the technical co-founder of a 14-person SaaS reviewing a proposal to migrate from Postgres to a managed alternative")
...
When you ask Claude to write a release note, it will produce a release note. But whose format? One that reflects whatever release-note conventions are most common in its training data — which may or may not match how your team structures them. If you already know what a good output looks like for your context, the fastest way to get it is to provide the skeleton and ask Claude to fill it in.

This is the difference between:

"Write a release note for the new bulk-import feature."

And:

```
Complete this release note using the information I'll give you. Use exactly this structure:

RELEASE NOTE — [VERSION]

What's new:
Why we built it: [2–3 sentences on the customer problem this solves]
How it works: [bullet list — the 3 things a user actually does]
Things to know: [bullet list — limitations, edge cases — if none, write "None"]
Where to find it: [one sentence on the entry point in the product]
Coming next: [one sentence on the related improvement on the roadmap, or "Nothing scheduled"]

Here is the feature information: [paste details]
```
...
If the task is simple, the best prompt is usually simple. "Summarise this support thread in three bullet points." "Rewrite this paragraph in plain English." "Give me five subject line options for this customer-success email." None of these benefit from chain-of-thought, few-shot examples, a detailed role, or a structural scaffold. They benefit from being clear about the task, the audience, and the format. That's it.

Ask yourself before adding a technique: what specific problem is this technique solving? If you can't answer that question directly — "I'm using few-shot because our brand voice is hard to describe in abstract terms" or "I'm using chain-of-thought because I need to see the trade-off reasoning before I decide" — don't use it. A technique used without a reason is just noise in the prompt.
