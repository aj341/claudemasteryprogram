...
Lessons 2.1 through 2.3 covered the fundamentals: how Claude reads your prompt, how to structure it, and how to use constraints to get a specific format. Those fundamentals do a lot of work. Most tasks you run in a campaign week — vendor updates, buyer follow-ups, listing copy — will respond well to a clear, specific, well-constrained prompt. No special technique required.

But there's a category of task where the fundamentals aren't quite enough. Complex reasoning tasks like deciding between two offer scenarios. Tasks where you want output that sounds like a very specific person — your principal, your senior PM — in a very specific situation. Tasks where you want Claude to consistently match a vendor report format you already have rather than inventing one of its own. Tasks where giving Claude one example of how you write to a landlord produces better results than a page of instructions describing the tone.
...
Why does this help? When Claude works through a problem sequentially — rather than jumping straight to an output — it catches errors in its own reasoning, considers factors it might otherwise skip, and produces conclusions that are grounded in visible logic rather than pattern-matched from training. You can also read the reasoning and spot where Claude made an assumption you disagree with, rather than just staring at a recommendation that doesn't feel right.

This technique earns its place on complex, multi-variable tasks. Think: deciding between two competing offers where price, settlement length, finance status and deposit size all differ. Reviewing a special condition in a contract that has several possible interpretations. Evaluating whether to take a listing on at the price the vendor wants. These are tasks where the answer is not obvious, where several factors pull in different directions, and where you want to see the working — not just the output.

Chain-of-thought prompting is overkill for simple tasks. If you want a subject line for a buyer email, you don't need Claude to reason through it. If you want three Instagram caption options for a new listing, showing its working adds noise, not value. The signal that you need chain-of-thought is when you find yourself reading a Claude answer and thinking "how did it get there?" — or when the task involves trade-offs and you want to understand what Claude is weighing.

```
I'm advising a vendor on two offers received before auction. Here are the key facts:

Offer A: $1.42m, 30-day settlement, 10% deposit, finance approved, family buyer who's been to three opens.
Offer B: $1.46m, 90-day settlement, 5% deposit, subject to finance (pre-approval only), investor buying remotely.

Think through this step by step — consider price, certainty, settlement timing, deposit risk, and the vendor's stated need to move into their next purchase by end of next month — then give me a clear recommendation with a one-line reason I can take to the vendor.
```
...
This technique is especially useful when you have a distinctive voice or format that's difficult to specify through instructions alone. Writing your own prospecting letters in a particular style. Describing properties in a way that's specific to your office. Writing buyer follow-ups that match a tone your team already uses. If you spend thirty minutes trying to describe the style in words — and Claude still doesn't quite get it — try showing it two examples instead.
...
```
Here are two examples of how I write buyer follow-ups after an open home for {{businessName}}:

Example 1:
Hi Sarah, great to meet you at the open at 14 Bayview Crescent on Saturday. If you'd like to take another look mid-week or bring your partner through, just let me know — happy to open it up. No pressure either way, just here if you want to talk through it.

Example 2:
Hi Marcus, thanks for coming through 22 Eldridge Street on the weekend. If the floorplan or the price guide raised any questions, I'd rather you ask than assume — happy to chat through it whenever suits. If it's not the one for you, also fine, just let me know what you're looking for and I'll keep an eye out.

Now write a follow-up for this situation: a buyer named James came through 8 Riverbend Road on Saturday with his wife. Asking price guide $1.1m–$1.2m. He stayed 20 minutes, asked about the school catchment, didn't book a second viewing. Same tone and style as the examples above.
```

Claude now has two reference points for what the voice looks like in practice. Warm but not pushy, first name only, short sentences, an offer to open the property again rather than a hard chase. The instruction "same tone and style as the examples above" locks that in.

### Technique 3: Role prompting with specificity

You've probably seen the advice to tell Claude to "act as an expert" before asking a question. This is a weak version of role prompting. An "expert" is too generic to change anything meaningful about the output. Expert in what? From what part of the property industry? With what priorities?
...
Weak: "You are a property expert."
Stronger: "You are the principal of a 12-person sales agency in regional NSW. You're reviewing a CMA your second-year sales rep has prepared for a vendor in a suburb where the median has dropped 4% in the last quarter."
...
The most effective role prompts for property tasks tend to be:

- A specific professional reviewing a specific type of document ("You are a conveyancer reviewing the special conditions in a contract where the buyer has requested a long settlement and a finance clause")
- A specific buyer or vendor responding to a specific piece of communication ("You are a 65-year-old vendor selling the family home of 30 years, reading this campaign update — tell me what reassures you, what worries you, and what you'd want to hear from your agent next")
- A specific internal voice evaluating a decision ("You are the head of property management for a 40-property strata portfolio reviewing a proposed maintenance contractor change")
...
When you ask Claude to write a vendor report, it will produce a vendor report. But whose report format? One that reflects whatever conventions are most common in its training data — which may or may not match how your office or your franchise group structures campaign updates. If you already know what a good output looks like for your context, the fastest way to get it is to provide the skeleton and ask Claude to fill it in.

This is the difference between:

"Write a campaign summary for a completed sales campaign."

And:

```
Complete this campaign summary using the information I'll give you. Use exactly this structure:

CAMPAIGN SUMMARY

Property:
Vendor:
List date and method (auction / private treaty / EOI):
Marketing summary: [2–3 sentences on portals, signboard, print, social]
Inspection numbers: [groups through each open / mid-week]
Buyer enquiry quality: [bullet list — top three themes from buyer feedback]
Offer history: [bullet list — date, amount, conditions, outcome]
Final result: [sale price, contract date, settlement date, or "withdrawn"]
Lessons for next similar campaign: [one sentence]

Here is the campaign information: [paste details]
```
...
Output scaffolding is particularly useful for: recurring property documents with a standard format (campaign summaries, weekly vendor updates, EOM landlord statements), client-facing templates that have to match your office's structure, and any task where you already know what a good output looks like — you just need the content generated.

### Common mistake → Better approach

**Mistake:** Using multiple advanced techniques at once on a simple task.

An agent asks Claude to review a draft buyer email, but the prompt includes: a detailed role specification, three example emails, a step-by-step reasoning instruction, and a detailed output scaffold. The prompt is 600 words. The task — reviewing a 100-word buyer follow-up — takes fifteen seconds to read. The elaborate prompt doesn't improve the output. It often makes it worse, because Claude spends its effort responding to the complexity of the prompt rather than the simplicity of the task.

**Better approach:** Match the technique to the task, {{firstName}}. A simple review task needs a clear question and maybe a constraint or two ("flag any sentences that sound like agent jargon"). Few-shot prompting earns its place when your voice is hard to describe. Role prompting earns its place when a specific perspective genuinely changes what you need. Chain-of-thought earns its place when the task involves trade-offs or multi-step reasoning. Output scaffolding earns its place when you have an established format that Claude can't infer. Use one at a time. Evaluate whether it changed the output. If it didn't, take it out.
...
If the task is simple, the best prompt is usually simple. "Summarise this buyer email in three bullet points." "Rewrite this listing description in plain English." "Give me five subject line options for the vendor newsletter." None of these benefit from chain-of-thought, few-shot examples, a detailed role, or a structural scaffold. They benefit from being clear about the task, the audience, and the format. That's it.

Ask yourself before adding a technique: what specific problem is this technique solving? If you can't answer that question directly — "I'm using few-shot because my prospecting voice is hard to describe in abstract terms" or "I'm using chain-of-thought because I need to see the trade-off reasoning before I recommend an offer to the vendor" — don't use it. A technique used without a reason is just noise in the prompt.
