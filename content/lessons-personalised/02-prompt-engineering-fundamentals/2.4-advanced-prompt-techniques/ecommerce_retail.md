...
This technique earns its place on complex, multi-variable retail decisions. Think: deciding which freight carrier to switch to based on price, transit time, and reliability. Reviewing a wholesale agreement clause that has several possible interpretations. Evaluating whether to drop a slow-moving SKU based on margin, sell-through, and stockholding cost. These are tasks where the answer is not obvious, where several factors pull in different directions, and where you want to see the working — not just the output.

Chain-of-thought prompting is overkill for simple tasks. If you want a subject line for a restock email, you don't need Claude to reason through it. If you want three Instagram caption options, showing its working adds noise, not value. The signal that you need chain-of-thought is when you find yourself reading a Claude answer and thinking "how did it get there?" — or when the task involves trade-offs and you want to understand what Claude is weighing.

```
I'm deciding between two freight carriers for my Shopify orders. Here are the key facts:

Carrier A: $9.50 average per parcel, 4–6 business day transit metro, 12 months in business, well-reviewed for damage rates but no track record with fragile homewares.
Carrier B: $11.20 average per parcel, 2–3 business day transit metro, 8 years in business, specialist in homewares and ceramics with insured handling.

Think through this step by step — consider per-parcel cost, transit speed, damage risk on fragile stock, and what matters most for a small homewares brand whose customers expect their order in under a week — then give me a clear recommendation with a one-line reason.
```
...
This technique is especially useful when you have a distinctive voice or format that's difficult to specify through instructions alone. Writing your own back-in-stock emails in a particular voice. Describing products in a way that's specific to your brand. Writing customer-service replies that match a tone your team already knows. If you spend thirty minutes trying to describe the style in words — and Claude still doesn't quite get it — try showing it two examples instead.

The structure is simple: provide the examples clearly labelled as examples, then provide the actual request. Keep the examples realistic and representative. If you give Claude an unusually polished example, it will aim at that level for everything. If you give it an example that's too casual, it will match that casualness even when you need something more considered. The examples you choose are the calibration.

```
Here are two examples of how I write "where's my order" replies for my Shopify homewares store:

Example 1:
Hi Sarah, just checking on your order — I can see it shipped Tuesday with Sendle and it's currently sitting in their Sydney depot. Sendle's tracking is a bit slow to update but the parcel is moving. I'd expect it with you by end of this week. I'll keep an eye on it; if it doesn't move by Friday I'll chase them directly.

Example 2:
Hi Marcus, sorry for the wait — I've checked your order and the freight company has had a delay at their Melbourne hub. Your parcel is still on its way, just slower than we'd like. Realistically, I'd expect it Wednesday or Thursday next week. If it's not with you by then, write back and I'll send a replacement at our cost.

Now write a reply for this situation: Customer Anna emailed today saying she ordered our linen tea towels two weeks ago for a birthday gift and the parcel still hasn't arrived. The order shipped 12 days ago via Australia Post and tracking last updated 6 days ago in transit between Brisbane and Cairns. Same tone and style as the examples above.
```

Claude now has two reference points for what the voice looks like in practice. First name only, short sentences, an honest assessment of what's happening, and an offer to act if it doesn't resolve. The instruction "same tone and style as the examples above" locks that in.
...
Weak: "You are a retail expert."
Stronger: "You are the buyer for a small chain of three Australian gift shops with average store turnover of $400k. You're reviewing a wholesale line sheet from a new homewares brand whose first email pitch landed in your inbox this morning."

The stronger version gives Claude a character who has a job, a context, and a specific situation in front of them. That context shapes what Claude notices, what it flags, and how it frames its response. It's not asking Claude to pretend — it's giving Claude a point of view that makes its response more useful.

The most effective role prompts for retail tasks tend to be:

- A specific buyer reviewing a specific type of pitch ("You are the buyer for a small Sydney homewares boutique reviewing a wholesale pitch from an emerging Victorian ceramics brand")
- A specific customer responding to a specific piece of communication ("You are a 58-year-old customer who has just received this back-in-stock email — read it and tell me what stands out, what feels pushy, and what would make you click through")
- A specific internal voice evaluating a decision ("You are the operations lead for a 6-person Shopify homewares brand reviewing a proposed change from Sendle to Australia Post for all metro orders")
...
When you ask Claude to write a wholesale line sheet, it will produce a wholesale line sheet. But whose format? One that reflects whatever line-sheet conventions are most common in its training data — which may or may not match how your brand or your stockists structure them. If you already know what a good output looks like for your context, the fastest way to get it is to provide the skeleton and ask Claude to fill it in.

This is the difference between:

"Write a returns and exchanges policy for our Shopify store."

And:

```
Complete this returns and exchanges policy using the information I'll give you. Use exactly this structure:

RETURNS & EXCHANGES — [BRAND NAME]

Window: [number of days from delivery]
What we accept: [2–3 sentences on which items are returnable, in what condition]
What we don't accept: [bullet list — sale items, personal-care, custom orders, etc.]
How to start a return: [one paragraph on the email-us-first process]
Refund vs. credit vs. exchange: [one sentence on which option applies when]
Who pays return shipping: [one sentence]
Damaged or faulty stock: [one paragraph — separate from change-of-mind returns]
Australian Consumer Law: [one short paragraph noting it sits above this policy]

Here is the policy information for our brand: [paste details]
```
...
Output scaffolding is particularly useful for: recurring retail documents with a standard format (returns policies, shipping pages, line sheets, wholesale order forms), customer-facing templates that have to match your brand's structure, and any task where you already know what a good output looks like — you just need the content generated.

### Common mistake → Better approach

**Mistake:** Using multiple advanced techniques at once on a simple task.

A retailer asks Claude to review a draft customer email, but the prompt includes: a detailed role specification, three example emails, a step-by-step reasoning instruction, and a detailed output scaffold. The prompt is 600 words. The task — reviewing a 100-word email — takes fifteen seconds to read. The elaborate prompt doesn't improve the output. It often makes it worse, because Claude spends its effort responding to the complexity of the prompt rather than the simplicity of the task.

**Better approach:** Match the technique to the task. A simple review task needs a clear question and maybe a constraint or two ("flag any sentences that sound like fake-urgency marketing copy"). Few-shot prompting earns its place when your voice is hard to describe. Role prompting earns its place when a specific perspective genuinely changes what you need. Chain-of-thought earns its place when the task involves trade-offs or multi-step reasoning. Output scaffolding earns its place when you have an established format that Claude can't infer. Use one at a time. Evaluate whether it changed the output. If it didn't, take it out.

### When not to use advanced techniques

The most important thing this lesson teaches, {{firstName}}, is not when to use advanced techniques — it's when not to.
...
If the task is simple, the best prompt is usually simple. "Summarise this customer-service thread in three bullet points." "Rewrite this product description in plain English." "Give me five subject line options for our Mother's Day email." None of these benefit from chain-of-thought, few-shot examples, a detailed role, or a structural scaffold. They benefit from being clear about the task, the audience, and the format. That's it.

Ask yourself before adding a technique: what specific problem is this technique solving? If you can't answer that question directly — "I'm using few-shot because my brand voice is hard to describe in abstract terms" or "I'm using chain-of-thought because I need to see the trade-off reasoning before I switch carriers" — don't use it. A technique used without a reason is just noise in the prompt.

The trap is complexity for its own sake. More elaborate prompts do not reliably produce better outputs. They sometimes produce worse ones, and they always produce slower workflows. The fundamentals from Lessons 2.1 through 2.3 handle most retail tasks well. Advanced techniques handle the edge cases. Know which one you're in before you reach for the toolkit.
