...
But there's a category of task where the fundamentals aren't quite enough. Complex reasoning tasks like deciding which supplier to use this quarter. Tasks where you want output that sounds like a very specific person — a tradie writing to a regular homeowner, not a generic "service professional." Tasks where you want Claude to consistently match a format you already use rather than inventing one of its own. Tasks where giving Claude one example of what you want produces better results than a page of instructions describing it.

This lesson covers four techniques for those situations, {{firstName}}. They are not tricks. They are structured ways of giving Claude more useful information about what you need, or more useful instructions about how to approach the problem. Each one has a specific use case. And at the end of the lesson, we'll talk about when not to use any of them — because over-engineering a simple prompt is its own problem.
...
This technique earns its place on complex, multi-variable tasks. Think: deciding which materials supplier to go with based on price, lead time, and back-order track record. Reviewing a builder's contract clause that has several possible interpretations. Evaluating whether to take on a job based on margin, scope, and the customer. These are tasks where the answer is not obvious, where several factors pull in different directions, and where you want to see the working — not just the output.

Chain-of-thought prompting is overkill for simple tasks. If you want a subject line for an email, you don't need Claude to reason through it. If you want three SMS reminder options for tomorrow's site visits, showing its working adds noise, not value. The signal that you need chain-of-thought is when you find yourself reading a Claude answer and thinking "how did it get there?" — or when the task involves trade-offs and you want to understand what Claude is weighing.

```
I'm deciding between two materials suppliers for my electrical business. Here are the key facts:

Supplier A: 5% cheaper on cable and switchboards, 3-day lead on stocked items, 2 years dealing with us, two missed deliveries last quarter.
Supplier B: 5% dearer on the same lines, next-day on stocked items, 8 years in the local market, no missed deliveries in 12 months.

Think through this step by step — consider cost, reliability, lead time, and what matters most when I'm running multiple residential jobs at once — then give me a clear recommendation with a one-line reason.
```
...
This technique is especially useful when you have a distinctive voice or format that's difficult to specify through instructions alone. Writing your own quote follow-ups in a particular style. Describing scope items in a way that's specific to how you and your customers talk. Writing site-update SMS messages that match a tone your regulars already know. If you spend thirty minutes trying to describe the style in words — and Claude still doesn't quite get it — try showing it two examples instead.
...
Now write a follow-up for this situation: I sent a quote to a customer called James three weeks ago for a full lawn renovation — around $4,500. I haven't heard anything. Same tone and style as the examples above.
```
...
You've probably seen the advice to tell Claude to "act as an expert" before asking a question. This is a weak version of role prompting. An "expert" is too generic to change anything meaningful about the output. Expert in what? From what trade? With what priorities?
...
Weak: "You are a building expert."
Stronger: "You are the office manager for a 12-person residential building business. You're reviewing a head-contractor's variation paperwork at the end of a job where materials costs came in 15% over the original quote."
...
The most effective role prompts for trades tasks tend to be:

- A specific professional reviewing a specific type of document ("You are a commercial solicitor reviewing a subcontractor agreement for a small electrical contractor")
- A specific customer type responding to a specific piece of communication ("You are a 58-year-old homeowner who has just received this end-of-job wrap email from a plumber — read it and tell me what stands out, what confuses you, and what you'd expect to hear next")
- A specific internal voice evaluating a decision ("You are the office manager for a 20-person plumbing business reviewing a proposed change of materials supplier")
...
When you ask Claude to write a job handover summary, it will produce one. But whose handover format? One that reflects whatever conventions are most common in its training data — which may or may not match how {{businessName}} or your industry structures handovers. If you already know what a good output looks like for your context, the fastest way to get it is to provide the skeleton and ask Claude to fill it in.

This is the difference between:

"Write a job handover summary for a completed residential build."
...
Customer sign-off notes: [one sentence on how sign-off went, any conditions]
Warranty period and terms: [one sentence]
Next scheduled contact: [date or "not applicable"]

Here is the job information: [paste details]
```
...
Output scaffolding is particularly useful for: recurring trade documents with a standard format (handovers, site notes, quote summaries), customer-facing templates that have to match how you brand things, and any task where you already know what a good output looks like — you just need the content generated.

### Common mistake → Better approach

**Mistake:** Using multiple advanced techniques at once on a simple task.

A learner asks Claude to review a draft email, but the prompt includes: a detailed role specification, three example emails, a step-by-step reasoning instruction, and a detailed output scaffold. The prompt is 600 words. The task — reviewing a 100-word email — takes fifteen seconds to read. The elaborate prompt doesn't improve the output. It often makes it worse, because Claude spends its effort responding to the complexity of the prompt rather than the simplicity of the task.

**Better approach:** Match the technique to the task. A simple review task needs a clear question and maybe a constraint or two ("flag any sentences that sound like corporate jargon a tradie wouldn't say"). Few-shot prompting earns its place when your voice is hard to describe. Role prompting earns its place when a specific perspective genuinely changes what you need. Chain-of-thought earns its place when the task involves trade-offs or multi-step reasoning. Output scaffolding earns its place when you have an established format that Claude can't infer. Use one at a time. Evaluate whether it changed the output. If it didn't, take it out.
...
If the task is simple, the best prompt is usually simple. "Summarise this builder's email in three bullet points." "Rewrite this paragraph in plain English for a homeowner." "Give me five subject line options for this end-of-job wrap email." None of these benefit from chain-of-thought, few-shot examples, a detailed role, or a structural scaffold. They benefit from being clear about the task, the audience, and the format. That's it.
