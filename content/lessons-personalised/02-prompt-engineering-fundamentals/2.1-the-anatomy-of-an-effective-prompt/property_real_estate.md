...
Prompt engineering is just the practice of writing clear, specific instructions. That's it. Every good agent already does a version of this in their daily work — a brief to a copywriter for a listing, a scope to a stylist for a campaign launch, a request to a new property manager joining the rent roll. The principles are the same. You're telling Claude who you are, what you need, and how you want it delivered.
...
**Role:** Tell Claude who it is for this task, or who you are. This shapes the perspective it takes and the expertise level it brings to the response. "You are an experienced sales agent" produces a different response than leaving Claude to guess. "I'm a property manager handling 180 rentals across the Northern Beaches" gives Claude the context to calibrate its language and assumptions.

**Context:** Give Claude the situation. What's the background? What's already happened? What's the environment this is happening in? Context is where most weak prompts fail — agents jump straight to the task without giving Claude any of the surrounding information it needs to produce something relevant. Without context, Claude fills the gaps with generic assumptions. Those generic assumptions produce generic listing copy, generic owner reports, and generic vendor updates.

**Task:** State clearly what you want Claude to do. Use a verb. "Write," "summarise," "analyse," "draft," "compare," "extract," "review." Vague task descriptions produce vague results. "Help me with this email" is a task description, but it's doing almost no work. "Draft a reply to this vendor email, declining their request to drop the reserve but proposing two alternative campaign tweaks" is a task description that tells Claude exactly what success looks like.

**Format:** Specify the shape of the output you want. Length, structure, tone, what to include, what to exclude. If you want a bullet list, say so. If you want 200 words, say 200 words. If you want formal language, say formal. If you want the output to avoid jargon because it's going to a vendor who isn't familiar with auction terminology, say that explicitly. Format is the component most agents forget, and it's why they get back a 600-word essay when they wanted a three-sentence SMS.
...
```
You are an experienced principal at a small AU sales agency. [Role]

I run a 10-person agency in regional NSW. We've just listed our first commercial property — a small office building on the main street, leased to a long-standing local tenant. The owner has only ever sold residential before and is nervous about how a commercial campaign differs. [Context]

Write a one-page overview of how a commercial sales campaign typically runs in regional AU markets, aimed at this vendor. [Task]

Keep it practical and plain — no real estate jargon, no recommendations that require legal advice. Use short paragraphs. 300 words maximum. [Format]
```

Compare that to: "Write me something about commercial real estate campaigns." Both are prompts. Only one is useful.
...
Think about how you read a vendor brief, an owner email, or a contract instruction. If the first sentence gives you the essential context, everything else lands in the right frame. If the first sentence is ambiguous or buried in background noise, you're guessing at what matters as you read.
...
Now flip the order. Lead with the Task first — "Write a vendor update" — then add context and role at the end. Claude has already begun generating a response in a particular direction. The context you add later may or may not fully override the initial assumptions it made. You'll often get an output that's half generic (from the first read) and half personalised (from the late context). It's patchwork.

The rule is simple: set the stage before you issue the instruction. Role and Context first. Task second. Format last.

There's one exception worth knowing: if the context is very long — for example, you're pasting in a full lease or a 30-page strata report for Claude to analyse — put the task instruction first, then the document. This tells Claude what to look for as it reads. But for most everyday prompts, the standard order holds.
...
"Write a social media post about our new listing."

**Strong prompt:**

```
You are a social media copywriter who specialises in residential real estate.

I'm a sales agent at {{businessName}}, a boutique agency in Wollongong. We've just listed a three-bedroom Federation cottage in West Wollongong — recently renovated, walking distance to the train, and styled for a young family or downsizers. The campaign launches Saturday with a 10am open home.

Write one Instagram caption promoting the open home. Target audience is local buyers aged 30–55. The tone should be warm and direct — not corporate. Use a call to action that points them to the listing link in bio.
...
- Names the actual business, its location, and the property
...
The weak prompt has none of that. Claude has to guess the property type, the audience, the tone, the length, and what "new listing" means. Every guess it makes is a coin flip. You're not directing it — you're asking it to operate blind.
...
"Can you improve this vendor email?" Claude will make changes — but its definition of improvement might mean making it longer, adding more formal language, or restructuring it in a way you didn't want. Without knowing what you consider improvement, Claude fills in the gap.

**Better approach:** Be specific about what you want changed and why.

"This email to a vendor is currently 300 words and too formal in tone. The vendor is a first-time seller, not a seasoned investor. Rewrite it to sound more like a direct conversation — warm but professional. Aim for under 180 words. Don't change the core message about lowering the price guide or the next-step ask in the final paragraph."
...
**Was the output too generic?** Context was missing. You didn't give Claude enough background about your agency, your vendor, or the situation. It made generic assumptions and produced a generic result.
...
**Did Claude solve the wrong problem?** The Task was vague. A task like "help with this" or "write something about the listing" leaves too much open. Claude picked an interpretation — it just wasn't your interpretation.
