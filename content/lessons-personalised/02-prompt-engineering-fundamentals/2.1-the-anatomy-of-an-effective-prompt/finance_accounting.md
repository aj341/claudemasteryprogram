...
Prompt engineering is just the practice of writing clear, specific instructions. That's it. Every accountant or bookkeeper already does a version of this in their daily work — a scope letter to a new client, a brief to a junior staff member, a request to a software vendor. The principles are the same. You're telling Claude who you are, what you need, and how you want it delivered.
...
**Role:** Tell Claude who it is for this task, or who you are. This shapes the perspective it takes and the expertise level it brings to the response. "You are an experienced practice manager at an Australian accounting firm" produces a different response than leaving Claude to guess. "I'm a sole-practitioner BAS agent working with tradies" gives Claude the context to calibrate its language and assumptions.

**Context:** Give Claude the situation. What's the background? What's already happened? What's the client environment this is happening in? Context is where most weak prompts fail — people jump straight to the task without giving Claude any of the surrounding information it needs to produce something relevant. Without context, Claude fills the gaps with generic assumptions (often US-flavoured ones — IRS instead of ATO, 1099 instead of TFN). Those generic assumptions produce generic output.

**Task:** State clearly what you want Claude to do. Use a verb. "Write," "summarise," "analyse," "draft," "compare," "extract," "review." Vague task descriptions produce vague results. "Help me with this client email" is a task description, but it's doing almost no work. "Draft a reply to this email, declining the urgent ad-hoc advice request and proposing a paid 30-minute review instead" is a task description that tells Claude exactly what success looks like.

**Format:** Specify the shape of the output you want. Length, structure, tone, what to include, what to exclude. If you want a bullet list, say so. If you want 200 words, say 200 words. If you want formal language, say formal. If you want the output to avoid jargon because it's going to a small business client who doesn't know the difference between accruals and cash, say that explicitly. Format is the component most people forget, and it's why they get back a 600-word essay when they wanted a three-sentence summary.
...
```
You are an experienced AU accountant who works with hospitality clients. [Role]

I run a 6-person accounting firm in regional Victoria. We've just taken on our
first multi-site cafe group — three trading locations under a single Pty Ltd.
The problem is that the bookkeeper we inherited has been using location-based
class codes inconsistently, which is making BAS prep slow and reconciliation
errors keep showing up. [Context]

Write a one-page plan for how we'll standardise their Xero class structure and
month-end review checklist over the next 30 days. [Task]

Keep it practical and plain — no software jargon, no recommendations that
require a Xero implementation partner. Use short paragraphs. 300 words maximum. [Format]
```

Compare that to: "Write me something about Xero setup for cafes." Both are prompts. Only one is useful.
...
Think about how you read a client engagement letter, a tax-position memo, or a working paper. If the first sentence gives you the essential context, everything else lands in the right frame. If the first sentence is ambiguous or buried in background noise, you're guessing at what matters as you read.
...
Now flip the order. Lead with the Task first — "Write a fee-increase letter" — then add context and role at the end. Claude has already begun generating a response in a particular direction. The context you add later may or may not fully override the initial assumptions it made. You'll often get an output that's half generic (from the first read) and half personalised (from the late context). It's patchwork.

The rule is simple: set the stage before you issue the instruction. Role and Context first. Task second. Format last.

There's one exception worth knowing: if the context is very long — for example, you're pasting in a full ATO ruling for Claude to analyse — put the task instruction first, then the document. This tells Claude what to look for as it reads. But for most everyday prompts, the standard order holds.
...
```
You are a marketing copywriter who works with small AU accounting practices.

I run a sole-practitioner bookkeeping business called Ledgerline in Wollongong.
We've just launched a "BAS rescue" service aimed at small business owners who
have fallen 2+ quarters behind on their BAS lodgements and need help getting
caught up before the ATO escalates. Fixed-fee, six-week catch-up program.

Write one LinkedIn post promoting this new service. Target audience is small
business owners aged 35–55 across NSW. The tone should be calm and reassuring
— not "we'll save you from the ATO" panic copy. Use a call to action pointing
to a free 15-minute eligibility call, with the booking link in my profile.
...
"This email is currently 300 words and too formal in tone. The client is a sole-trader plumber, not a corporate finance director. Rewrite it to sound more like a direct conversation — warm but still clear that the BAS deadline is firm. Aim for under 180 words. Don't change the core message or the ask in the final paragraph about the missing receipts."
...
**Was the output too generic?** Context was missing. You didn't give Claude enough background about your practice, your client, or the situation. It made generic assumptions and produced a generic result — possibly with US tax terminology you'd then have to scrub out.
...
**Did Claude solve the wrong problem?** The Task was vague. A task like "help with this" or "write something about FY end" leaves too much open. Claude picked an interpretation — it just wasn't your interpretation.

**Did the response sound like it came from the wrong person?** Role was missing. Claude had no expertise signal to anchor to, so it defaulted to a neutral, middle-of-the-road voice that doesn't serve anyone well — and tends to drift toward Big-4 audit register.
