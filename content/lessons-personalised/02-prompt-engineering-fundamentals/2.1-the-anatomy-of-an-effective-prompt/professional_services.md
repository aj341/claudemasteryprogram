...
Prompt engineering is just the practice of writing clear, specific instructions. That's it. Every consultant or advisor already does a version of this in their daily work — a scope of work for a new engagement, a brief to a junior associate, a request to a client about missing information. The principles are the same. You're telling Claude who you are, what you need, and how you want it delivered.
...
**Role:** Tell Claude who it is for this task, or who you are. This shapes the perspective it takes and the expertise level it brings to the response. "You are an experienced HR consultant working with AU small businesses" produces a different response than leaving Claude to guess. "I'm a sole-operator management consultant working with not-for-profit boards" gives Claude the context to calibrate its language and assumptions.

**Context:** Give Claude the situation. What's the background? What's already happened? What's the engagement environment this is happening in? Context is where most weak prompts fail — people jump straight to the task without giving Claude any of the surrounding information it needs to produce something relevant. Without context, Claude fills the gaps with generic assumptions (often US-flavoured ones — HR Business Partner instead of HR consultant, MBA-school frameworks instead of practical advice). Those generic assumptions produce generic output.

**Task:** State clearly what you want Claude to do. Use a verb. "Write," "summarise," "analyse," "draft," "compare," "extract," "review." Vague task descriptions produce vague results. "Help me with this client email" is a task description, but it's doing almost no work. "Draft a reply to this email, declining the unscoped follow-up request and proposing a paid scoping session instead" is a task description that tells Claude exactly what success looks like.

**Format:** Specify the shape of the output you want. Length, structure, tone, what to include, what to exclude. If you want a bullet list, say so. If you want 200 words, say 200 words. If you want formal language, say formal. If you want the output to avoid jargon because it's going to a small business owner who doesn't speak consultant, say that explicitly. Format is the component most people forget, and it's why they get back a 600-word essay when they wanted a three-sentence summary.
...
```
You are an experienced AU management consultant who works with mid-sized
not-for-profits. [Role]

I run a 5-person consulting practice in Brisbane. We've just been engaged
by a regional disability services provider — three sites, 80 staff — to
review their case-management workflow. The problem is that their general
manager has been burned by previous consultants who delivered a 60-page
report that nobody read. [Context]

Write a one-page outline of how we'll structure this engagement so the
deliverable is something the GM and team leaders will actually use. [Task]

Keep it practical and plain — no consulting jargon, no recommendations
that require a separate change-management workstream. Use short
paragraphs. 300 words maximum. [Format]
```

Compare that to: "Write me something about consulting engagement structure." Both are prompts. Only one is useful.
...
Think about how you read an engagement letter, a discovery brief, or a client status update. If the first sentence gives you the essential context, everything else lands in the right frame. If the first sentence is ambiguous or buried in background noise, you're guessing at what matters as you read.
...
Now flip the order. Lead with the Task first — "Write a scope-change letter" — then add context and role at the end. Claude has already begun generating a response in a particular direction. The context you add later may or may not fully override the initial assumptions it made. You'll often get an output that's half generic (from the first read) and half personalised (from the late context). It's patchwork.

The rule is simple: set the stage before you issue the instruction. Role and Context first. Task second. Format last.

There's one exception worth knowing: if the context is very long — for example, you're pasting in a full client policy document for Claude to analyse — put the task instruction first, then the document. This tells Claude what to look for as it reads. But for most everyday prompts, the standard order holds.
...
```
You are a marketing copywriter who works with small AU professional services firms.

I run a sole-operator HR consultancy called Northline Advisory in Perth.
We've just launched a "Manager Reset" service aimed at small business
owners whose first-time team leads are struggling with difficult
conversations. Fixed-fee, four-session coaching package over six weeks.

Write one LinkedIn post promoting this new service. Target audience is
owners of 10-30 person businesses across WA. The tone should be calm and
practical — not "fix your toxic workplace" panic copy. Use a call to
action pointing to a free 20-minute fit call, with the booking link in
my profile.

200 words maximum. No hashtags in the post itself.
```
...
- Names the actual practice, its location, and its service
...
"This email is currently 300 words and too formal in tone. The client is the owner of a 12-person IT support firm in Geelong, not a corporate procurement lead. Rewrite it to sound more like a direct conversation — warm but still clear that the engagement scope is set. Aim for under 180 words. Don't change the core message or the ask in the final paragraph about the missing access details."
...
Here's a quick diagnostic you can run on any unsatisfactory output at {{businessName}}:

**Was the output too generic?** Context was missing. You didn't give Claude enough background about your practice, your client, or the engagement. It made generic assumptions and produced a generic result — possibly with US terminology you'd then have to scrub out.
...
**Did Claude solve the wrong problem?** The Task was vague. A task like "help with this" or "write something about the engagement" leaves too much open. Claude picked an interpretation — it just wasn't your interpretation.

**Did the response sound like it came from the wrong person?** Role was missing. Claude had no expertise signal to anchor to, so it defaulted to a neutral, middle-of-the-road voice that doesn't serve anyone well — and tends to drift toward Big-4 consulting register.
