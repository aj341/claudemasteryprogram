...
Prompt engineering is just the practice of writing clear, specific instructions. That's it. Every tradie already does a version of this every week — a brief to an apprentice before they head out, a scope handed to a sub-contractor, a clear note to the office about how to invoice a job. The principles are the same. You're telling Claude who you are, what you need, and how you want it delivered.

The reason it has a technical-sounding name is that researchers first studied it in the context of AI systems — they were literally engineering the prompts to get better model outputs. But the skill itself is not technical. It's a communication skill applied to a new medium.

The good news: you already have most of what you need. Module 1 gave you the foundational mental model — Claude takes you at face value, reads only what you write, and has no context beyond what you give it. Prompt engineering is just the systematic application of that mental model. It gives you a repeatable structure so you're not starting from scratch every time you open Claude in the ute.
...
**Role:** Tell Claude who it is for this task, or who you are. This shapes the perspective it takes and the expertise level it brings to the response. "You are an experienced site supervisor" produces a different response than leaving Claude to guess. "I'm a sole-trader sparkie" gives Claude the context to calibrate its language and assumptions.

**Context:** Give Claude the situation. What's the background? What's already happened on the job? What's the environment this is happening in? Context is where most weak prompts fail — people jump straight to the task without giving Claude any of the surrounding information it needs to produce something relevant. Without context, Claude fills the gaps with generic assumptions. Those generic assumptions produce generic output that no homeowner or builder will read twice.

**Task:** State clearly what you want Claude to do. Use a verb. "Write," "summarise," "draft," "compare," "extract," "review." Vague task descriptions produce vague results. "Help me with this email" is a task description, but it's doing almost no work. "Draft a reply to this builder, declining the Saturday work but offering two weekday slots next week" is a task description that tells Claude exactly what success looks like.

**Format:** Specify the shape of the output you want. Length, structure, tone, what to include, what to exclude. If you want a bullet list, say so. If you want 200 words, say 200 words. If you want plain language, say plain. If you want the output to avoid trade jargon because it's going to a homeowner who isn't familiar with the work, say that explicitly. Format is the component most people forget, and it's why they get back a 600-word essay when they wanted a three-sentence SMS.
...
```
You are an experienced trades operations manager. [Role]

I run a 12-person residential plumbing business in Brisbane. We've just
picked up our first ongoing maintenance contract — weekly call-outs across
80 rental properties for a property manager. The problem is that our
current job-tracking is done on paper and it's already creating missed
visits and duplicate invoices. [Context]

Write a one-page overview of a simple digital job-tracking setup we could
roll out in the next 30 days. [Task]

Keep it practical and plain — no software jargon, no recommendations that
need a developer to set up. Use short paragraphs. 300 words maximum. [Format]
```

Compare that to: "Write me something about job-tracking systems for plumbers." Both are prompts. Only one is useful.
...
Think about how you read a job brief, a builder's email, or a scope of works. If the first sentence gives you the essential context, everything else lands in the right frame. If the first sentence is ambiguous or buried in background noise, you're guessing at what matters as you read.
...
Now flip the order. Lead with the Task first — "Write a quote rejection letter" — then add context and role at the end. Claude has already begun generating a response in a particular direction. The context you add later may or may not fully override the initial assumptions it made. You'll often get an output that's half generic (from the first read) and half personalised (from the late context). It's patchwork.

The rule is simple: set the stage before you issue the instruction. Role and Context first. Task second. Format last.

There's one exception worth knowing: if the context is very long — for example, you're pasting in a full head-contractor scope or a long builder's email thread for Claude to work through — put the task instruction first, then the document. This tells Claude what to look for as it reads. But for most everyday prompts, the standard order holds.
...
```
You are a social media copywriter who specialises in trades businesses.

I own a residential painting company called Headland Painting based in
Wollongong. We've just launched a pre-sale touch-up service for homeowners
getting their place ready to list — patching, ceiling spot-fixes, and door
re-coats that knock years off a tired interior. The kind of work most
painters won't take on because it's small, but it makes a real difference
at inspection time.

Write one Instagram caption promoting this new service. Target audience is
homeowners aged 35–60 about to list with a local agent. The tone should be
warm and direct — not corporate. Use a call to action that sends them to
our link in bio.
...
The output from the strong prompt will be usable. The output from the weak prompt will need so much editing that you'd have been faster writing it yourself between site visits.
...
"This email is currently 300 words and too formal in tone. The customer is a homeowner I've worked with twice before, not a builder or strata manager. Rewrite it to sound more like a direct conversation — warm but professional. Aim for under 180 words. Don't change the core message or the request for sign-off in the final paragraph."
...
**Was the output too generic?** Context was missing. You didn't give Claude enough background about {{businessName}}, your customer, or the situation on site. It made generic assumptions and produced a generic result.

**Was the output the wrong shape?** Format was missing. You didn't specify length, structure, or tone. Claude defaulted to its own best guess, which may not match what you needed for a quick SMS to a regular customer or a formal variation notice to a builder.
...
**Did the response sound like it came from the wrong person?** Role was missing. Claude had no expertise signal to anchor to, so it defaulted to a neutral, middle-of-the-road voice that doesn't sound like a tradie talking to a real customer.

Run this diagnostic before you decide Claude can't do the thing, {{firstName}}. Nine times out of ten, adding or sharpening the missing component produces a dramatically different output from the same request.
