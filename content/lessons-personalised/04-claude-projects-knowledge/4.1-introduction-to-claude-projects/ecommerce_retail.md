...
Here's something that trips up a lot of Claude users once they've been using it for a while. You've put in real effort to get Claude working well for {{businessName}}. You've written a good prompt, given it your context, taught it your tone. The response is exactly what you wanted. You close the tab.

Next day, you open Claude and start a new conversation. Claude has no idea who you are. No memory of your store name, your customers, your brand voice, the tone you spent three prompts calibrating. Every new conversation starts from scratch. You're back to the beginning.
...
The practical effect: every time you start a fresh conversation, you either paste in your business context again, or you accept a more generic output. Most retailers do one of two things — they paste the same context block over and over, or they don't bother and get frustrated when Claude doesn't know what it should already know. Neither is a good solution.
...
Here's an example of custom instructions for a retail brand context Project:

```
You are a brand assistant for Saltbush & Co, a Melbourne-based small homewares brand with 3 full-time staff and 2 part-time. We sell hand-thrown ceramics, linen tea towels, and beeswax candles. Our customers are gift-buyers and self-treating women aged 35–55, average basket $95. We sell direct on Shopify and through 15 wholesale stockists nationwide.

My name is Sarah. I'm the founder and handle all marketing, customer-service comms, and stockist relationships.

Communication style: Direct, warm, never corporate or shouty. We're a small brand — write like one. Australian English. No fake urgency.

What I'll ask you about: customer-service replies, product descriptions, stockist comms, restock-alert emails, social captions, and the occasional planning question.

What you should not do: Assume we have a marketing team, large ad budget, or enterprise tools. We use Klaviyo, Shopify, Lightspeed, and Instagram. Keep recommendations practical and proportionate to our size. Never invent stock-on-hand numbers, prices, or dispatch dates.
```

That's the shape of a good custom instruction set. Who Claude is for, who you are, your store context, your communication style, what you'll use it for, what to avoid.

**Knowledge documents** are files you upload into the Project. These can be SKU lists, brand guidelines, FAQs, pricing schedules, example emails, wholesale line sheets, or anything else Claude should be able to reference. When you ask Claude a question inside the Project, it can draw on these documents. You don't need to paste them into every conversation. They're available.
...
**Conversation history** is the ongoing record of everything you've discussed inside the Project. Unlike regular conversations that disappear when you close the tab, conversations inside a Project accumulate. Claude can see what you've discussed before within the same Project. If you briefed it on a stockist situation last week, it still has that context this week — inside the same Project.

### The three types of Projects worth building

Most retailers get maximum value from three categories of Projects. You don't need to build all three on day one. But understanding the categories helps you decide where to start.

**The brand context Project.** This is your "always on" assistant. It holds your complete brand brief — store name and overview, your role, your customers, your communication style, your core range, and the tools you use. Any task that doesn't belong to a specific stockist or recurring workflow goes here. It's your general-purpose Claude, already configured for your store. Most retailers build this one first, and it immediately makes every interaction faster.

**The stockist or customer-segment Project.** One Project per major wholesale stockist, or per repeating customer segment (retail vs. wholesale, AU vs. NZ). The custom instructions hold that segment's context — their preferences, their reorder patterns, the way you communicate with them. Knowledge documents hold any relevant files: their line sheet, past order history, notes from buyer meetings. Over the course of a wholesale relationship, the conversation history builds up a record of what you've worked on. When you need to pick up a task for that stockist, you open their Project. Everything is there.

**The task Project.** For recurring work that follows a consistent pattern — the fortnightly customer email, the weekly social caption batch, the monthly returns summary. The custom instructions define the format, the tone, and the constraints of that specific task. Knowledge documents hold any fixed content: the content pillars, the email template, the style rules. Each time you run the task, you're starting from a pre-configured environment. The Project doesn't just remember your store — it remembers the specific requirements of this particular output.

You can build more than three. Some retailers run a separate Project for each major sales channel, or keep a Project specifically for internal documents like staff training notes or floor scripts. The category structure is a starting point, not a limit.
...
**Projects does not search the web.** When Claude works in a Project, it draws on its training data plus whatever you've put in the Project. It doesn't go out and look up current information. If you need Claude to work with current data — today's freight rates, live stock numbers, updated competitor pricing — you need to paste that information into the conversation yourself. The Project holds what you've given it. It doesn't update itself.

**Projects does not update itself.** If your pricing changes, your range changes, or your wholesale terms shift, your Project custom instructions and documents stay the same until you change them. Claude will continue to reference outdated information if you don't update it. Set a reminder to review your Project documents whenever something meaningful changes in your store.

**Projects does not replace a proper knowledge base for large volumes of information.** The knowledge document system works well for a brand of your size with a focused set of documents. If you're trying to load in hundreds of SKUs across dozens of files and expect Claude to search and retrieve perfectly, you'll hit limits. Projects is not a search system. It reads documents you've uploaded, but it works best when those documents are concise, relevant, and well-organised. If your information needs are genuinely large and complex — your full Shopify catalogue, your full Lightspeed transaction history — those belong in the systems built for them, not in a Project.

**Projects does not make Claude smarter.** This one surprises retailers. Claude's underlying capabilities are the same inside a Project as outside one. What changes is the context Claude has available. A better-configured Project gets you better outputs because Claude has better information — not because the model has changed. The quality ceiling is still Claude's capability. The floor is how well you've configured the Project.
...
**Better approach:** Curate the Project. Add documents that Claude will actually need to reference when doing the specific work you do in that Project. A wholesale-stockist Project doesn't need your full brand strategy deck — it needs the line sheet and the agreed wholesale terms. A task Project for your fortnightly customer email doesn't need your full SKU catalogue — it needs the content pillars, the customer-segment definition, and a few strong example issues. Relevance over volume. Claude reads documents, not just stores them. The more focused the documents are, the more usable the outputs.
