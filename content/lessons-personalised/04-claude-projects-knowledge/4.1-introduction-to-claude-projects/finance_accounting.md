...
Here's something that trips up a lot of accountants once they've been using Claude for a while. You've put in real effort to get Claude working well for your practice. You've written a good prompt, given it your context, taught it your tone. The response is exactly what you wanted for that BAS reminder. You close the tab.

Next day, you open Claude and start a new conversation. Claude has no idea who you are. No memory of {{businessName}}, your client mix, your preferences, the tone you spent three prompts calibrating. Every new conversation starts from scratch. You're back to the beginning.
...
The practical effect: every time you start a fresh conversation, you either paste in your practice context again, or you accept a more generic output. Most accountants do one of two things — they paste the same context block over and over, or they don't bother and get frustrated when Claude doesn't know what it should already know. Neither is a good solution.
...
Claude Projects is a workspace inside Claude where you can store custom instructions, upload knowledge documents, and maintain an ongoing conversation history — all in one place. When you work inside a Project, Claude reads those instructions and documents at the start of every conversation. You don't have to paste anything. You don't have to explain your practice again. You open the Project, and Claude is already briefed.

Think of it as a standing brief. In a regular Claude conversation, you're briefing Claude fresh each time — like emailing a contractor with all the background every time you need a piece of work done. A Project is like giving that same contractor a proper onboarding document they can refer to whenever you work together. The brief is always there. The context is persistent.
...
**Custom instructions** are the most important. This is where you tell Claude: who it is for this Project, what practice or situation it's working in, what it should and shouldn't do, how it should communicate, and what assumptions it can make. Custom instructions live at the Project level. Every conversation you start inside the Project reads them. They're your standing brief.

Here's an example of custom instructions for a practice context Project:

```
You are a practice assistant for Riverbend Accounting, a 6-person regional
Victorian accounting firm. Two of our six staff are registered tax agents.
Our clients are small business owners (sole traders to 15-staff Pty Ltds)
across hospitality, trades, and primary production. We do FY accounts,
tax returns, BAS, and quarterly advisory meetings.

My name is Sarah. I'm the principal and handle all client-facing work,
fee proposals, and partner-level reviews.

Communication style: Warm, plain-language, never Big-4 audit. Australian
English always (lodgement, organisation, behaviour, programme).

What I'll ask you about: BAS reminders, fee proposals, scope letters,
end-of-FY client communications, file-note summaries, internal team briefs.

What you should not do: Use US tax terminology (no IRS, 1099, 401(k));
don't write anything that reads as finished tax advice — frame outputs
as drafts for my review; don't invent ATO rates, section numbers, or
dates. We use Xero and FYI for practice management.
```

That's the shape of a good custom instruction set. Who Claude is for, who you are, your practice context, your communication style, what you'll use it for, what to avoid.

**Knowledge documents** are files you upload into the Project. These can be a master engagement-letter template, a fee schedule, a one-page brand voice guide, FAQs you've drafted for common client questions, example BAS reminder emails, client briefs, or anything else Claude should be able to reference. When you ask Claude a question inside the Project, it can draw on these documents. You don't need to paste them into every conversation. They're available.

Keep documents focused and up to date. A focused 5-page voice and tone guide works better than a single massive document containing everything — Claude can reference the relevant parts more reliably when documents are separated by topic.

**Conversation history** is the ongoing record of everything you've discussed inside the Project. Unlike regular conversations that disappear when you close the tab, conversations inside a Project accumulate. Claude can see what you've discussed before within the same Project. If you briefed it on a client's restructure last quarter, it still has that context this quarter — inside the same Project.

### The three types of Projects worth building

Most accountants get maximum value from three categories of Projects. You don't need to build all three on day one. But understanding the categories helps you decide where to start.

**The practice context Project.** This is your "always on" assistant. It holds your complete practice brief — firm name and overview, your role, your client mix, your communication style, your core services, and the tools you use. Any task that doesn't belong to a specific client or recurring workflow goes here. It's your general-purpose Claude, already configured for your practice. Most accountants build this one first, and it immediately makes every interaction faster.

**The client Project.** One Project per major active client. The custom instructions hold that client's background — their entity structure, their industry, their preferences, their history with the firm. Knowledge documents hold any relevant files: their FY engagement letter, last year's working papers (anonymised if needed), notes from quarterly meetings. Over the course of an engagement, the conversation history builds up a record of what you've worked on. When you need to pick up a client task, you open their Project. Everything is there.

**The task Project.** For recurring work that follows a consistent pattern — a quarterly client newsletter, a weekly fee follow-up batch, a monthly social media post. The custom instructions define the format, the tone, and the constraints of that specific task. Knowledge documents hold any fixed content: the content pillars, the template, the voice rules. Each time you run the task, you're starting from a pre-configured environment. The Project doesn't just remember your practice — it remembers the specific requirements of this particular output.

You can build more than three. Some firms run a separate Project for each service line (compliance, advisory, SMSF), or keep a Project specifically for internal documents like staff onboarding or partner-board updates. The category structure is a starting point, not a limit.
...
**Projects does not search the web.** When Claude works in a Project, it draws on its training data plus whatever you've put in the Project. It doesn't go out and look up current ATO determinations or today's super contribution caps. If you need Claude to work with current data — today's ATO bulletin, live rates, updated industry stats — you need to paste that information into the conversation yourself. The Project holds what you've given it. It doesn't update itself.

**Projects does not update itself.** If your fee schedule changes, your service list changes, or your practice context shifts (a new partner, a rebrand), your Project custom instructions and documents stay the same until you change them. Claude will continue to reference outdated information if you don't update it. Set a reminder to review your Project documents whenever something meaningful changes — at minimum, at the start of each FY.

**Projects does not replace a proper knowledge base for large volumes of information.** The knowledge document system works well for a practice of your size with a focused set of documents. If you're trying to load in hundreds of pages of ATO rulings across dozens of files and expect Claude to search and retrieve perfectly, you'll hit limits. Projects is not a search system. It reads documents you've uploaded, but it works best when those documents are concise, relevant, and well-organised. If your information needs are genuinely large and complex, a dedicated tax research tool is a better fit.
...
**Better approach:** Curate the Project. Add documents that Claude will actually need to reference when doing the specific work you do in that Project. A client Project doesn't need your firm's full marketing collateral — it needs the client's engagement letter and any agreed deliverables. A task Project for your quarterly client newsletter doesn't need your full practice business plan — it needs the content pillars, audience definition, and a few strong example issues. Relevance over volume. Claude reads documents, not just stores them. The more focused the documents are, the more usable the outputs.
...
That's a meaningful change in daily practice workflow. It makes Claude faster to use. It makes outputs more consistent across the firm. And it means the effort you put into setting up the Project pays off across every future conversation you have inside it.
