...
When agency principals first discover that Claude Projects can hold documents, the instinct is to upload everything. The agency profile, the fee schedule, last year's listing presentations, the team headshots, the supplier contract from 2021, the spreadsheet of vendor prospects you sort of keep updated. Everything gets loaded in, and then the Project is treated like a searchable filing cabinet.
...
The right question is not: what does {{businessName}} have? The right question is: what does Claude need to do this specific task well, that it doesn't already know?

That reframe changes which documents you choose. It also changes how you write or reformat them. A Project's knowledge is not a digital archive. It's a briefing pack — the minimum set of well-organised documents that gives Claude the context it needs to do real work without you filling in the gaps every time you draft a vendor update or owner report.
...
**Agency context.** This is who you are, how the agency operates, and what makes it distinct. It answers the background questions Claude would otherwise ask or guess at. Your agency name, suburb or region, what you list and manage, who your typical vendors and landlords are, how you communicate, what you don't offer. The tone document from Lesson 1.5 belongs here. So does a one-page "about us" written for Claude — not for vendors, but as a plain-English briefing on the agency that a new sales rep or BDM would need on their first day.

Agency context documents don't change often. Once they're written and in the Project, they're mostly stable. You'll update them when something material changes — a new service line like buyer's agency work, a different price bracket, a rebrand — but you're not touching them weekly.

**Process knowledge.** This covers how you do things. Your listing-presentation process. How you handle a vendor who wants to pull the property off the market. The steps you follow when onboarding a new managed property. How you structure a CMA. What happens after a buyer's offer is accepted. These documents exist so Claude understands your workflow and can help you at each stage of it — not just generate generic output, but output that fits how the agency actually operates.
...
**Listing and engagement knowledge.** This is the context specific to a current campaign or active engagement — a listing brief, the agency agreement summary, notes from the listing presentation, the vendor's circumstances, the marketing schedule, the agreed inclusions for a property under management. This category is different from the other two: it changes with every new listing or new managed property and isn't useful once the campaign settles or the lease ends.

Most agencies have one Project per significant campaign or per active engagement, and the listing-specific knowledge documents in that Project are specific to that work. When the campaign exchanges and settles, the Project is archived. The agency context and process documents stay in your main Projects; the campaign-specific material goes with that campaign's Project.

Keep these three categories distinct. If you find yourself wondering where a document belongs, ask: does this describe the agency permanently, does it describe how we do something, or does it describe a specific current campaign or property? That question usually resolves it.
...
**Plain text** is the most reliable. A `.txt` or `.md` file with headings, numbered steps, and short sections is what Claude reads with the highest accuracy. When you write a document specifically for Claude — a process guide, an agency context brief, a set of instructions — write it in plain text.

**Structured PDF** (text-based, not scanned) works well when the document is logically formatted. A listing-presentation template or a fee schedule in a clean PDF is fine. Claude can read it accurately if the text is selectable — i.e., not a scan or an image of a document.

**Image-based PDFs and photos of documents** are the least reliable. Claude can read them using vision, but accuracy drops when the layout is complex, when there's a lot of small text, or when the image quality is low. If an important document only exists as a scanned condition report or a phone photo, transcribe the relevant parts into plain text before adding it to a Project.
...
**Mistake:** Uploading an existing agency document directly to a Project without reformatting it.

A vendor-facing listing presentation written to win the listing is not a useful knowledge document for Claude. It's written in marketing language, it omits operational detail, it prioritises persuasion over accuracy, and it buries the information Claude actually needs (what the agency does, for whom, at what fee, with what inclusions) inside copy designed for a different purpose.

**Better approach:** Write a separate Claude-facing version of that document. Keep it short, plain, and specific. Write it the way you'd brief a smart new sales rep on their first morning — factual, direct, structured. "We are a four-person agency in Newcastle. We list primarily in Merewether, The Hill, and Bar Beach, between $900K and $2.2M. Our rent roll is 110 properties, mostly residential houses. We charge 2.2% inc GST on sales and 7.7% inc GST on management. We do not list commercial property — we refer those to Pacific Commercial. Our standard private treaty campaign runs 28 days." That briefing is more useful to Claude than a glossy six-page presentation.

### The update problem

Knowledge documents go stale. This is the most common reason a Project that worked well three months ago is producing weaker output now. The agency changed. The fee structure changed. A new agent joined. The auction calendar shifted. The knowledge documents didn't change with them, and now Claude is working from an outdated picture of the agency.

The update problem doesn't need a complicated solution. You don't need a review calendar or a quarterly audit. You need one habit: when something about the agency changes that a knowledge document covers, update the document that day.

The trigger is the change, not the calendar. You add a buyer's-agent service — update your agency context document the same afternoon. You change your CMA process — update the process document before the next time you run it for a vendor. You list a new property — create or update its listing knowledge document before you start using the Project for that campaign.

The sign that a document has gone stale isn't always obvious. Sometimes it shows up as Claude getting details slightly wrong — mentioning a fee you no longer charge, describing an inspection step that's been removed, or failing to account for a tenancy change that now matters under the latest residential tenancies amendments. When you see that, the fix is simple: find the document that contains the outdated information, correct it, and move on.

There's also a harder version of the update problem: knowledge that never existed as a document. Claude is making assumptions about how {{firstName}} handles something — say, how the agency responds when a tenant disputes the bond — because nobody ever wrote it down. The output is off, but not because the document is wrong. There is no document. In that case, the fix is to write one. Five minutes of documentation now prevents the same correction ten times in the future.
...
**Sensitive credentials.** Passwords, API keys, trust account logins, PINs, REI Forms Live or VaultRE credentials. Claude Projects are not a password manager. Even if you trust the platform, storing credentials in a knowledge document creates a single location that could expose them if your account were ever accessed by someone else.

**Financial account details.** BSB and account numbers, full trust-account records, vendor payout details, tenant bond reference numbers tied to identifiable individuals. A reference to "we use a separate trust account for sales deposits" is fine. Actual account numbers are not.

**Live operational data that changes frequently.** Today's rent roll arrears report, the current open-home schedule for the week, this morning's pending offers list, the live status of pending exchanges. This kind of data goes stale within hours or days. Putting it in a knowledge document means Claude will use the stale version. Operational data belongs in the tools you use to manage it (your trust accounting platform, your CRM, your sales pipeline) — not in a static knowledge document.

The test is simple: if this information changed tomorrow and Claude used the old version, would that cause a problem with a vendor, landlord, tenant, or auditor? For credentials and financials, the answer is always yes. For operational data, it's almost always yes. Keep those out.
