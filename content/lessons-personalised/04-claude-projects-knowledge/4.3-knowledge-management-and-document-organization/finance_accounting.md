...
When accountants first discover that Claude Projects can hold documents, the instinct is to upload everything. Firm brochure, fee schedule, last year's engagement letters, photos of the team, an old supplier contract from 2021, the spreadsheet you sort of use to track lodgements. Everything gets loaded in, and then the Project is treated like a searchable filing cabinet.

This is the wrong mental model, {{firstName}}, and it creates real problems. Claude isn't a search engine for your files. It works best when the knowledge in a Project is deliberately chosen — not comprehensive, but relevant. Every document you add increases the chance that Claude draws on the wrong thing, or gets confused by conflicting details, or buries the useful information under material that doesn't apply.
...
**Business context.** This is who you are, how you operate, and what makes your practice distinct. It answers the background questions Claude would otherwise ask or guess at. Your firm name, location, what you do, who your clients are, how you communicate, what you don't offer. The tone document from Lesson 1.5 belongs here. So does a one-page "about us" written for Claude — not for prospects, but as a plain-English briefing on {{businessName}} that a new junior bookkeeper would need on day one.

Business context documents don't change often. Once they're written and in the Project, they're mostly stable. You'll update them when something material changes — a new service line (advisory, SMSF), a different client type, a rebrand — but you're not touching them weekly.

**Process knowledge.** This covers how you do things. Your quoting process. How you handle a client complaint about a fee. The steps you follow when onboarding a new compliance client. How you structure a fee proposal. What happens after someone signs an engagement letter. These documents exist so Claude understands your workflow and can help you at each stage of it — not just generate generic output, but output that fits how your practice actually operates.
...
**Client and project knowledge.** This is the context specific to a current engagement — a client brief, a fee proposal, notes from a discovery call, a client's group structure diagram, the agreed deliverables for an FY engagement you're delivering right now. This category is different from the other two: it changes with every new engagement and isn't useful once the engagement ends.

Most accountants have one Project per major client or per active engagement, and the client/project knowledge documents in that Project are specific to that work. When the engagement ends, the Project is archived. The business context and process documents stay in your main Projects; the client-specific material goes with the client's Project.

Keep these three categories distinct. If you find yourself wondering where a document belongs, ask: does this describe my practice permanently, does it describe how I do something, or does it describe a specific current piece of work? That question usually resolves it.
...
**Plain text** is the most reliable. A `.txt` or `.md` file with headings, numbered steps, and short sections is what Claude reads with the highest accuracy. When you write a document specifically for Claude — a process guide, a practice context brief, a set of instructions — write it in plain text.

**Structured PDF** (text-based, not scanned) works well when the document is logically formatted. A fee proposal template or a fee schedule in a clean PDF is fine. Claude can read it accurately if the text is selectable — i.e., not a scan or an image of a document.

**Image-based PDFs and photos of documents** are the least reliable. Claude can read them using vision, but accuracy drops when the layout is complex, when there's a lot of small text (think a scanned ATO bulletin), or when the image quality is low. If an important document only exists as a scanned PDF or a photo, transcribe the relevant parts into plain text before adding it to a Project.
...
**Mistake:** Uploading an existing practice document directly to a Project without reformatting it.

A client-facing brochure written to impress prospects is not a useful knowledge document for Claude. It's written in marketing language, it omits operational detail, it prioritises persuasion over accuracy, and it buries the information Claude actually needs (what you do, for whom, at what fee, with what constraints) inside copy designed for a different purpose.

**Better approach:** Write a separate Claude-facing version of that document. Keep it short, plain, and specific. Write it the way you'd brief a smart new junior bookkeeper on their first day — factual, direct, structured. "We are a four-person bookkeeping practice in Hobart. We work exclusively with trade businesses. Our clients are typically 1–10 employees. We use Xero and FYI for practice management. We do not do company tax returns — we refer those to our partner firm, Hargreaves Tax. Our standard turnaround for monthly reconciliation is five business days." That briefing is more useful to Claude than a six-page brochure.

### The update problem

Knowledge documents go stale. This is the most common reason a Project that worked well three months ago is producing weaker output now. The practice changed. The fee structure changed. A new service was added (e.g., quarterly advisory). A client engagement ended. The knowledge documents didn't change with them, and now Claude is working from an outdated picture of {{businessName}}.

The update problem doesn't need a complicated solution. You don't need a review calendar or a quarterly audit. You need one habit: when something about your practice changes that a knowledge document covers, update the document that day.

The trigger is the change, not the calendar. You add a new service — update your practice context document the same afternoon. You change your fee proposal process — update the process document before the next time you run the proposal workflow. You onboard a new compliance client — create or update their client knowledge document before you start using the Project for their work.

The sign that a document has gone stale isn't always obvious. Sometimes it shows up as Claude getting details slightly wrong — mentioning a fee tier you no longer offer, describing a step that you've since removed from your BAS-prep workflow, or failing to account for a constraint that now matters (the new partner reviews all advisory fee proposals over $5k, say). When you see that, the fix is simple: find the document that contains the outdated information, correct it, and move on.
...
**Sensitive credentials.** Passwords, API keys, ATO portal logins, MyGov credentials, software vendor logins. Claude Projects are not a password manager. Even if you trust the platform, storing credentials in a knowledge document creates a single location that could expose them if your account were ever accessed by someone else.

**Client tax and financial identifiers.** TFNs, ABNs of specific clients, BSB and account numbers, credit card details, full banking records. A reference to the bank a client uses is fine. Their actual account number, TFN, or ABN is not.

**Live operational data that changes frequently.** Your current lodgement queue, today's staff roster, a client's bank balance, the status of open invoices. This kind of data goes stale within hours or days. Putting it in a knowledge document means Claude will use the stale version. Operational data belongs in the tools you use to manage it (your practice management software, Xero, the ATO portal) — not in a static knowledge document.

The test is simple: if this information changed tomorrow and Claude used the old version, would that cause a problem? For credentials and client identifiers, the answer is always yes. For operational data, it's almost always yes. Keep those out.
