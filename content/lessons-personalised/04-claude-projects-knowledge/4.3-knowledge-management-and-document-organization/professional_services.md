...
When people first discover that Claude Projects can hold documents, the instinct is to upload everything. Capability deck, fee schedule, last year's proposals, photos of the team, the supplier contract from 2021, the spreadsheet you sort of use to track engagements. Everything gets loaded in, and then the Project is treated like a searchable filing cabinet.
...
**Practice context.** This is who you are, how you operate, and what makes your firm distinct. It answers the background questions Claude would otherwise ask or guess at. Your firm name, location, what you do, who your clients are, how you communicate, what you don't offer. The tone document from Lesson 1.5 belongs here. So does a one-page "about the firm" written for Claude — not for prospective clients, but as a plain-English briefing on your practice that a new associate would need on day one.

Practice context documents don't change often. Once they're written and in the Project, they're mostly stable. You'll update them when something material changes — a new service line, a different client type, a rebrand — but you're not touching them weekly.

**Process knowledge.** This covers how you do things. Your scoping process. How you handle a difficult client conversation. The steps you follow when onboarding a new engagement. How you structure a fee proposal. What happens after a discovery workshop. These documents exist so Claude understands your workflow and can help you at each stage of it — not just generate generic output, but output that fits how your practice actually operates.
...
**Client and engagement knowledge.** This is the context specific to a current engagement — a client brief, a scope of work, notes from a discovery call, a client's brand guide, the agreed deliverables for a project you're delivering right now. This category is different from the other two: it changes with every new engagement and isn't useful once the work is done.

Most consultants run one Project per client or per active engagement, and the client/engagement knowledge documents in that Project are specific to that work. When the engagement ends, the Project is archived. The practice context and process documents stay in your main Projects; the client-specific material goes with the client's Project.

Keep these three categories distinct. If you find yourself wondering where a document belongs, ask: does this describe my practice permanently, does it describe how I do something, or does it describe a specific current engagement? That question usually resolves it.
...
**Structured PDF** (text-based, not scanned) works well when the document is logically formatted. A proposal template or a fee schedule in a clean PDF is fine. Claude can read it accurately if the text is selectable — i.e., not a scan or an image of a document.
...
**Mistake:** Uploading an existing client-facing document directly to a Project without reformatting it.

A capability deck written to impress prospects is not a useful knowledge document for Claude. It's written in marketing language, it omits operational detail, it prioritises persuasion over accuracy, and it buries the information Claude actually needs (what you do, for whom, at what fee, with what constraints) inside copy designed for a different purpose.

**Better approach:** Write a separate Claude-facing version of that document. Keep it short, plain, and specific. Write it the way you'd brief a smart new associate on their first day — factual, direct, structured. "{{businessName}} is a four-person business advisory firm in Hobart. We work exclusively with trade-services SMEs. Our clients are typically 5–25 employees. Our default engagement is a 12-week operating review. We do not do tax or audit work — we refer those to our partner firm, Hargreaves Tax. Our standard turnaround for a fee proposal is three business days." That briefing is more useful to Claude than a six-page deck.

### The update problem

Knowledge documents go stale. This is the most common reason a Project that worked well three months ago is producing weaker output now. The practice changed. The fee structure changed. A new service was added. A client finished their engagement. The knowledge documents didn't change with them, and now Claude is working from an outdated picture of your firm.
...
The trigger is the change, not the calendar. You add a new service — update your practice context document the same afternoon. You change your scoping process — update the process document before the next time you run the scoping workflow. You onboard a new client — create or update their client knowledge document before you start using the Project for their work.

The sign that a document has gone stale isn't always obvious. Sometimes it shows up as Claude getting details slightly wrong — mentioning a fee you no longer charge, describing a step that you've since removed, or failing to account for a constraint that now matters. When you see that, the fix is simple: find the document that contains the outdated information, correct it, and move on.
...
**Live operational data that changes frequently.** Your current engagement queue, this week's consultant utilisation, your bank balance, the status of open invoices. This kind of data goes stale within hours or days. Putting it in a knowledge document means Claude will use the stale version. Operational data belongs in the tools you use to manage it (your practice management software, your accounting platform) — not in a static knowledge document.
