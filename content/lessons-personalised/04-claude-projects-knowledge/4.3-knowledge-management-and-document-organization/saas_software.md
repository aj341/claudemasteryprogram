...
When founders first discover that Claude Projects can hold documents, the instinct is to upload everything. Marketing site copy, pricing page, last year's launch decks, photos of the team, the cloud-vendor contract from 2021, the spreadsheet you sort of use to track customer accounts. Everything gets loaded in, and then the Project is treated like a searchable filing cabinet.
...
**Business context.** This is who you are, how you operate, and what makes {{businessName}} distinct. It answers the background questions Claude would otherwise ask or guess at. Your product name, location, what it does, who your customers are, how you communicate, what you don't offer. The tone document from Lesson 1.5 belongs here. So does a one-page "about us" written for Claude — not for marketing, but as a plain-English briefing on your business that a new engineer would need on day one.

Business context documents don't change often. Once they're written and in the Project, they're mostly stable. You'll update them when something material changes — a new pricing tier, a different ICP, a rebrand — but you're not touching them weekly.

**Process knowledge.** This covers how you do things. Your release process. How you handle customer escalations. The steps you follow when onboarding a new account. How you structure a customer-success email. What happens after someone signs up for a trial. These documents exist so Claude understands your workflow and can help you at each stage of it — not just generate generic output, but output that fits how your SaaS actually operates.
...
**Customer and project knowledge.** This is the context specific to a current engagement — a customer's contract, a rollout scope document, notes from a discovery call, a strategic account's brand guide, the agreed deliverables for a launch you're delivering right now. This category is different from the other two: it changes with every new project and isn't useful once the project is done.

Most founders have one Project per strategic account or per active engagement, and the customer/project knowledge documents in that Project are specific to that work. When the engagement ends, the Project is archived. The business context and process documents stay in your main Projects; the customer-specific material goes with the customer's Project.

Keep these three categories distinct. If you find yourself wondering where a document belongs, ask: does this describe my product permanently, does it describe how I do something, or does it describe a specific current piece of work? That question usually resolves it.
...
**Plain text** is the most reliable. A `.txt` or `.md` file with headings, numbered steps, and short sections is what Claude reads with the highest accuracy. When you write a document specifically for Claude — a process guide, a product context brief, a set of instructions — write it in Markdown.

**Structured PDF** (text-based, not scanned) works well when the document is logically formatted. A proposal template or a pricing page in a clean PDF is fine. Claude can read it accurately if the text is selectable — i.e., not a scan or an image of a document.

**Image-based PDFs and screenshots of documents** are the least reliable. Claude can read them using vision, but accuracy drops when the layout is complex, when there's a lot of small text, or when the image quality is low. If an important document only exists as a scanned PDF or a screenshot, transcribe the relevant parts into plain text before adding it to a Project.
...
A customer-facing pricing page written to convert prospects is not a useful knowledge document for Claude. It's written in marketing language, it omits operational detail, it prioritises persuasion over accuracy, and it buries the information Claude actually needs (what your plans include, for whom, at what price, with what constraints) inside copy designed for a different purpose.

**Better approach:** Write a separate Claude-facing version of that document. Keep it short, plain, and specific. Write it the way you'd brief a smart new engineer on their first day — factual, direct, structured. "We are a 6-person micro-SaaS in Hobart. We sell a quoting tool to AU plumbers and electricians. Our customers are typically owner-operators with 1–10 staff. We integrate with Xero and MYOB. We do not support multi-currency or invoicing in non-AU jurisdictions. Our standard plan is $79/month with annual discount; the Pro plan is $129/month." That briefing is more useful to Claude than a six-page pricing page.

### The update problem

Knowledge documents go stale. This is the most common reason a Project that worked well three months ago is producing weaker output now. The product changed. A pricing tier changed. A new feature shipped. A customer finished their rollout. The knowledge documents didn't change with them, and now Claude is working from an outdated picture of your business.

The update problem doesn't need a complicated solution. You don't need a review calendar or a quarterly audit. You need one habit: when something about your product changes that a knowledge document covers, update the document that day.

The trigger is the change, not the calendar. You ship a new feature — update your product context document the same afternoon. You change your release process — update the process document before the next time you run the release workflow. You onboard a strategic account — create or update their customer knowledge document before you start using the Project for their work.

The sign that a document has gone stale isn't always obvious. Sometimes it shows up as Claude getting details slightly wrong — mentioning a price you no longer charge, describing a step that you've since automated away, or failing to account for a constraint that now matters. When you see that, the fix is simple: find the document that contains the outdated information, correct it, and move on.
...
**Sensitive credentials.** Passwords, API keys, Stripe secret keys, account numbers, login details, MFA backup codes. Claude Projects are not a password manager. Even if you trust the platform, storing credentials in a knowledge document creates a single location that could expose them if your account were ever accessed by someone else.

**Financial account details.** BSB and account numbers, credit card numbers, full banking records, your business savings details. A reference to the bank you use is fine. Your actual account numbers are not.

**Live operational data that changes frequently.** Your current support queue, today's signup numbers, your trial-to-paid pipeline, the status of open incidents. This kind of data goes stale within hours or days. Putting it in a knowledge document means Claude will use the stale version. Operational data belongs in the tools you use to manage it (your dashboards, your warehouse, your status page) — not in a static knowledge document.

The test is simple, {{firstName}}: if this information changed tomorrow and Claude used the old version, would that cause a problem? For credentials and financials, the answer is always yes. For operational data, it's almost always yes. Keep those out.
