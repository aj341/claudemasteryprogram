...
By this point in Module 4 you've built at least one working Project. You know how to set it up, populate a knowledge base, write instructions that hold, and maintain it as your store changes. That's a meaningful achievement. Most retailers who use Claude never get there.

But a single Project has a ceiling. It covers one domain of your work — one role Claude plays for you, one set of context and instructions. {{businessName}} doesn't run in one domain. You have customers to serve, operations to manage, marketing to maintain, maybe wholesale stockists to keep happy. One Project, configured for one of those domains, leaves the others back where you started: no persistent context, no tailored instructions, starting from scratch each conversation.

A Project System changes that. Not one Project — a small set of 2–3 Projects, each covering a different domain of your retail work, each configured precisely for that domain. Together they give Claude structured context across the parts of your store that matter most. Separately, each one is specific enough to be genuinely useful rather than generic.

This lesson is about building that system. Not in theory. In your actual store, this week.
...
Think about how your retail work actually divides. Most small retail operators run across three broad areas: customer-facing work (replies, returns, post-purchase), brand and content (product copy, campaigns, captions), and stockist or supplier work (wholesale comms, supplier follow-ups, inbound restocks). Those three areas have different audiences, different tones, different context Claude needs to do useful work. One Project trying to cover all three produces instructions so broad they're almost useless.

A Project System gives each domain its own configuration. Your customer-facing Project has your CS tone, your returns policy, your shipping rules. Your brand Project has your brand voice, your customer segments, your product structure. Your wholesale Project has your line sheet, your wholesale terms, your stockist roster. When you open the right Project, Claude is already calibrated. You spend your time on the task, not on re-explaining who you are and what you sell.

Two Projects is enough for many retailers. Three is a practical ceiling for this stage. More than three, and the system starts to require maintenance overhead that eats the time you were trying to save. Start with two if you're unsure. You can always add a third once the first two are running well.
...
**Name** — Short and specific. Not "Store" or "Business Claude" — something that tells you what it's for at a glance. "Customer Service," "Brand & Content," "Wholesale." The name is the first thing you see when you open Claude. Make it work for you.

**Purpose** — One or two sentences on what this Project handles. Not a list of every possible task — the core function. "All written replies to customers: 'where's my order,' returns, refunds, damaged stock, gift-card queries, sizing questions." That's a purpose. "Customer stuff" is not.

**Knowledge base** — What you've uploaded or written into the Project's knowledge. The documents, the reference materials, the context Claude needs to do useful work here. Your architecture doesn't need to list every file — just the categories. "Returns and exchanges policy, shipping FAQ, top-20 SKU list, brand tone guide."
...
Written out, your architecture for a small retail brand might look like this:

```
Project: Customer Service
Purpose: All written replies to customers via email, helpdesk, and DM.
Knowledge: Returns/exchanges policy, shipping FAQ, top SKU details, CS tone guide.
When to use: Drafting reply to any customer ticket — order, return, refund, complaint, or sizing query.

Project: Brand & Content
Purpose: All marketing-side writing — product copy, captions, campaign emails, blog.
Knowledge: Brand voice guide, range overview, three exemplar product descriptions, content pillars.
When to use: Writing product descriptions, social captions, email campaigns, restock alerts.
```
...
Your architecture document doesn't need to be elaborate. A table in Notion, a note in your phone, even a sticky note on your monitor — whatever you'll actually refer to. The format is irrelevant. The discipline of writing it out is what matters. It forces the clarity that makes the system work.

### Beat 3 — From Project builder to Project maintainer

Building a Project System is one job. Maintaining it is a different, ongoing job. Most retailers do the first and neglect the second. The system slowly drifts out of alignment with their actual store, and six months later it's no longer useful.

The maintenance job is not hard. It requires three habits, each of which takes about five minutes when you do it.

**Habit 1: Quarterly knowledge audit.** Every three months, open each Project and read through what's in the knowledge base. Ask: is this still accurate? Has anything in my store changed that this document doesn't reflect? Range changes, pricing changes, team changes, stockist changes. If the knowledge base still describes the brand you ran six months ago, it's giving Claude the wrong foundation. Update the documents, remove what's stale, add what's new.
...
In Module 5 you'll add capabilities that make your Projects substantially more powerful. Vision: the ability to give Claude product photos, screenshots, PDFs with visual content, and have it interpret them in the context of your Project — useful for product copy, packaging reviews, and visual feedback on a campaign asset. File processing: uploading documents — Shopify exports, supplier contracts, returns reports — and asking Claude to analyse, extract, or summarise them against the knowledge your Project already holds. And the first introduction to automation patterns: how to set up repeatable Claude workflows that run consistently without you rebuilding the context each time.
...
This is why the capstone matters, {{firstName}}. Not as a box to tick before Module 5 — as the infrastructure that makes Module 5 useful. If you go into Module 5 without a working Project System, you're adding capability to an unstable base. If you go in with two or three well-maintained Projects, every new capability has structured context to build on.
