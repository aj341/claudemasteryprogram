...
```
Project: Parent & Family Comms
Purpose: All written communication with current and prospective parents and families.
Knowledge: Program descriptions, fees, past enrolment examples, parent tone guide.
When to use: Drafting enrolment proposals, term updates, scope changes, parent check-ins.

Project: Operations
Purpose: Internal planning, tutor team communication, and business tracking.
Knowledge: Tutor roster, internal processes doc, weekly update format, financial context.
When to use: Writing tutor updates, planning documents, any task facing inward, not outward.
```

That is a functional architecture for {{businessName}}. Two Projects, clearly differentiated, each with a distinct domain. When a task arrives, you look at it for five seconds, you know which Project it belongs in, and you open that one.
