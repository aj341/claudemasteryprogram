...
You set up your first Project, {{firstName}}. You added your business context, a system prompt, and a few key documents. You ran a task and the output was noticeably better than what you were getting before — because Claude already knew {{businessName}} when the conversation started.

That's the foundation. This lesson covers what happens next: when your Project library grows past one, when other people in your business start needing access, when conversations accumulate and you have to manage them, and when you want Claude's output to end up somewhere useful rather than just sitting in a chat window on your phone.
...
That is the core value of a shared Project. Not convenience. Not fewer logins. It's the fact that when your office admin opens the Customer Comms Project, they're working with the same context as when you open it from the ute. Claude knows your services, your callout-fee structure, your supplier names, the way you phrase things to homeowners. You don't have to brief Claude separately for each person. The Project does the briefing for everyone who has access.

When does sharing make sense? When two or more people in your business regularly do the same type of Claude task. A shared Customer Communications Project makes sense if you have an admin and yourself both handling homeowner emails and you want them both sounding like the same business. A shared Onboarding Project makes sense if you have multiple people who train new apprentices. A shared Content Project makes sense if more than one person writes social posts or wrap-up emails.

When does sharing not make sense? When a Project contains information only one person should see — financial details, HR matters, a head contractor's confidential rates. Keep those as personal Projects. One person, one set of private documents, no shared access.
...
If you're drafting an email to a homeowner, you need Claude to know your communication tone, your service descriptions, and ideally the context of that particular customer relationship. That points to a Customer Communications Project.

If you're writing a quote, you need Claude to know your rate card, your service inclusions, and your standard terms. That points to a Quotes Project.

If you're writing a staff update, you need Claude to know your team structure, your internal voice, and the policies you've already documented (SWMS, JSAs, induction docs). That points to an Internal Operations Project.
...
Task type → Quote or variation → open Quotes Project
Task type → SWMS, JSA, or apprentice → open Internal Operations Project
...
Start a new conversation when the task is new — when you're working on something different, even if it's in the same general area. Working on a new homeowner email starts a new conversation. Working on a different quote starts a new conversation. Starting a new week's batch of social posts starts a new conversation.
...
This means if you started a conversation last week to draft a homeowner quote and left it at 60% complete, opening a new conversation and saying "continue the quote" won't work — Claude won't have access to what was generated in the old conversation. You'd need to open the old conversation and continue from there.

A practical naming habit helps here. Give each conversation a clear title as soon as you start it. Most Claude interfaces let you rename conversations. "Baker Street reno — switchboard variation" is easier to find and continue than "New conversation 14." Ten seconds of naming saves five minutes of scrolling.

When should you archive or delete old conversations? When the job is fully done. When the output has been sent, the invoice is paid, or the defect liability has expired, you don't need the conversation anymore. An accumulation of finished conversations adds scroll time without adding value. Clear out the done ones regularly — once a month is usually enough.

### Common mistake → Better approach

**Mistake:** Using one Project for every type of task, because it's simpler than managing several.

A tradie creates one Project called "Business" and uploads every document they have — service list, rate card, supplier contacts, SWMS template, sample quotes, apprentice induction notes. The system prompt tries to cover everything: tone, trade, customers, format preferences for every output type. The Project becomes a general-purpose context that is partially useful for every task but precisely right for none. Claude is working from a system prompt that covers too much ground, and the document library is too broad to be applied cleanly to any single task.

**Better approach:** One Project per recurring task type. A Customer Communications Project with just the documents relevant to homeowner work. A Quotes Project with rate card, service descriptions, and standard terms. An Internal Project with team-facing documents (SWMS, JSAs, apprentice induction). Each system prompt is short and focused. When you open a Project, the context is immediately relevant to the task at hand. You might have three or four Projects rather than one, but each one works better than a single overloaded Project ever could.
...
**Context management as a habit.** Treat each conversation like a work session. When you close a task, paste the final output somewhere permanent — your job-management system, your document folder, your email drafts. Don't rely on Claude's conversation history as your storage system. The conversation is a working space, not a filing cabinet.

### Integrating Projects with external tools — no code required

Claude doesn't connect directly to your job-management software, your accounting tool, or your email system without additional technical setup. But that doesn't mean Claude stays isolated. The most practical integration approach for trades businesses is structured copy-paste — and it works well.
...
**From email to Claude and back.** Copy the text of a homeowner email, paste it into your Project conversation with a clear instruction ("Draft a reply to this email in our standard customer tone, addressing each of the three questions"), read Claude's draft, copy it, paste it into your email reply. The Project handles the standing context — your tone, your services, your terms. Your prompt handles the specific task. Your email client handles the send.

**From a document to Claude and back.** Copy the relevant section of a head contractor's contract, a builder's brief, or a strata scope into Claude with a specific task ("Summarise the key obligations from this contract section in plain English — what do we have to do, and by when"). Take the summary, paste it into your job-management tool as a task note. Claude processed the document. Your tool stores the output. You did no technical integration.

**From your job-management software to Claude and back.** Copy a job record or a set of site-meeting notes from Tradify or simPRO, paste into Claude ("Based on these site notes, draft a follow-up email and a list of three next actions"). Take Claude's follow-up email and paste it into your email. Take the three next actions and paste them into your job tracker. The Project's customer communications context does the heavy lifting. Your copy-paste moves the information between systems.

This approach has limits. It's manual. It doesn't scale to hundreds of tasks per day. But for a trades business running twenty to fifty Claude tasks per week across the whole team, structured copy-paste is efficient, reliable, and requires no technical setup. It's also easier to review and correct than automated integrations that run without human oversight.

When does automation make sense? When you're running the same workflow multiple times every day, the inputs are always in the same format, and the output goes to the same place every time. At that point, it's worth talking to someone about a lightweight integration — tools like Zapier or Make can connect Claude to your job-management software or email without code. But that's a future step, not a Module 4 step.

For now, structured copy-paste is the right tool. It puts a human in the loop, which is appropriate for customer-facing output. It requires no setup. And it works well with Projects precisely because the Project carries the standing context — you're not re-briefing Claude every time you paste something in. The Project already knows your business. You're just bringing it the raw material for the current task.
