...
You set up your first Project. You added your practice context, a system prompt, and a few key documents. You ran a task and the output was noticeably better than what you were getting before — because Claude already knew your firm when the conversation started.

That's the foundation. This lesson covers what happens next: when your Project library grows past one, when other people in your practice start needing access, when conversations accumulate and you have to manage them, and when you want Claude's output to end up somewhere useful rather than just sitting in a chat window.
...
That is the core value of a shared Project. Not convenience. Not fewer logins. It's the fact that when your associate opens the Client Comms Project, they're working with the same context as when you open it. Claude knows your service lines, your standard terms, your client names, the way you phrase things in updates. You don't have to brief Claude separately for each person. The Project does the briefing for everyone who has access.

When does sharing make sense? When two or more people in your practice regularly do the same type of Claude task. A shared Client Communications Project makes sense if you have two consultants handling client emails and you want them both sounding like the same firm. A shared Onboarding Project makes sense if you have multiple people who run engagement kickoffs. A shared Content Project makes sense if more than one person writes articles or LinkedIn posts.

When does sharing not make sense? When a Project contains information only one person should see — fee details, HR matters, client engagement letters with confidentiality clauses. Keep those as personal Projects. One person, one set of private documents, no shared access.
...
If you're writing a fee proposal, you need Claude to know your fee tiers, your scope inclusions, and your standard terms. That points to a Proposals Project.

If you're writing a team update, you need Claude to know your firm structure, your internal voice, and the policies you've already documented. That points to an Internal Operations Project.
...
Task type → Fee proposal → open Proposals Project
...
Start a new conversation when the task is new — when you're working on something different, even if it's in the same general area. Working on a new client email starts a new conversation. Working on a different fee proposal starts a new conversation. Starting a new week's batch of LinkedIn posts starts a new conversation.
...
This means if you started a conversation last week to draft a client scope and left it at 60% complete, opening a new conversation and saying "continue the scope" won't work — Claude won't have access to what was generated in the old conversation. You'd need to open the old conversation and continue from there.

A practical naming habit helps here. Give each conversation a clear title as soon as you start it. Most Claude interfaces let you rename conversations. "Baker Street Café — fee proposal August" is easier to find and continue than "New conversation 14." Ten seconds of naming saves five minutes of scrolling.
...
```
I'm continuing work on the Northside Logistics scope of work.
We've agreed on: 10-week engagement, weekly status calls, fixed-fee billing
in three milestones.
Still to complete: the deliverables section and the assumptions clause.
Please draft the deliverables section next, using our standard scope
language from the Project knowledge.
```
...
**Context management as a habit.** Treat each conversation like a work session. When you close a task, paste the final output somewhere permanent — your practice management tool, your document folder, your email drafts. Don't rely on Claude's conversation history as your storage system. The conversation is a working space, not a filing cabinet.

### Integrating Projects with external tools — no code required

Claude doesn't connect directly to your practice management tool, your project tool, or your email system without additional technical setup. But that doesn't mean Claude stays isolated. The most practical integration approach for small firms is structured copy-paste — and it works well.
...
**From a document to Claude and back.** Copy the relevant section of an engagement letter, a brief, or a client form into Claude with a specific task ("Summarise the key obligations from this engagement letter section in plain English — what do we have to do, and by when"). Take the summary, paste it into your project management tool as a task note. Claude processed the document. Your tool stores the output. You did no technical integration.

**From your practice tool to Claude and back.** Copy a client record or a set of meeting notes from your practice management system, paste into Claude ("Based on these meeting notes, draft a follow-up email and a list of three next actions"). Take Claude's follow-up email and paste it into your email. Take the three next actions and paste them into your task manager. The Project's client communications context does the heavy lifting. Your copy-paste moves the information between systems.

This approach has limits. It's manual. It doesn't scale to hundreds of tasks per day. But for a small firm running twenty to fifty Claude tasks per week across a team, structured copy-paste is efficient, reliable, and requires no technical setup. It's also easier to review and correct than automated integrations that run without human oversight.

When does automation make sense? When you're running the same workflow multiple times every day, the inputs are always in the same format, and the output goes to the same place every time. At that point, it's worth talking to someone about a lightweight integration — tools like Zapier or Make can connect Claude to your practice tools or email without code. But that's a future step, not a Module 4 step.

For now, structured copy-paste is the right tool. It puts a human in the loop, which is appropriate for client-facing output. It requires no setup. And it works well with Projects precisely because the Project carries the standing context — you're not re-briefing Claude every time you paste something in. The Project already knows your firm. You're just bringing it the raw material for the current task.
