...
You set up your first Project. You added your practice context, a system prompt, and a few key documents. You ran a task and the output was noticeably better than what you were getting before — because Claude already knew {{businessName}} when the conversation started.

That's the foundation, {{firstName}}. This lesson covers what happens next: when your Project library grows past one, when other people in your firm start needing access, when conversations accumulate and you have to manage them, and when you want Claude's output to end up somewhere useful rather than just sitting in a chat window.
...
That is the core value of a shared Project. Not convenience. Not fewer logins. It's the fact that when your bookkeeper opens the Accounts Project, they're working with the same context as when you open it. Claude knows your chart of accounts, your fee tiers, your supplier names, the way you phrase things to clients. You don't have to brief Claude separately for each person. The Project does the briefing for everyone who has access.

When does sharing make sense? When two or more people in your practice regularly do the same type of Claude task. A shared Client Communications Project makes sense if you have two people handling client emails and you want them both sounding like the same firm. A shared Onboarding Project makes sense if you have multiple people who train new staff. A shared Content Project makes sense if more than one person writes copy or social posts.

When does sharing not make sense? When a Project contains information only one person should see — partner-level financials, HR matters, client engagement letters with confidentiality clauses. Keep those as personal Projects. One person, one set of private documents, no shared access.
...
If you're writing a fee proposal, you need Claude to know your fee structure, your scope inclusions, and your standard terms. That points to a Proposals Project.
...
Task type → Fee or proposal → open Proposals Project
...
Start a new conversation when the task is new — when you're working on something different, even if it's in the same general area. Working on a new client email starts a new conversation. Working on a different fee proposal starts a new conversation. Starting a new BAS quarter's batch of reminders starts a new conversation.
...
This means if you started a conversation last week to draft a fee proposal and left it at 60% complete, opening a new conversation and saying "continue the proposal" won't work — Claude won't have access to what was generated in the old conversation. You'd need to open the old conversation and continue from there.

A practical naming habit helps here. Give each conversation a clear title as soon as you start it. Most Claude interfaces let you rename conversations. "Baker Street Café — fee proposal Q3" is easier to find and continue than "New conversation 14." Ten seconds of naming saves five minutes of scrolling.

When should you archive or delete old conversations? When the task is fully done. When the output has been sent, saved, or delivered (and the file note is filed), you don't need the conversation anymore. An accumulation of finished conversations adds scroll time without adding value. Clear out the done ones regularly — once a month is usually enough.

### Common mistake → Better approach

**Mistake:** Using one Project for every type of task, because it's simpler than managing several.

A practitioner creates one Project called "Practice" and uploads every document they have — brand guide, service descriptions, fee schedule, team roster, supplier contacts, client correspondence templates. The system prompt tries to cover everything: tone, industry, client mix, format preferences for every output type. The Project becomes a general-purpose context that is partially useful for every task but precisely right for none. Claude is working from a system prompt that covers too much ground, and the document library is too broad to be applied cleanly to any single task.

**Better approach:** One Project per recurring task type. A Client Communications Project with just the documents relevant to client work. A Fee Proposals Project with fee schedule, service descriptions, and standard terms. An Internal Project with team-facing documents. Each system prompt is short and focused. When you open a Project, the context is immediately relevant to the task at hand. You might have three or four Projects rather than one, but each one works better than a single overloaded Project ever could.
...
```
I'm continuing work on the Northside Plumbing FY26 engagement letter.
We've agreed on: 12-month term, monthly billing, 30-day termination
notice, GST inclusive fees. Still to complete: the scope of work section
and the payment terms clause. Please draft the scope of work section
next, using our standard service descriptions from the Project knowledge.
```
...
**Context management as a habit.** Treat each conversation like a work session. When you close a task, paste the final output somewhere permanent — your practice management system, your client folder, your email drafts. Don't rely on Claude's conversation history as your storage system. The conversation is a working space, not a filing cabinet.

### Integrating Projects with external tools — no code required

Claude doesn't connect directly to your practice management software, Xero, or your email system without additional technical setup. But that doesn't mean Claude stays isolated. The most practical integration approach for small AU practices is structured copy-paste — and it works well.
...
**From email to Claude and back.** Copy the text of a client email, paste it into your Project conversation with a clear instruction ("Draft a reply to this email in our standard client tone, addressing each of the three BAS questions"), read Claude's draft, copy it, paste it into your email reply. The Project handles the standing context — your tone, your services, your terms. Your prompt handles the specific task. Your email client handles the send.

**From a document to Claude and back.** Copy the relevant section of an engagement letter, a fee proposal, or a client form into Claude with a specific task ("Summarise the key obligations from this engagement letter section in plain English — what do we have to do, and by when"). Take the summary, paste it into your practice management system as a task note. Claude processed the document. Your system stores the output. You did no technical integration.

**From your practice management system to Claude and back.** Copy a client file note or a set of meeting notes, paste into Claude ("Based on these meeting notes, draft a follow-up email and a list of three next actions for the file"). Take Claude's follow-up email and paste it into your email. Take the three next actions and paste them into your task manager or practice management workflow. The Project's client communications context does the heavy lifting. Your copy-paste moves the information between systems.

This approach has limits. It's manual. It doesn't scale to hundreds of tasks per day. But for a small practice running twenty to fifty Claude tasks per week across a team, structured copy-paste is efficient, reliable, and requires no technical setup. It's also easier to review and correct than automated integrations that run without human oversight — important when TPB and confidentiality obligations apply.

When does automation make sense? When you're running the same workflow multiple times every day, the inputs are always in the same format, and the output goes to the same place every time. At that point, it's worth talking to someone about a lightweight integration — tools like Zapier or Make can connect Claude to your practice management software or email without code. But that's a future step, not a Module 4 step.

For now, structured copy-paste is the right tool. It puts a human in the loop, which is appropriate for client-facing output. It requires no setup. And it works well with Projects precisely because the Project carries the standing context — you're not re-briefing Claude every time you paste something in. The Project already knows your practice. You're just bringing it the raw material for the current task.
