...
You set up your first Project. You added your business context, a system prompt, and a few key documents. You ran a task and the output was noticeably better than what you were getting before — because Claude already knew your product when the conversation started.

That's the foundation. This lesson covers what happens next: when your Project library grows past one, when other people in your SaaS start needing access, when conversations accumulate and you have to manage them, and when you want Claude's output to end up somewhere useful rather than just sitting in a chat window.

None of this requires deep technical skills. It does require a few clear decisions. This lesson gives you the framework for those decisions.
...
That is the core value of a shared Project. Not convenience. Not fewer logins. It's the fact that when your customer-success lead opens the CS Project, they're working with the same context as when you open it. Claude knows your plan tiers, your pricing terms, your strategic accounts, the way you phrase things to customers. You don't have to brief Claude separately for each person. The Project does the briefing for everyone who has access.

When does sharing make sense? When two or more people in your SaaS regularly do the same type of Claude task. A shared Customer Communications Project makes sense if you have two people handling Intercom replies and you want them both sounding like the same product. A shared Onboarding Project makes sense if you have multiple people who run new-account kickoffs. A shared Content Project makes sense if more than one person writes release notes or LinkedIn posts.

When does sharing not make sense? When a Project contains information only one person should see — investor financials, hiring shortlists, customer contracts with confidentiality clauses. Keep those as personal Projects. One person, one set of private documents, no shared access.
...
The practical benefit of a shared Project is what you might call single source of truth. When you update the system prompt — because your features changed, or your pricing updated, or your brand tone shifted — it updates for everyone in the Project immediately. No need to send an updated brief to each person. No risk that one person is running on old instructions while another has the new ones. One edit, consistent behaviour.

### Using multiple Projects together

By the time you finish Module 4 you might have two, three, or four Projects, {{firstName}}. A Customer Communications Project. A Release Notes Project. An Internal Operations Project. Maybe one for a specific quarterly campaign.
...
If you're drafting an email to a customer, you need Claude to know your communication tone, your feature descriptions, and ideally the context of that particular account. That points to a Customer Communications Project.

If you're writing a release note, you need Claude to know your changelog format, your audience, and your standard structure. That points to a Release Notes Project.

If you're writing a team update, you need Claude to know your team structure, your internal voice, and the standards you've already documented. That points to an Internal Operations Project.
...
Task type → Customer comms → open Customer Communications Project
Task type → Release note → open Release Notes Project
...
Start a new conversation when the task is new — when you're working on something different, even if it's in the same general area. Working on a new customer email starts a new conversation. Working on a different release note starts a new conversation. Starting a new week's batch of social posts starts a new conversation.
...
This means if you started a conversation last week to draft a customer renewal proposal and left it at 60% complete, opening a new conversation and saying "continue the proposal" won't work — Claude won't have access to what was generated in the old conversation. You'd need to open the old conversation and continue from there.

A practical naming habit helps here. Give each conversation a clear title as soon as you start it. Most Claude interfaces let you rename conversations. "Baker Street Café — renewal August" is easier to find and continue than "New conversation 14." Ten seconds of naming saves five minutes of scrolling.

When should you archive or delete old conversations? When the task is fully done. When the output has been sent, saved, or shipped, you don't need the conversation anymore. An accumulation of finished conversations adds scroll time without adding value. Clear out the done ones regularly — once a month is usually enough.

### Common mistake → Better approach

**Mistake:** Using one Project for every type of task, because it's simpler than managing several.

A founder creates one Project called "SaaS" and uploads every document they have — brand guide, feature descriptions, pricing tiers, team handbook, vendor list, customer email templates. The system prompt tries to cover everything: tone, category, customers, format preferences for every output type. The Project becomes a general-purpose context that is partially useful for every task but precisely right for none. Claude is working from a system prompt that covers too much ground, and the document library is too broad to be applied cleanly to any single task.

**Better approach:** One Project per recurring task type. A Customer Communications Project with just the documents relevant to customer work. A Release Notes Project with format template, audience definition, and example releases. An Internal Project with team-facing documents. Each system prompt is short and focused. When you open a Project, the context is immediately relevant to the task at hand. You might have three or four Projects rather than one, but each one works better than a single overloaded Project ever could.
...
```
I'm continuing work on the Northside Plumbing renewal proposal at
{{businessName}}. We've agreed on: 12-month term, monthly billing,
30-day termination notice. Still to complete: the scope of services
section and the support response-time clause. Please draft the scope
section next, using our standard plan descriptions from the Project
knowledge.
```
...
Claude doesn't connect directly to your CRM, your project management tool, or your email system without additional technical setup. But that doesn't mean Claude stays isolated. The most practical integration approach for small SaaS teams is structured copy-paste — and it works well.

The pattern is this: bring information from your external tool into Claude, get Claude's output, then take that output back into your external tool. No code. No plugins. No engineering involvement.

Here are the most common versions of this workflow.

**From Intercom to Claude and back.** Copy the text of a customer thread, paste it into your Project conversation with a clear instruction ("Draft a reply to this thread in our standard CS tone, addressing each of the three questions"), read Claude's draft, copy it, paste it into your Intercom reply. The Project handles the standing context — your tone, your features, your terms. Your prompt handles the specific task. Intercom handles the send.

**From a document to Claude and back.** Copy the relevant section of a security questionnaire, a brief, or a customer contract into Claude with a specific task ("Summarise the key obligations from this DPA section in plain English — what do we have to do, and by when"). Take the summary, paste it into your Linear ticket as a task note. Claude processed the document. Linear stores the output. You did no technical integration.

**From your CRM to Claude and back.** Copy a customer record or a set of demo notes from Pipedrive, paste into Claude ("Based on these demo notes, draft a follow-up email and a list of three next actions"). Take Claude's follow-up email and paste it into Gmail. Take the three next actions and paste them into Pipedrive as activities. The Project's customer communications context does the heavy lifting. Your copy-paste moves the information between systems.

This approach has limits. It's manual. It doesn't scale to hundreds of tasks per day. But for a small SaaS team running twenty to fifty Claude tasks per week across a team of five to fifteen, structured copy-paste is efficient, reliable, and requires no technical setup. It's also easier to review and correct than automated integrations that run without human oversight.

When does automation make sense? When you're running the same workflow multiple times every day, the inputs are always in the same format, and the output goes to the same place every time. At that point, it's worth talking to your engineer about a lightweight integration — tools like Zapier, Make, or the Anthropic API can connect Claude to your CRM or email. But that's a future step, not a Module 4 step.

For now, structured copy-paste is the right tool. It puts a human in the loop, which is appropriate for customer-facing output. It requires no setup. And it works well with Projects precisely because the Project carries the standing context — you're not re-briefing Claude every time you paste something in. The Project already knows your product. You're just bringing it the raw material for the current task.
