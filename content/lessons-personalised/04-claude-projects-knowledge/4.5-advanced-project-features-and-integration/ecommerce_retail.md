...
You set up your first Project for {{businessName}}. You added your brand context, a system prompt, and a few key documents. You ran a task and the output was noticeably better than what you were getting before — because Claude already knew your store when the conversation started.
...
That is the core value of a shared Project. Not convenience. Not fewer logins. It's the fact that when your customer-service casual opens the CS Project, they're working with the same context as when you open it. Claude knows your returns policy, your standard reply tone, your supplier names, the way you phrase things to customers. You don't have to brief Claude separately for each person. The Project does the briefing for everyone who has access.

When does sharing make sense? When two or more people in your store regularly do the same type of Claude task. A shared Customer Service Project makes sense if you have two casuals handling tickets and you want them both sounding like the same brand. A shared Onboarding Project makes sense if you have multiple people who train new floor staff. A shared Content Project makes sense if more than one person writes captions or email copy.

When does sharing not make sense? When a Project contains information only one person should see — financial details, HR matters, wholesale agreements with confidentiality clauses. Keep those as personal Projects. One person, one set of private documents, no shared access.
...
The practical benefit of a shared Project is what you might call single source of truth. When you update the system prompt — because your range changed, or your shipping cut-off updated, or your brand tone shifted — it updates for everyone in the Project immediately. No need to send an updated brief to each casual. No risk that one person is running on old instructions while another has the new ones. One edit, consistent behaviour.

### Using multiple Projects together

By the time you finish Module 4 you might have two, three, or four Projects. A Customer Service Project. A Product Copy Project. A Wholesale Project. Maybe one for a specific seasonal campaign like Mother's Day or EOFY.
...
If you're drafting a reply to an upset customer, you need Claude to know your customer-service tone, your returns policy, and ideally the context of that particular order. That points to a Customer Service Project.

If you're writing a product description for a new SKU, you need Claude to know your brand voice, your standard product-page structure, and your tone rules. That points to a Product Copy Project.

If you're writing a stockist follow-up, you need Claude to know your wholesale terms, your stockist roster, and the way you write to buyers. That points to a Wholesale Project.
...
Task type → Customer reply → open Customer Service Project
Task type → Product description → open Product Copy Project
Task type → Stockist comms → open Wholesale Project
...
Start a new conversation when the task is new — when you're working on something different, even if it's in the same general area. Working on a new customer ticket starts a new conversation. Working on a different stockist's follow-up starts a new conversation. Starting a new week's batch of social captions starts a new conversation.
...
This means if you started a conversation last week to draft a wholesale pitch for a Sydney boutique and left it at 60% complete, opening a new conversation and saying "continue the pitch" won't work — Claude won't have access to what was generated in the old conversation. You'd need to open the old conversation and continue from there.

A practical naming habit helps here. Give each conversation a clear title as soon as you start it. Most Claude interfaces let you rename conversations. "Paddington Boutique — wholesale pitch May" is easier to find and continue than "New conversation 14." Ten seconds of naming saves five minutes of scrolling.

When should you archive or delete old conversations? When the task is fully done. When the output has been sent, scheduled, or shipped, you don't need the conversation anymore. An accumulation of finished conversations adds scroll time without adding value. Clear out the done ones regularly — once a month is usually enough.

### Common mistake → Better approach

**Mistake:** Using one Project for every type of task, because it's simpler than managing several.

A retailer creates one Project called "Store" and uploads every document they have — brand guide, full SKU list, wholesale terms, supplier contacts, customer reply templates. The system prompt tries to cover everything: tone, customer types, wholesale buyers, format preferences for every output type. The Project becomes a general-purpose context that is partially useful for every task but precisely right for none. Claude is working from a system prompt that covers too much ground, and the document library is too broad to be applied cleanly to any single task.

**Better approach:** One Project per recurring task type. A Customer Service Project with just the documents relevant to customer replies. A Product Copy Project with brand voice, structure, and a few good examples. A Wholesale Project with line sheet, terms, and stockist roster. Each system prompt is short and focused. When you open a Project, the context is immediately relevant to the task at hand. You might have three or four Projects rather than one, but each one works better than a single overloaded Project ever could.
...
```
I'm continuing work on the Northside Homewares wholesale pitch.
We've agreed on: 6-piece MOQ per SKU, 14-day payment, sample box of three SKUs sent before commitment.
Still to complete: the line-sheet attachment cover note and the follow-up timing.
Please draft the cover note next, using our standard wholesale tone from the Project knowledge.
```
...
**Context management as a habit.** Treat each conversation like a work session. When you close a task, paste the final output somewhere permanent — your helpdesk, your Shopify product page, your Klaviyo campaign draft. Don't rely on Claude's conversation history as your storage system. The conversation is a working space, not a filing cabinet.

### Integrating Projects with external tools — no code required

Claude doesn't connect directly to Shopify, your helpdesk, or your email system without additional technical setup. But that doesn't mean Claude stays isolated. The most practical integration approach for small retailers is structured copy-paste — and it works well.
...
**From helpdesk to Claude and back.** Copy the text of a customer ticket, paste it into your Project conversation with a clear instruction ("Draft a reply in our standard customer-service tone, addressing each of the three questions"), read Claude's draft, copy it, paste it into your helpdesk reply. The Project handles the standing context — your tone, your returns policy, your shipping rules. Your prompt handles the specific task. Your helpdesk handles the send.

**From a document to Claude and back.** Copy the relevant section of a wholesale agreement, a supplier contract, or a customer-feedback export into Claude with a specific task ("Summarise the key obligations from this section in plain English — what do we have to do, and by when"). Take the summary, paste it into your project management tool as a task note. Claude processed the document. Your tool stores the output. You did no technical integration.

**From Shopify to Claude and back.** Copy a customer's order history or a product's description from Shopify, paste into Claude ("Based on this order history, draft a follow-up email and a list of three next actions"). Take Claude's follow-up email and paste it into Klaviyo or Gmail. Take the three next actions and paste them into your helpdesk or task manager. The Project's customer-service context does the heavy lifting. Your copy-paste moves the information between systems.

This approach has limits. It's manual. It doesn't scale to hundreds of tasks per day. But for a small store running twenty to fifty Claude tasks per week across a small team, structured copy-paste is efficient, reliable, and requires no technical setup. It's also easier to review and correct than automated integrations that run without human oversight.

When does automation make sense? When you're running the same workflow multiple times every day, the inputs are always in the same format, and the output goes to the same place every time. At that point, it's worth talking to someone about a lightweight integration — tools like Zapier or Make can connect Claude to your helpdesk or email without code. But that's a future step, not a Module 4 step.

For now, structured copy-paste is the right tool. It puts a human in the loop, which is appropriate for customer-facing output. It requires no setup. And it works well with Projects precisely because the Project carries the standing context — you're not re-briefing Claude every time you paste something in. The Project already knows your store. You're just bringing it the raw material for the current task.
