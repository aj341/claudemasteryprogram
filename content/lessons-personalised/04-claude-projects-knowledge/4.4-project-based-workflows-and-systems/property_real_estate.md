...
Most agencies start using Claude the same way. A task appears — a vendor update, a listing description, a tenant email. They open Claude, type a prompt, get an output, and move on. The task is done.

That's useful. But it's not where the real time savings come from.

The bigger gain comes when you stop treating Claude as a task tool and start treating it as part of a workflow. A workflow is a recurring, multi-step process — something you do weekly, fortnightly, or monthly, with mostly the same inputs and mostly the same required outputs. Every agency has several of these. The question is whether Claude has a defined role in each one, or whether {{firstName}} is improvising every time.

When Claude has no defined role in a workflow, you repeat the same mental overhead every time: what context does Claude need here? What format should I ask for? What am I actually asking it to do? That friction adds up. It doesn't feel like much per task, but across a month of weekly vendor updates, monthly owner reports, and lease-renewal letters, it's significant.

When Claude has a defined role, that overhead drops to near zero. You know which steps Claude handles. You know what prompt to use. You know what you get back and what you do with it. The process becomes repeatable, and repeatable processes are the foundation of an agency that doesn't depend entirely on the principal thinking hard every day.
...
**First: map all the steps.** Write out the full process from start to finish. Don't skip steps because they seem obvious. "Check the agency agreement" is a step. "Notify the vendor" is a step. Every action is a step.

**Second: decide which steps Claude handles, which you handle, and which are handoffs.** Not every step belongs to Claude. Some require your judgement, your relationships with vendors and landlords, or information Claude can't have. Some are genuinely faster for you to do yourself. Some steps are transitions — places where Claude produces something and you take it to the next phase, or where you gather information and pass it to Claude.
...
**New listing onboarding workflow**

This is the process that runs from "vendor has signed the agency agreement" to "campaign is live and the vendor knows what to expect." Most agencies have this process. Most run it inconsistently because there's no defined system.

Step 1 — You receive the signed agency agreement. (You)
Step 2 — Claude drafts the vendor welcome email confirming the campaign plan, key dates, and what happens next. (Claude)
Step 3 — You review and send the welcome email. (Handoff: Claude → you → vendor)
Step 4 — Claude generates the listing preparation checklist for this property type from the Project knowledge base. (Claude)
Step 5 — You confirm the checklist is correct for this specific property and tick off any items that don't apply. (You)
Step 6 — Claude drafts the internal campaign brief the sales team needs before the photographer arrives. (Claude)
Step 7 — You review the brief, add anything Claude couldn't know about the vendor's situation, and share it with the team. (Handoff)

In this workflow, Claude handles three of the seven steps: the welcome email, the checklist, and the internal brief. You handle the parts that require judgement about the specific vendor and property. The handoffs are explicit: Claude produces something, you review it before it goes anywhere.

**Monthly owner reporting workflow**

This is the process of producing a monthly report for each landlord on the rent roll — covering rent received, arrears, maintenance, inspections, and any matters needing attention. The data exists in the trust accounting and PM software. The job is to interpret it and communicate it clearly.

Step 1 — Your PM pulls the data from the trust accounting platform, the inspection notes, and the maintenance log. (You / PM)
Step 2 — The PM pastes the relevant data into Claude with a brief note on anything significant — a tenant change, an outstanding repair, an upcoming lease renewal. (Handoff: you → Claude)
Step 3 — Claude drafts the owner report narrative — what the rent roll for this property has done this month, what's coming up, and what action the landlord needs to take. (Claude)
Step 4 — The PM reads the draft and checks whether the interpretation is accurate. This is the critical check. Claude works from the data given to it; it can't know if a number looks odd because of a posting error. (You)
Step 5 — Claude drafts the action items or "next month focus" section based on the narrative. (Claude)
Step 6 — The PM approves, edits, and sends to the landlord. (Handoff)

The workflow is efficient because Claude handles the writing and the interpretation logic. Your team handles the data gathering and the accuracy check — the parts that require knowing what the numbers actually mean for the specific property.

**Vendor update workflow**

This is the process of sending a weekly vendor update during an active sales campaign — typically after the Saturday open home and the mid-week buyer feedback. The challenge isn't writing one update; it's repeating the process across 8–12 active listings without it consuming your Sunday.

Step 1 — You note the OFI numbers, the buyer feedback, and any specific question or concern the vendor raised this week. (You)
Step 2 — Claude drafts the vendor update from your brief. (Claude)
Step 3 — You read the draft and identify what needs fixing: missing specifics about a particular buyer, wrong tone for this vendor, anything that doesn't reflect what actually happened. (You)
Step 4 — Claude revises based on your feedback. (Claude, after handoff)
Step 5 — Claude produces the short-form variations: an SMS summary, a follow-up email subject line, an internal team note. (Claude)
Step 6 — You do a final read on everything before sending. (You — this is always the last step)

The key handoff point in this workflow is step 3. You have to read the draft before it moves forward. Skipping that check — sending Claude's update straight to the vendor — is how you end up with a message that's factually accurate but sounds nothing like the agent who's been talking to that vendor for three weeks, or that misrepresents a buyer comment in a way the vendor will pick up on at the next open home.

### The handoff problem

Handoffs are the most important part of any Claude-assisted workflow, and the part most agencies underdesign.

A handoff is a point in the workflow where Claude produces something that will affect the real world — an email that will be sent to a vendor, a report that will go to a landlord, a listing description that will be published on REA and Domain. At every one of these points, you need a review step before the output leaves your hands.

This isn't about distrust. Claude's output quality in a well-designed workflow is high. The issue is that Claude produces text from what you gave it, and what you gave it might be incomplete, slightly wrong, or missing context that you know but didn't write down — the off-the-record buyer comment, the vendor's family situation, the fact that the next-door neighbour is also about to list. Claude doesn't know what it doesn't know. It will produce a confident, well-structured output even when it's working from information that has a gap.

Your review at the handoff point is the place where you close that gap. It doesn't have to be long. A 30-second read of a vendor update before you hit send is a handoff review. A 2-minute check of an owner report narrative to confirm the interpretation matches what actually happened on the property is a handoff review.

The rule is simple: Claude's output goes through you before it goes to anyone else — vendor, landlord, tenant, buyer. No exceptions, even when the workflow is running smoothly and Claude's outputs have been good for three months.

### Common mistake → Better approach

**Mistake:** Using Claude in a workflow without telling it where the step fits.

You're running your monthly owner reporting workflow. At step 3, you paste the rent roll data and ask: "Write a summary of this month's performance." Claude produces a reasonable paragraph. But it's written like a standalone piece — it doesn't connect to anything before or after. It reads like a fresh interpretation, not like a section of an ongoing report that the landlord has been reading for eight months.

**Better approach:** Include the workflow context in the prompt:

```
I'm preparing the April 2026 owner report for 14 Banksia Drive,
Coffs Harbour — managed by {{businessName}}. This narrative section
follows the rent and arrears table the landlord has already seen.
It should interpret the figures — not restate them — and connect
to the "next month focus" section that follows.

Rent received: $2,860 (March: $2,860, on time)
Arrears: $0 (clear since lease commenced)
Maintenance: 1 completed (hot water service replaced, $1,180 inc GST)
Inspections: routine completed 18 April, condition rated good
Lease renewal: due 30 June, current rent $660/wk, market $695–$710/wk

Write the narrative section in plain, direct English. 150–200 words.
The landlord prefers honest interpretation over salesy language.
Mention the upcoming lease renewal and recommend a market rent review.
```

The difference is the framing: "this follows the rent and arrears table" and "connects to the next month focus section" tells Claude what comes before and after. That context changes how it writes the output. It becomes a section of a workflow, not an isolated task.

### Systemising versus automating

Here's a distinction that matters for most agencies in this course.

Automating a workflow means software does the steps without you. A trigger fires, a tool runs, an output appears — and you weren't involved. Automation requires technical setup: Zapier, Make, API connections, code. For most principals running independent agencies, automation is not accessible without outside help, and it introduces reliability risks when a CRM integration breaks the morning before settlement.
...
Systemising is accessible to any agency, right now, without technical skills. You design the workflow, write the prompts, and run it. It's manual in the sense that you're doing it — but it's not improvised. Every vendor update, every owner report, every lease renewal letter runs the same way. That consistency is where the time saving and quality improvement come from.

For most independent agencies, systemising is the right goal. It's achievable, sustainable, and it doesn't break when a software integration fails on a Saturday morning before open homes.
