...
Most consultants start using Claude the same way. A task appears. They open Claude, type a prompt, get an output, and move on. The task is done.
...
**First: map all the steps.** Write out the full process from start to finish. Don't skip steps because they seem obvious. "Re-read the brief" is a step. "Notify the client" is a step. Every action is a step.
...
This is the process that runs from "client has signed the engagement letter" to "kickoff is booked and the work can begin." Most consulting firms have this process. Most run it inconsistently because there's no defined system.

Step 1 — You receive the signed engagement letter. (You)
Step 2 — Claude drafts the welcome email with kickoff workshop details and prep questions. (Claude)
Step 3 — You review and send the welcome email. (Handoff: Claude → you → client)
Step 4 — Claude generates the discovery brief template for this engagement type from the Project knowledge base. (Claude)
Step 5 — You confirm the discovery brief is correct for this specific client and adjust any items that don't apply. (You)
Step 6 — Claude drafts the internal kickoff brief your team needs to start work. (Claude)
Step 7 — You review the brief, add anything Claude couldn't know, and share it with your team. (Handoff)

In this workflow, Claude handles three of the seven steps: the welcome email, the discovery brief, and the internal kickoff brief. You handle the parts that require judgement about the specific client and relationship. The handoffs are explicit: Claude produces something, you review it before it goes anywhere.

**Monthly status workflow**

This is the process of producing a monthly engagement update — for the client steering group, a sponsor, or a partner. The data exists somewhere. The job is to interpret it and communicate it clearly.

Step 1 — You pull the engagement metrics from your delivery tracker, project plan, or whatever system tracks the relevant milestones. (You)
Step 2 — You paste the metrics into Claude with a brief note on anything significant that happened this month. (Handoff: you → Claude)
Step 3 — Claude drafts the narrative status section — what the numbers mean, what changed, what the pattern suggests. (Claude)
Step 4 — You read the draft and check whether the interpretation is accurate. This is the critical check. Claude works from the data you gave it; it can't know if the numbers look weird because of a mis-categorised hour or a slipped milestone. (You)
...
The workflow is efficient because Claude handles the writing and the interpretation logic. You handle the data gathering and the accuracy check — the parts that require knowing what the numbers actually mean in your specific engagement context.

**Content creation workflow**

This is the process of producing regular content — articles, LinkedIn posts, email newsletters — for a consistent publishing schedule. The challenge isn't writing one piece; it's repeating the process without it consuming your week.

Step 1 — You note the topic, the angle, and any timely hook (an industry event, a recent engagement, a seasonal theme). (You)
...
Step 5 — Claude produces the short-form variations: the LinkedIn captions, the email subject line, the newsletter intro. (Claude)
...
A handoff is a point in the workflow where Claude produces something that will affect the real world — an email that will be sent to a client, a status update that will go to a sponsor, a post that will be published. At every one of these points, you need a review step before the output leaves your hands.
...
Your review at the handoff point is the place where you close that gap. It doesn't have to be long. A 30-second read of a client email before you hit send is a handoff review. A 2-minute check of a monthly status narrative to confirm the interpretation matches what actually happened is a handoff review.
...
You're running your monthly status workflow. At step 3, you paste the metrics and ask: "Write a summary of this month's progress." Claude produces a reasonable paragraph. But it's written like a standalone piece — it doesn't connect to anything before or after. It reads like a fresh interpretation, not like a section of an ongoing update that your client sponsor has been reading for four months.

**Better approach:** Include the workflow context in the prompt:

```
I'm preparing our monthly engagement update for April 2026. This
narrative section follows the milestone table the client sponsor Marcus
has already seen. It should interpret progress — not restate it — and
connect to the "next month focus" section that follows.

Workshops delivered: 4 of 4 planned (on track)
Stakeholder interviews: 9 of 12 completed (3 rescheduled into May)
Draft recommendations: 2 of 6 sections circulated to the steering group
...
the rescheduled interviews and connect them to the May workshop schedule.
```

The difference is the framing: "this follows the milestone table" and "connects to the next month focus section" tells Claude what comes before and after. That context changes how it writes the output. It becomes a section of a workflow, not an isolated task.

### Systemising versus automating

Here's a distinction that matters for most people in this course.

Automating a workflow means software does the steps without you. A trigger fires, a tool runs, an output appears — and you weren't involved. Automation requires technical setup: Zapier, Make, API connections, code. For most non-developers running small firms, automation is not accessible without outside help, and it introduces reliability risks when something breaks.
...
For most consultants, systemising is the right goal. It's achievable, sustainable, and it doesn't break when a software integration fails.
