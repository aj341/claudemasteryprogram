...
Most accountants start using Claude the same way. A task appears. They open Claude, type a prompt, get an output, and move on. The task is done.

That's useful. But it's not where the real time savings come from.

The bigger gain comes when you stop treating Claude as a task tool and start treating it as part of a workflow. A workflow is a recurring, multi-step process — something you do weekly, fortnightly, or monthly, with mostly the same inputs and mostly the same required outputs. Every practice has several of these. The question is whether Claude has a defined role in each one, or whether you're improvising every time.

When Claude has no defined role in a workflow, you repeat the same mental overhead every time, {{firstName}}: what context does Claude need here? What format should I ask for? What am I actually asking it to do? That friction adds up. It doesn't feel like much per task, but across a BAS quarter of repeating the same process, it's significant.

When Claude has a defined role, that overhead drops to near zero. You know which steps Claude handles. You know what prompt to use. You know what you get back and what you do with it. The process becomes repeatable, and repeatable processes are the foundation of a practice that doesn't depend entirely on you thinking hard every day.
...
**First: map all the steps.** Write out the full process from start to finish. Don't skip steps because they seem obvious. "Check the engagement letter" is a step. "Notify the client" is a step. Every action is a step.

**Second: decide which steps Claude handles, which you handle, and which are handoffs.** Not every step belongs to Claude. Some require your judgement, your client relationships, or information Claude can't have (the conversation you had at last quarter's review meeting). Some are genuinely faster for you to do yourself. Some steps are transitions — places where Claude produces something and you take it to the next phase, or where you gather information and pass it to Claude.
...
This is the process that runs from "client has signed the engagement letter" to "client is set up in our practice systems and the work can begin." Most accounting firms have this process. Most run it inconsistently because there's no defined system.

Step 1 — You receive the signed engagement letter. (You)
Step 2 — Claude drafts the welcome email with next steps and the kickoff call link. (Claude)
Step 3 — You review and send the welcome email. (Handoff: Claude → you → client)
Step 4 — Claude generates the onboarding checklist for this client type from the Project knowledge base (sole trader / Pty Ltd / SMSF). (Claude)
Step 5 — You confirm the checklist is correct for this specific client and tick off any items that don't apply. (You)
Step 6 — Claude drafts the internal team brief your bookkeeper or junior accountant needs to start work. (Claude)
Step 7 — You review the brief, add anything Claude couldn't know (the verbal context from discovery), and share it with your team. (Handoff)
...
**Quarterly client report workflow**

This is the process of producing a quarterly performance summary — for yourself, a partner, or a client. The data exists somewhere (Xero, the practice management system). The job is to interpret it and communicate it clearly.

Step 1 — You pull the numbers from Xero, the lodgement tracker, or whatever system tracks the relevant metrics. (You)
Step 2 — You paste the numbers into Claude with a brief note on anything significant that happened this quarter. (Handoff: you → Claude)
Step 3 — Claude drafts the narrative report section — what the numbers mean, what changed, what the pattern suggests. (Claude)
Step 4 — You read the draft and check whether the interpretation is accurate. This is the critical check. Claude works from the data you gave it; it can't know if the numbers look weird because of a coding error in the client's ledger. (You)
Step 5 — Claude drafts the action items or "next quarter focus" section based on the narrative. (Claude)
Step 6 — You approve, edit, and send. (Handoff)

The workflow is efficient because Claude handles the writing and the interpretation logic. You handle the data gathering and the accuracy check — the parts that require knowing what the numbers actually mean in this specific client's context.

**Content creation workflow**

This is the process of producing regular content — blog posts, social posts, email newsletters to your client base — for a consistent publishing schedule. The challenge isn't writing one piece; it's repeating the process without it consuming your week.

Step 1 — You note the topic, the angle, and any timely hook (an ATO change, a recent client win, a seasonal theme like end-of-FY). (You)
Step 2 — Claude drafts the long-form piece from your brief. (Claude)
Step 3 — You read the draft and identify what needs fixing: missing AU specifics, wrong tone, anything that doesn't reflect your actual professional view or could be read as advice. (You)
Step 4 — Claude revises based on your feedback. (Claude, after handoff)
Step 5 — Claude produces the short-form variations: the LinkedIn caption, the email subject line, the client newsletter intro. (Claude)
Step 6 — You do a final read on everything before scheduling. (You — this is always the last step)

The key handoff point in this workflow is step 3. You have to read the draft before it moves forward. Skipping that check — sending Claude's output straight to step 5 or straight to publishing — is how you end up with content that's factually accurate but sounds nothing like {{businessName}}, or that makes a tax claim you wouldn't actually make.

### The handoff problem

Handoffs are the most important part of any Claude-assisted workflow, and the part most people underdesign.

A handoff is a point in the workflow where Claude produces something that will affect the real world — an email that will be sent to a client, a report that will go to a partner, a post that will be published. At every one of these points, you need a review step before the output leaves your hands.
...
Your review at the handoff point is the place where you close that gap. It doesn't have to be long. A 30-second read of a client email before you hit send is a handoff review. A 2-minute check of a quarterly report narrative to confirm the interpretation matches what actually happened on the client's file is a handoff review.

The rule is simple: Claude's output goes through you before it goes to anyone else. No exceptions, even when the workflow is running smoothly and Claude's outputs have been good for three months. The TPB doesn't care that "Claude wrote it" — it's still your professional output.

### Common mistake → Better approach

**Mistake:** Using Claude in a workflow without telling it where the step fits.

You're running your quarterly report workflow. At step 3, you paste the numbers and ask: "Write a summary of this quarter's performance." Claude produces a reasonable paragraph. But it's written like a standalone piece — it doesn't connect to anything before or after. It reads like a fresh interpretation, not like a section of an ongoing report that your client has been reading for eight quarters.

**Better approach:** Include the workflow context in the prompt:

```
I'm preparing the quarterly business report for our client Marcus Webb
(Webb Constructions, FY26 Q3). This narrative section follows the metrics
table he has already seen. It should interpret the numbers — not restate
them — and connect to the "next quarter focus" section that follows.

Revenue: $410,200 (Q2: $385,000, up 6.5%)
Job completion rate: 87% (target: 85%, up from 83% in Q2)
Variation revenue: $21,000 (target: $35,000 — second quarter below target)
...
the variation underperformance and connect it to the new pricing
framework starting next quarter.
```

The difference is the framing: "this follows the metrics table" and "connects to the next quarter focus section" tells Claude what comes before and after. That context changes how it writes the output. It becomes a section of a workflow, not an isolated task.

### Systemising versus automating

Here's a distinction that matters for most accountants in this course.

Automating a workflow means software does the steps without you. A trigger fires, a tool runs, an output appears — and you weren't involved. Automation requires technical setup: Zapier, Make, API connections, code. For most non-developers running small practices, automation is not accessible without outside help, and it introduces reliability risks when something breaks at end of FY.
...
For most accountants and bookkeepers, systemising is the right goal. It's achievable, sustainable, and it doesn't break when a software integration fails the day before a quarterly BAS deadline.
