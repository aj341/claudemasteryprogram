...
Most tradies start using Claude the same way. A task pops up. They open Claude on the phone or laptop, type a prompt, get an output, send it, and move on. The task is done.

That's useful. But it's not where the real time savings come from, {{firstName}}.

The bigger gain comes when you stop treating Claude as a task tool and start treating it as part of a workflow. A workflow is a recurring, multi-step process — something you do weekly, fortnightly, or monthly, with mostly the same inputs and mostly the same required outputs. Every trades business has several of these. The question is whether Claude has a defined role in each one, or whether you're improvising every time.
...
**First: map all the steps.** Write out the full process from start to finish. Don't skip steps because they seem obvious. "Check the brief from the head contractor" is a step. "Notify the homeowner" is a step. Every action is a step.

**Second: decide which steps Claude handles, which you handle, and which are handoffs.** Not every step belongs to Claude. Some require your judgement, your relationships, or information Claude can't have — what you saw on site that morning, what the builder said over coffee, what the apprentice mentioned about the meter board. Some are genuinely faster for you to do yourself. Some steps are transitions — places where Claude produces something and you take it to the next phase, or where you gather information and pass it to Claude.
...
**Job onboarding workflow**

This is the process that runs from "homeowner has signed the quote" to "job is set up and the work can begin." Most trades businesses have this process. Most run it inconsistently because there's no defined system.

Step 1 — You receive the signed quote. (You)
Step 2 — Claude drafts the welcome email with next steps and the booked start date. (Claude)
Step 3 — You review and send the welcome email. (Handoff: Claude → you → homeowner)
Step 4 — Claude generates the onboarding checklist for this job type from the Project knowledge base. (Claude)
Step 5 — You confirm the checklist is correct for this specific job and tick off any items that don't apply. (You)
Step 6 — Claude drafts the internal job brief your apprentice or sub-contractor needs to start work. (Claude)
Step 7 — You review the brief, add anything Claude couldn't know, and share it with your team. (Handoff)

In this workflow, Claude handles three of the seven steps: the welcome email, the checklist, and the internal brief. You handle the parts that require judgement about the specific homeowner and relationship. The handoffs are explicit: Claude produces something, you review it before it goes anywhere.

**Monthly reporting workflow**

This is the process of producing a monthly performance summary — for yourself, the missus who does the books, or a head contractor you sub-contract to. The data exists somewhere. The job is to interpret it and communicate it clearly.

Step 1 — You pull the numbers from your accounting software, job-management platform, or whatever system tracks the relevant numbers. (You)
...
This is the process of producing regular content — Facebook posts, Instagram reels, email updates — for a consistent publishing schedule. The challenge isn't writing one piece; it's repeating the process without it consuming your week.

Step 1 — You note the topic, the angle, and any timely hook (a job you just finished, a seasonal theme, a homeowner question that comes up a lot). (You)
...
Step 5 — Claude produces the short-form variations: the social captions, the SMS update, the newsletter intro. (Claude)
...
Handoffs are the most important part of any Claude-assisted workflow, and the part most tradies underdesign.

A handoff is a point in the workflow where Claude produces something that will affect the real world — an email that will be sent to a homeowner, a report that will go to a head contractor, a post that will be published. At every one of these points, you need a review step before the output leaves your hands.
...
Your review at the handoff point is the place where you close that gap. It doesn't have to be long. A 30-second read of a homeowner email before you hit send is a handoff review. A 2-minute check of a monthly report narrative to confirm the interpretation matches what actually happened on site is a handoff review.
...
section follows the metrics table my bookkeeper Marcus has already seen.
...
Job-completion rate within quoted hours: 87% (target: 85%, up from 83%)
Callback rate on completed jobs: 4 callbacks (target: 2 — second month
above target)
...
the callback rate and connect it to the apprentice training plan
starting in May.
```
...
Here's a distinction that matters for most tradies in this course.

Automating a workflow means software does the steps without you. A trigger fires, a tool runs, an output appears — and you weren't involved. Automation requires technical setup: Zapier, Make, API connections, code. For most non-developers running a small trades business, automation is not accessible without outside help, and it introduces reliability risks when something breaks at 7am before the first job.
...
For most tradies, systemising is the right goal. It's achievable, sustainable, and it doesn't break when a software integration fails on the morning you're trying to send a progress claim.
