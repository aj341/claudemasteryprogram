...
**Word documents (.docx)** work well. The text content comes through clearly. Formatting — headers, bold text, tables — is usually preserved or at least interpretable. If you're sharing a wholesale agreement, a supplier proposal, or a long brief, a Word document is a solid choice.
...
**Spreadsheets (.xlsx, .csv)** are read-only. Claude can read the data, analyse it, summarise it, and extract patterns from it. It cannot write back to your spreadsheet or push data anywhere. If you need Claude to process spreadsheet data — a Shopify order export, a Lightspeed sales report, a stocktake sheet — CSV format is the most reliable. Export the relevant sheet as CSV before pasting or uploading.

**Scanned images of documents** are the least reliable input. If you have a scanned supplier invoice or a photographed wholesale agreement, Claude will attempt to read it, but errors increase. Where you can, convert scans to proper text-based formats first. Many scanners and phone apps (like Adobe Scan or Apple's built-in Notes scanner) now offer OCR — optical character recognition — which converts scanned text into machine-readable text. Use that output, not the raw image, when you can.
...
**Upload** when the document is long or when copying and pasting the content would be cumbersome. PDFs, multi-page wholesale agreements, and large CSV files (your monthly Shopify order export) are all candidates for upload. The upload feature is available on Claude.ai (in the browser and the desktop app) — you'll see a paperclip or attachment icon in the input area.
...
Mistake: Pasting a 40-page wholesale distribution agreement as raw text. The formatting collapses, the pricing tables become unreadable rows of data, and you spend five minutes wondering why the output is chaotic.

Better approach: Upload the PDF or Word version of the agreement directly. Claude processes the file, the formatting is preserved, and you get a coherent output. If you only need one section — the termination clause, the payment terms, the exclusivity provisions — copy just that section and paste it. You don't need the whole document if you have a specific target.

### Working with long documents

Here's a limitation that trips retailers up: Claude has a context window. That means there's a ceiling on how much text it can hold in active memory at once. For most everyday documents — wholesale agreements up to 30 or 40 pages, supplier proposals, returns reports — you'll be well within the limit. But for very long documents — a 200-page department-store supplier tender, a multi-year audit report, a compiled customer-service email archive — you may hit the ceiling.
...
```
This is Section 3 of a department-store supplier tender — Methodology and Approach.
It runs from pages 18 to 34. Please do two things:
1. Summarise the methodology in plain language — no more than 150 words.
2. List any specific commitments the supplier (us) would have to make in this
   section — exact promises, not vague claims. Format as a bullet list.
...
This is one of the most practical things Claude does with documents, and most retailers never try it.

Messy documents are everywhere in retail. A supplier invoice that lists line items across a paragraph of prose instead of a table. A wholesale proposal that buries key terms in narrative. A long customer-service email thread where the action items are scattered across fifteen messages. A wholesale agreement where the obligations are mixed in with definitions and whereas clauses.
...
```
Below is an invoice from a homewares supplier. The line items are
...
SKU description | Quantity | Unit price (AUD) | Total (AUD) | GST included (Y/N)
...
The instruction "do not estimate or calculate — only include what is explicitly written" is doing real work here. Without it, Claude may fill gaps with plausible-looking numbers. That's the wrong behaviour when you're checking financial data against your purchase order.

**Key-field extraction.** Use this when the document has specific terms, dates, parties, or obligations you need to pull out — but the document isn't structured around rows. Wholesale agreements are the classic case.

```
Below is a wholesale stockist agreement. I need to extract the following specific fields.
...
- Agreement end date or notice period for termination
- Payment terms (when payment is due after invoice — net days)
- Late payment provisions (interest, penalties, freezing of further orders)
- Exclusivity clause (whether stockist is exclusive in their region or category)
- Minimum reorder quantity or value (per order or per period)
- Returns policy for damaged or unsold stock
...
**Reading a wholesale agreement before you sign.** You don't need a solicitor to do a first pass. Paste the agreement and ask Claude to extract the key commercial terms (payment, exclusivity, termination, returns) using the key-field extraction pattern above. Then identify anything that looks unusual or missing relative to what you agreed in your buyer call. Take that list to a solicitor for the final review. You show up to that conversation better prepared, which saves time and fees.

**Extracting line items from a supplier invoice.** When a supplier invoice arrives as a PDF or pasted text, use the table extraction prompt to pull it into a structured format. Compare it against your purchase order. Flag discrepancies. This is faster than reading line by line, and you're less likely to miss a duplicate charge or a quantity error before the freight ships.

**Summarising a long industry report.** Your industry association releases a 60-page AU retail outlook report. You have 10 minutes. Paste the executive summary and the conclusions section into Claude and ask it to produce: (1) a three-sentence overview, (2) the five most relevant findings for a small Shopify and bricks-and-mortar retailer of your type and size, (3) any actions or recommendations aimed at retailers like yours. You get what you need without reading the whole document.

**Reviewing a freight or 3PL proposal.** A new freight carrier sends you a 25-page proposal. You need to understand what they're offering, what they've committed to, and what's vague. Use the section-by-section pass approach from earlier. Or, if it's short enough, upload the whole thing and ask Claude to: (1) summarise what's included in scope, (2) list explicit commitments (transit times, damage guarantees, pickup windows), (3) list anything described vaguely that you'd need to clarify before signing.

**Processing a long customer-service email thread.** Copy and paste the whole email thread — including headers, dates, and names — and ask Claude to produce: a timeline of decisions made, a list of open action items with the person responsible for each, and any commitments one party made to the other. This turns a 30-message customer thread into a usable handover document in about 30 seconds.
...
Never submit an extraction to a wholesale stockist, forward it to your bookkeeper as a source of truth, or make a financial decision based on it without checking a sample first. Claude extracted it. You verified it. That's the workflow at {{businessName}}.
