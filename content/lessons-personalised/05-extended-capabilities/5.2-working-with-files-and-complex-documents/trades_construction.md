...
**Word documents (.docx)** work well. The text content comes through clearly. Formatting — headers, bold text, tables — is usually preserved or at least interpretable. If you're sharing a head-contractor scope, a supplier proposal, or a long brief from a builder, a Word document is a solid choice.

**PDFs** are common and mostly workable, with one important caveat: it depends entirely on how the PDF was made. A PDF created by exporting from Word or simPRO contains machine-readable text. Claude reads it directly. A PDF created by scanning a physical page is an image — it contains no text, only pixels. Claude can attempt to read scanned PDFs, but the accuracy drops significantly and can fail entirely on poor scans or handwritten material.

**Spreadsheets (.xlsx, .csv)** are read-only. Claude can read the data, analyse it, summarise it, and extract patterns from it. It cannot write back to your spreadsheet or push data anywhere. If you need Claude to process spreadsheet data — a job-margin export from Tradify, a debtors aging from Xero — CSV format is the most reliable. Export the relevant sheet as CSV before pasting or uploading.

**Scanned images of documents** are the least reliable input. If you have a scanned compliance certificate or a photographed variation note from a builder, Claude will attempt to read it, but errors increase. Where you can, convert scans to proper text-based formats first. Many scanners and phone apps (like Adobe Scan or Apple's built-in Notes scanner) now offer OCR — optical character recognition — which converts scanned text into machine-readable text. Use that output, not the raw image, when you can.
...
Mistake: Pasting a 40-page head-contractor agreement as raw text. The formatting collapses, tables become unreadable rows of data, and you spend five minutes wondering why the output is chaotic.

Better approach: Upload the PDF or Word version of the contract directly. Claude processes the file, the formatting is preserved, and you get a coherent output. If you only need one section — the variation clause, the payment terms, the back-charges provisions — copy just that section and paste it. You don't need the whole document if you have a specific target.

### Working with long documents

Here's a limitation that trips people up: Claude has a context window. That means there's a ceiling on how much text it can hold in active memory at once. For most everyday documents — agreements up to 30 or 40 pages, reports, supplier proposals — you'll be well within the limit. But for very long documents — a 200-page government tender for a council infrastructure job, a multi-year audit report, a compiled email archive on a defects dispute — you may hit the ceiling.
...
```
This is Section 3 of a head-contractor tender response — Methodology and Approach.
It runs from pages 18 to 34. Please do two things:
1. Summarise the methodology in plain language — no more than 150 words.
2. List any specific commitments the head contractor makes in this section — exact
...
This is one of the most practical things Claude does with documents, and most tradies never try it.

Messy documents are everywhere in a trades business. A wholesaler invoice that lists line items across a paragraph of prose instead of a table. A subcontractor proposal that buries the back-charge terms in narrative. A long email thread where the variation agreements are scattered across fifteen messages. A head-contractor agreement where the obligations are mixed in with definitions and whereas clauses.
...
```
Below is an invoice from a building supplies wholesaler. The line items are
...
The instruction "do not estimate or calculate — only include what is explicitly written" is doing real work here. Without it, Claude may fill gaps with plausible-looking numbers. That's the wrong behaviour when you're checking a wholesaler's invoice against the delivery docket.

**Key-field extraction.** Use this when the document has specific terms, dates, parties, or obligations you need to pull out — but the document isn't structured around rows. Head-contractor agreements are the classic case.

```
Below is a sub-contract agreement from a head builder. I need to extract the following specific fields.
...
- Payment terms (when payment is due after invoice or progress claim)
- Late payment provisions (interest, penalties)
- Variation clause (how variations are agreed and priced)
...
Asking Claude to quote the relevant text exactly — rather than paraphrase it — is a critical constraint. Paraphrases can drift. An exact quote gives you something you can verify against the original and show to a solicitor or your industry association if needed.

### Real business use cases

These are the scenarios where this lesson pays off most directly.

**Reading a head-contractor agreement before you sign.** You don't need a solicitor to do a first pass. Paste the agreement and ask Claude to extract the key commercial terms (payment, variations, defects, retention, back-charges) using the key-field extraction pattern above. Then identify anything that looks unusual or missing relative to what you agreed verbally on site. Take that list to a solicitor for the final review. You show up to that conversation better prepared, which saves time and fees.

**Extracting line items from a wholesaler invoice.** When a Reece or Tradelink invoice arrives as a PDF or pasted text, use the table extraction prompt to pull it into a structured format. Compare it against your purchase order or delivery docket. Flag discrepancies. This is faster than reading line by line, and you're less likely to miss a duplicate charge or a quantity error before you pay it.

**Summarising a long report.** Master Builders or your industry association releases a 60-page sector outlook report. You have 10 minutes. Paste the executive summary and the conclusions section into Claude and ask it to produce: (1) a three-sentence overview, (2) the five most relevant findings for a business of your trade and size, (3) any actions or recommendations aimed at businesses like yours. You get what you need without reading the whole document.

**Reviewing a sub-contractor proposal.** A roof plumber sends you a 25-page quote and methodology for a job your business is head-contracting. You need to understand what they're offering, what they've committed to, and what's vague. Use the section-by-section pass approach from earlier. Or, if it's short enough, upload the whole thing and ask Claude to: (1) summarise what's included in scope, (2) list explicit commitments, (3) list anything described vaguely that you'd need to clarify before signing them up.

**Processing a long email thread.** Copy and paste the whole email thread — including headers, dates, and names — and ask Claude to produce: a timeline of decisions made, a list of open action items with the person responsible for each, and any commitments one party made to another. This turns a 30-message thread on a defects dispute into a usable document in about 30 seconds.
...
Never submit an extraction to a builder, forward it to your bookkeeper as a source of truth, or make a financial decision based on it without checking a sample first. Claude extracted it. You verified it. That's the workflow.
