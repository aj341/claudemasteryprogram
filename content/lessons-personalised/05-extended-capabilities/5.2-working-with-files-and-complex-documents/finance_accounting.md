...
Claude can work with a range of file types — but they are not all equal. Understanding the hierarchy saves you a lot of frustration in a busy practice.
...
**Word documents (.docx)** work well. The text content comes through clearly. Formatting — headers, bold text, tables — is usually preserved or at least interpretable. If you're sharing an engagement letter, a fee proposal, or a long client brief, a Word document is a solid choice.
...
**Spreadsheets (.xlsx, .csv)** are read-only. Claude can read the data, analyse it, summarise it, and extract patterns from it. It cannot write back to your spreadsheet or push data into Xero. If you need Claude to process spreadsheet data — say, a client's expense list or a debtor aging report — CSV format is the most reliable. Export the relevant sheet as CSV before pasting or uploading.

**Scanned images of documents** are the least reliable input. If you have a scanned ATO letter or a photographed receipt, Claude will attempt to read it, but errors increase. Where you can, convert scans to proper text-based formats first. Many scanners and phone apps (like Adobe Scan or Apple's built-in Notes scanner) now offer OCR — optical character recognition — which converts scanned text into machine-readable text. Use that output, not the raw image, when you can.
...
**Upload** when the document is long or when copying and pasting the content would be cumbersome. PDFs, multi-page Word documents, and large CSV files (a year's worth of bank transactions) are all candidates for upload. The upload feature is available on Claude.ai (in the browser and the desktop app) — you'll see a paperclip or attachment icon in the input area.
...
Mistake: Pasting a 40-page commercial lease as raw text. The formatting collapses, tables become unreadable rows of data, and you spend five minutes wondering why the output is chaotic.

Better approach: Upload the PDF or Word version of the lease directly. Claude processes the file, the formatting is preserved, and you get a coherent output. If you only need one section — the termination clause, the rent review provisions, the indemnity clauses — copy just that section and paste it. You don't need the whole document if you have a specific target.

### Working with long documents

Here's a limitation that trips accountants up, {{firstName}}: Claude has a context window. That means there's a ceiling on how much text it can hold in active memory at once. For most everyday documents — engagement letters up to 30 or 40 pages, fee proposals, client memos — you'll be well within the limit. But for very long documents — a 200-page ATO ruling, a multi-year audit report, a compiled email archive on a complex client matter — you may hit the ceiling.

When that happens, you have two practical options.

**Option 1: Reduce the input.** Extract only the sections you need. You don't need to give Claude a 200-page tax ruling if your question is about the eligibility tests in Section 5 and the worked examples in Appendix B. Copy those sections and work with them directly.
...
```
This is Section 3 of an ATO ruling — Application of the Provisions.
It runs from pages 18 to 34. Please do two things:
1. Summarise the application in plain language — no more than 150 words.
2. List any specific tests the ATO sets out in this section — exact
   eligibility criteria, not vague principles. Format as a bullet list.
...
This is one of the most practical things Claude does with documents, and most accountants never try it.

Messy documents are everywhere in practice life. An invoice that lists line items across a paragraph of prose instead of a table. A fee proposal that buries key terms in narrative. A long email thread where the action items are scattered across fifteen messages with the client. A contract where the obligations are mixed in with definitions and whereas clauses.
...
```
Below is an invoice from a building supplies company that one of our
hospitality clients has sent through for FY26 expense capture. The line
items are described in running prose, not a table. Please extract every
...
The instruction "do not estimate or calculate — only include what is explicitly written" is doing real work here. Without it, Claude may fill gaps with plausible-looking numbers. That's the wrong behaviour when you're checking GST coding for a client.

**Key-field extraction.** Use this when the document has specific terms, dates, parties, or obligations you need to pull out — but the document isn't structured around rows. Engagement letters and commercial leases are the classic cases.

```
Below is a services agreement between a client and an external IT vendor.
I need to extract the following specific fields. For each field, quote the
relevant text from the document exactly, then state the page or section
reference where it appears. If a field is not addressed in the agreement,
write "not found."
...
Asking Claude to quote the relevant text exactly — rather than paraphrase it — is a critical constraint. Paraphrases can drift. An exact quote gives you something you can verify against the original and pass on to the firm's lawyer if the client wants the agreement reviewed.

### Real business use cases

These are the scenarios at {{businessName}} where this lesson pays off most directly.

**Reading a client's commercial lease before they sign.** You don't need to be a property solicitor to do a first pass. Paste the lease and ask Claude to extract the key commercial terms (rent, term, options, outgoings, IP) using the key-field extraction pattern above. Then identify anything that looks unusual or missing relative to what the client described. Take that list to the firm's lawyer for the final review. The client shows up to that conversation better prepared, which saves time and legal fees.

**Extracting line items from a supplier invoice.** When a client's supplier invoice arrives as a PDF or pasted text, use the table extraction prompt to pull it into a structured format. Compare it against the purchase order or the budget. Flag discrepancies. This is faster than reading line by line, and you're less likely to miss a duplicate charge or a GST coding error.

**Summarising a long ATO publication.** The ATO releases a 60-page draft taxation determination. You have 10 minutes before a partner meeting. Paste the executive summary and the practical implications section into Claude and ask it to produce: (1) a three-sentence overview, (2) the five most relevant findings for a small AU practice handling small-business clients, (3) any actions or transitional arrangements aimed at clients in your client base. You get what you need without reading the whole document.

**Reviewing a fee proposal or engagement letter from another firm.** A new client sends you the engagement letter from their previous accountant for context. You need to understand what was in scope, what they've committed to, and what's vague. Use the section-by-section pass approach from earlier. Or, if it's short enough, upload the whole thing and ask Claude to: (1) summarise what was included in scope, (2) list explicit fee commitments, (3) list anything described vaguely that you'd want to clarify in your own engagement letter.

**Processing a long client email thread.** Copy and paste the whole email thread — including headers, dates, and names — and ask Claude to produce: a timeline of decisions made, a list of open action items with the person responsible for each, and any commitments one party made to another. This turns a 30-message thread into a usable file note in about 30 seconds.
...
Never submit an extraction to a client, forward it as a source of truth to the firm's lawyer, or use it as the basis for a working paper without checking a sample first. Claude extracted it. You verified it. That's the workflow.
