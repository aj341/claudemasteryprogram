...
**Word documents (.docx)** work well. The text content comes through clearly. Formatting — headers, bold text, tables — is usually preserved or at least interpretable. If you're sharing a contract, a vendor proposal, or a long brief, a Word document is a solid choice.

**PDFs** are common and mostly workable, with one important caveat: it depends entirely on how the PDF was made. A PDF created by exporting from Notion or Google Docs contains machine-readable text. Claude reads it directly. A PDF created by scanning a physical page is an image — it contains no text, only pixels. Claude can attempt to read scanned PDFs, but the accuracy drops significantly and can fail entirely on poor scans or handwritten material.

**Spreadsheets (.xlsx, .csv)** are read-only. Claude can read the data, analyse it, summarise it, and extract patterns from it. It cannot write back to your spreadsheet or push data anywhere. If you need Claude to process spreadsheet data — a Stripe export, a customer-success log, a churn-survey CSV — CSV format is the most reliable. Export the relevant sheet as CSV before pasting or uploading.
...
**Upload** when the document is long or when copying and pasting the content would be cumbersome. PDFs, multi-page Word documents, and large CSV files (a 12-month Stripe payouts export, for example) are all candidates for upload. The upload feature is available on Claude.ai (in the browser and the desktop app) — you'll see a paperclip or attachment icon in the input area.
...
Mistake: Pasting a 40-page MSA as raw text. The formatting collapses, tables become unreadable rows of data, and you spend five minutes wondering why the output is chaotic.
...
Here's a limitation that trips founders up: Claude has a context window. That means there's a ceiling on how much text it can hold in active memory at once. For most everyday documents — contracts up to 30 or 40 pages, reports, proposals — you'll be well within the limit. But for very long documents — a 200-page enterprise tender, a multi-year SOC 2 evidence pack, a compiled support-thread archive — you may hit the ceiling.
...
```
This is Section 3 of a vendor security tender — Methodology and Approach.
It runs from pages 18 to 34. Please do two things:
1. Summarise the methodology in plain language — no more than 150 words.
2. List any specific commitments the vendor makes in this section — exact
...
This is one of the most practical things Claude does with documents, and most founders never try it.

Messy documents are everywhere in a small SaaS. An invoice that lists line items across a paragraph of prose instead of a table. A vendor proposal that buries key terms in narrative. A long Slack export where the action items are scattered across fifteen messages. A contract where the obligations are mixed in with definitions and whereas clauses.
...
**Table extraction.** Use this when the document contains records — line items, rows, transactions — that should be in a table but aren't. Tell Claude exactly which columns you want.

```
Below is an invoice from a cloud-services vendor. The line items are
...
The instruction "do not estimate or calculate — only include what is explicitly written" is doing real work here. Without it, Claude may fill gaps with plausible-looking numbers. That's the wrong behaviour when you're reconciling vendor charges to your books.
...
```
Below is a master services agreement. I need to extract the following
specific fields. For each field, quote the relevant text from the document
exactly, then state the page or section reference where it appears. If a
field is not addressed in the agreement, write "not found."
...
- Intellectual property ownership clause (who owns work created under this
  agreement)
- Limitation of liability cap (the dollar amount or formula)
- Governing law (which state or jurisdiction applies)

[Agreement pasted below]
```

Asking Claude to quote the relevant text exactly — rather than paraphrase it — is a critical constraint. Paraphrases can drift. An exact quote gives you something you can verify against the original and show to a lawyer if needed.

### Real business use cases

These are the scenarios where this lesson pays off most directly at {{businessName}}.

**Reading a vendor contract before you sign.** You don't need a lawyer to do a first pass. Paste the contract and ask Claude to extract the key commercial terms (payment, termination, liability, IP) using the key-field extraction pattern above. Then identify anything that looks unusual or missing relative to what you agreed verbally on the call. Take that list to a lawyer for the final review. You show up to that conversation better prepared, which saves time and fees.

**Extracting line items from a vendor invoice.** When a cloud-services or SaaS-tooling invoice arrives as a PDF or pasted text, use the table extraction prompt to pull it into a structured format. Compare it against your subscription expectations. Flag discrepancies. This is faster than reading line by line, and you're less likely to miss a duplicate charge or a seat-count error.

**Summarising a long report.** A category research firm releases a 60-page B2B SaaS benchmarks report. You have 10 minutes. Paste the executive summary and the conclusions section into Claude and ask it to produce: (1) a three-sentence overview, (2) the five most relevant findings for a SaaS of your type and size, (3) any recommendations aimed at vendors like you. You get what you need without reading the whole document.

**Reviewing a tender or RFP response.** A prospect sends you a 25-page RFP. You need to understand what they're asking, where they've left things vague, and where you'll need legal review. Use the section-by-section pass approach from earlier. Or, if it's short enough, upload the whole thing and ask Claude to: (1) summarise the in-scope requirements, (2) list explicit must-haves, (3) list anything described vaguely that you'd need to clarify before responding.

**Processing a long email or Slack thread.** Copy and paste the whole thread — including headers, dates, and names — and ask Claude to produce: a timeline of decisions made, a list of open action items with the person responsible for each, and any commitments one party made to another. This turns a 30-message thread into a usable document in about 30 seconds.

### Accuracy is your responsibility

One more thing before the worked examples, {{firstName}}.
...
Never submit an extraction to a customer, forward it to a teammate as a source of truth, or make a financial decision based on it without checking a sample first. Claude extracted it. You verified it. That's the workflow.
