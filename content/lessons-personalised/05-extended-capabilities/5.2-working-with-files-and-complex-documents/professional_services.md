...
**Word documents (.docx)** work well. The text content comes through clearly. Formatting — headers, bold text, tables — is usually preserved or at least interpretable. If you're sharing an engagement letter, a subcontractor proposal, or a long discovery brief, a Word document is a solid choice.
...
```
This is Section 3 of a subcontractor tender document — Methodology and Approach.
It runs from pages 18 to 34. Please do two things:
1. Summarise the methodology in plain language — no more than 150 words.
2. List any specific commitments the subcontractor makes in this section — exact
...
This is one of the most practical things Claude does with documents, and most consultants never try it.

Messy documents are everywhere in client work. An invoice that lists line items across a paragraph of prose instead of a table. A subcontractor proposal that buries key terms in narrative. A long email thread where the action items are scattered across fifteen messages. A contract where the obligations are mixed in with definitions and whereas clauses.
...
**Table extraction.** Use this when the document contains records — items, rows, line entries — that should be in a table but aren't. Tell Claude exactly which columns you want.

```
Below is an invoice from a research subcontractor. The line items are
...
**Key-field extraction.** Use this when the document has specific terms, dates, parties, or obligations you need to pull out — but the document isn't structured around rows. Contracts and engagement letters are the classic case.
...
**Reading a subcontractor agreement before you sign.** You don't need a solicitor to do a first pass. Paste the agreement and ask Claude to extract the key commercial terms (payment, termination, liability, IP) using the key-field extraction pattern above. Then identify anything that looks unusual or missing relative to what you agreed verbally. Take that list to a solicitor for the final review. You show up to that conversation better prepared, which saves time and fees.

**Extracting line items from an invoice.** When a subcontractor invoice arrives as a PDF or pasted text, use the table extraction prompt to pull it into a structured format. Compare it against your purchase order or scope. Flag discrepancies. This is faster than reading line by line, and you're less likely to miss a duplicate charge or a quantity error.

**Summarising a long report.** Your industry association releases a 60-page industry outlook report. You have 10 minutes. Paste the executive summary and the conclusions section into Claude and ask it to produce: (1) a three-sentence overview, (2) the five most relevant findings for a firm of your type and size, (3) any actions or recommendations aimed at firms like yours. You get what you need without reading the whole document.
...
Never submit an extraction to a client, forward it to an associate as a source of truth, or make a financial decision based on it without checking a sample first. Claude extracted it. You verified it. That's the workflow.
