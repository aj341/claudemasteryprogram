...
Here is what that covers in practice for a trades or building business.

**Photos.** A photo of a damaged switchboard, a leaking trap under a kitchen sink, a competitor's signage on a job site, or anything you've snapped on the phone before clambering back into the ute. Claude can describe what's in the frame, identify text, assess layout, and respond to questions about what it sees.

**Screenshots.** A screenshot of a ServiceM8 job card, a simPRO dashboard, a Tradify timesheet, a supplier's online quote, or a homeowner's text message with a photo attached. Claude can extract text, comment on what's shown, summarise the data on screen, or compare what you've screenshotted against another job you describe.

**Scanned documents.** A scanned compliance certificate, a handwritten variation note from a builder, a signed SWMS, a delivery docket, an invoice from a wholesaler. Claude can read the text, pull out key figures, and summarise what the document says — as long as the scan is clear enough to read.

**PDFs.** Many chat interfaces let you upload a PDF directly. Claude reads it as a document and can answer questions about it, extract key information, or summarise sections — useful for builders' contracts, AS/NZS standards excerpts, and supplier quotes.

**Handwritten notes.** Claude can read reasonably clear handwriting. Not always perfectly — more on that shortly — but well enough to be useful for site notes, sketched layouts, or a builder's redlines on a plan.
...
These are the use cases that come up regularly for AU/NZ trades businesses.

**Reading invoices and delivery dockets.** You've got a stack of scanned wholesaler invoices from the last fortnight. Instead of typing each one out for the bookkeeper, {{firstName}} uploads the image and asks Claude to extract the supplier, date, and total. It does this quickly and accurately for clear scans.

**Analysing a competitor's ad or social post.** You screenshotted a competing sparkie's Facebook post or a builder's Instagram reel. You want to understand their offer, their hook, their pricing pitch. You upload the screenshot and ask Claude to break it down. Faster and more structured than describing it in text.

**Reviewing a scanned document.** A homeowner sends a scanned letter from strata or a copy of their previous compliance certificate. {{businessName}} uploads it and asks Claude to summarise the key terms, flag anything unusual, or pull out specific figures. You still need to review the original — Claude is giving you a first pass, not a legal opinion — but that first pass saves time.

**Checking a flyer or quote layout.** You've got a draft letterbox flyer, a magnetic ute sticker design, or a one-page capability statement saved as an image. You ask Claude to comment on the layout, check whether the phone number and core trades are clear, or flag anything that looks off. Claude won't replace a designer, but it will catch obvious problems before the print run.

**Extracting text from an image.** You have a photo of a whiteboard from a Friday afternoon planning session where you mapped out the next fortnight's jobs and apprentice rotation. Claude reads the whiteboard and returns the text in a structured format.

**Summarising a dashboard or report screenshot.** You have a screenshot of a simPRO job-margin report, a Xero P&L for the quarter, or a debtors-days summary. You ask Claude what the data shows and it gives you a plain-language summary. Useful when you want the headline before the bookkeeper meeting.

These are not edge cases. These are tasks that come up weekly in most trades businesses.

### What vision is NOT good for

Claude's vision is useful but it is not infallible. You need to know where it struggles.

**Blurry or low-resolution images.** If you can barely read the text in the image yourself, Claude will struggle too. A photo taken under a house in poor torchlight, a screenshot at too low a resolution, or a heavily compressed image will produce guesses rather than accurate readings. Rubbish in, rubbish out.

**Complex diagrams with fine detail.** A council redline on a set of plans, a detailed switchboard schematic, a dense AS/NZS standards table — Claude can describe the overall structure but it will miss fine detail and sometimes make errors on exact figures, dimensions, or labels.

**Tasks requiring perfect OCR accuracy.** If you need every word exactly right — for a compliance certificate, a head-contractor variation, a progress claim, or a financial record — do not rely solely on Claude. It can miss words, misread numbers, or skip lines. Use it as a first pass, then verify the original. Never use Claude's text extraction as your official record for anything that goes near a regulator or a courtroom.

**Identifying specific people.** Claude will not identify named individuals from photos of faces. This is a deliberate design decision. If you need face recognition on a site sign-in photo, that is a different tool.

**Reading very small text.** Footnotes on a plan, fine print on a special condition, small captions on a manufacturer's spec sheet — Claude often misses these or reads them incorrectly.

The honest framing: Claude's vision is a capable general assistant, not a precision instrument. For most trades tasks, that is enough. For tasks where errors have real consequences — compliance certificates, progress claims, gas or electrical safety figures — you verify.
...
**In a Project.** If you are working in a Claude Project that already has documents or context files attached — say, a Project for a particular builder you do regular sub-contract work for — you can upload an image the same way via the attachment button. The image is treated as part of that conversation's context. Useful when you're doing repeated visual work on related images: a series of defect photos for one site, a set of supplier invoices for one job, or a batch of progress photos for a homeowner update.

**Via file upload on mobile.** The Claude mobile app supports image uploads from your camera roll or files. Tap the attachment icon, choose the image or document, and send. Useful when you've just walked off a site and want to analyse a photo on the spot before you drive to the next job.

One practical note: if you are working on a document-heavy task — like reviewing several defect photos or comparing three supplier quotes — upload one at a time and give Claude a separate prompt for each. Asking it to process many images at once and compare them is harder to do well.
...
Tell Claude what the image is, where it came from, and any context that helps it read the image correctly. This is not because Claude cannot see the image — it can. It is because context changes interpretation. A screenshot of a competitor's ad means something different than a screenshot of your own ad that underperformed. Telling Claude which it is shapes how it responds.
...
**Effective prompt:**
```
I've uploaded a screenshot of a Facebook ad from a competing sparkie — a 6-person electrical mob about 15 minutes from us in outer Brisbane, going after the same residential switchboard upgrade work.
...
Keep the response to dot points — I want to share it with my office manager.
```
...
That structure — describe what you're sending, then state what you need — applies to every vision task. A wholesaler invoice: "This is a Reece invoice from our last bulk PEX order. Extract the vendor name, date, line items, and GST total." A scanned letter: "This is a letter from a strata manager about a defect she wants us to look at. Summarise the key dates and any requests she's making." A flyer: "This is a draft of our new letterbox flyer for the autumn switchboard upgrade campaign. Tell me if the hierarchy is clear — can a homeowner find the offer and the phone number quickly, or does the layout make it hard?"

### Common mistake → better approach

**Mistake:** Uploading a screenshot of a simPRO job-margin report and typing "Can you summarise this data?"

Claude returns a general description: "The dashboard shows several metrics including labour hours, materials, and gross margin. It appears to be from a trades job-management platform." Technically accurate but completely useless.

**Better approach:**

```
I've uploaded a screenshot of our simPRO job-margin report for March. We're a 4-person residential electrical business in outer Brisbane — switchboard upgrades, rewires, and small commercial maintenance. Most jobs sit between $1,500 and $8,000.

I need you to:
1. Pull out the three or four headline numbers visible in the screenshot (gross margin %, labour hours, materials cost, or whatever is shown)
2. Identify any metric that looks significantly different from what you'd expect — unusually high or low
3. Write a two-sentence plain-English summary I can send to my old man, who's the silent partner

Note: if any numbers are unclear or you're unsure whether you've read them correctly, flag that explicitly.
```

The improved prompt does four things well: it gives business context (residential sparkie, outer Brisbane, job-size range), asks for specific outputs rather than a general summary, sets the format (two sentences), and — critically — asks Claude to flag uncertainty rather than guess silently. That last instruction is important. Claude will sometimes read a number wrong in an image. Asking it to flag uncertainty makes errors visible rather than invisible.
