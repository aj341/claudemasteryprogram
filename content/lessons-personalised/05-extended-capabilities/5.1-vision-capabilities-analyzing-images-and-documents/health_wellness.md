...
Claude is not just a text model — and {{firstName}} can put that to work today. You can hand it an image — a photo, a screenshot, a scanned document, a PDF — and it will read, interpret, and respond to what it sees.
...
**Reading receipts and invoices.** {{businessName}} has a stack of scanned receipts from a clinical-supplies trip. Instead of typing out each one, you upload the image and ask Claude to extract the vendor, date, and total. It does this quickly and accurately for clear scans.
...
**Effective prompt:**
```
I've uploaded a screenshot of a Facebook ad from a local competitor — they're another physiotherapy clinic two blocks from {{businessName}} targeting the same desk-bound office workers we do.
...
4. One thing they're doing well and one thing I could do differently in our own ads

Keep the response to dot points — I want to share it with my practice partner.
```
...
That structure — describe what you're sending, then state what you need — applies to every vision task. A receipt: "This is a supplier receipt from our last clinical-consumables order. Extract the vendor name, date, line items, and GST total." A scanned letter: "This is a letter from our landlord about the clinic-rooms lease renewal. Summarise the key dates and any requests they are making." A design file: "This is a draft of our new fee schedule and intake handout for {{businessName}}. Tell me if the hierarchy is clear — can a new client find the gap-fee figures quickly, or does the layout make it hard?"
...
```
I've uploaded a screenshot of {{businessName}}'s Cliniko dashboard for March. This is for our 3-clinician physiotherapy practice. We see clients across the metro area but most come from a 5km radius around the clinic.

I need you to:
1. Pull out the three or four headline numbers visible in the screenshot (new bookings, attendance rate, no-show count, or whatever is shown)
2. Identify any metric that looks significantly different from what you'd expect — unusually high or low
3. Write a two-sentence plain-English summary I can paste into our monthly practice review
