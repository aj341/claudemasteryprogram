...
Here is what that covers in practice for an accounting or bookkeeping practice:

**Photos.** A photo of a receipt, a whiteboard from a planning meeting, a competitor firm's shopfront, or anything you've taken on your phone. Claude can describe what's in the frame, identify text, assess layout, and respond to questions about what it sees.

**Screenshots.** A screenshot of a Xero report, a LinkedIn post, a website, a working paper, or a dashboard. Claude can extract text, comment on layout, summarise the data it sees, or compare what you've screenshotted to something you describe.

**Scanned documents.** A scanned ATO letter, a handwritten file note, a client's lease page, a signed engagement letter. Claude can read the text, pull out key figures, and summarise what the document says — as long as the scan is clear enough to read.
...
**Handwritten notes.** Claude can read reasonably clear handwriting. Not always perfectly — more on that shortly — but well enough to be useful for client meeting notes, working-paper margin notes, or annotated documents.
...
These are the use cases that come up regularly for accountants, bookkeepers, and financial planners.

**Reading receipts and supplier invoices.** A client has sent you a stack of scanned receipts for their FY return. Instead of typing out each one, you upload the image and ask Claude to extract the vendor, date, GST amount, and total. It does this quickly and accurately for clear scans — you still verify the numbers before they hit the working paper, but the manual transcription cost goes away.

**Analysing a competitor's ad or post.** You screenshotted another firm's LinkedIn post promoting their FY-planning service. You want to understand their offer, their hook, their call to action. You upload the screenshot and ask Claude to break it down. This is faster and more structured than trying to describe it in text.

**Reviewing a scanned client document.** A client sends a scanned commercial lease or shareholder agreement. You upload it and ask Claude to summarise the key terms, flag anything unusual, or pull out specific figures. You still need to review the original — Claude is giving you a first pass, not a legal opinion — but that first pass saves time and helps you frame the conversation with the firm's lawyer.

**Checking a fee proposal or client letter layout.** You've got a draft fee proposal, a year-end client newsletter, or an engagement letter saved as a PDF. You ask Claude to comment on the layout, check whether the key terms are clear, or flag anything that looks off. Claude won't replace a designer, but it will catch obvious problems before you send it to a client.

**Extracting text from an image.** You have a photo of the whiteboard from a partner-board planning session. You want the text without transcribing it yourself. Claude reads the whiteboard and returns the text in a structured format.

**Summarising a Xero or report screenshot.** You have a screenshot of a Xero P&L, a client's debtor aging report, or a survey result. You ask Claude what the data shows and it gives you a plain-language summary. Useful when you want to draft a quick client comment without re-typing the numbers yourself.

These are not edge cases. These are the kinds of tasks that come up weekly in most AU practices.

### What vision is NOT good for

Claude's vision is useful but it is not infallible. You need to know where it struggles — especially when accuracy in numbers matters.
...
**Complex tables or working papers with fine detail.** A 20-row reconciliation, a detailed client schedule, a dense tax computation with small column headers — Claude can describe the overall structure but it will miss fine detail and sometimes make errors on exact figures, dates, or labels. Always verify any number Claude reads from a complex working paper before you act on it.

**Tasks requiring perfect OCR accuracy.** If you need every word exactly right — for a tax return, an engagement letter, a client deed — do not rely solely on Claude. It can miss words, misread numbers, or skip lines. Use it as a first pass, then verify the original. Never use Claude's text extraction as your official record on a lodgement or working paper.
...
**Reading very small text.** Footnotes on a deed, fine print in a lease, small captions at the edge of an ATO bulletin — Claude often misses these or reads them incorrectly.

The honest framing: Claude's vision is a capable general assistant, not a precision instrument. For most small-practice tasks, that is enough. For tasks where errors have real consequences (anything heading to a client file or the ATO), you verify.
...
**In a Project.** If you are working in a Claude Project that already has documents or context files attached (your fee schedule, your client tone guide), you can upload an image the same way — via the attachment button in the conversation. The image is treated as part of that conversation's context. This is useful if you are doing repeated visual work on related images — a series of supplier receipts, a set of working paper screenshots, a batch of competitor LinkedIn posts.

**Via file upload on mobile.** The Claude mobile app supports image uploads from your camera roll or files. Tap the attachment icon, choose the image or document, and send. Useful when you are between client meetings and have just taken a photo of something you want analysed.

One practical note: if you are working on a document-heavy task — like analysing multiple receipts or comparing several engagement letter drafts — consider uploading one at a time and giving Claude a separate prompt for each. Asking it to process many images at once and compare them is harder to do well.

### Prompting with images: the two-part structure

This is the most important part of the lesson, {{firstName}}.

Most accountants send an image and type something like: "What do you see?" or "What do you think of this?" These prompts produce vague, general responses. Claude describes what it sees. That is rarely what you actually need.

An effective vision prompt has two parts:

**Part one: describe what you are sending.**
Tell Claude what the image is, where it came from, and any context that helps it read the image correctly. This is not because Claude cannot see the image — it can. It is because context changes interpretation. A screenshot of a fee proposal from a competitor firm means something different than a screenshot of your own proposal that didn't convert. Telling Claude which it is shapes how it responds.
...
**Effective prompt:**
```
I've uploaded a screenshot of a LinkedIn post from a local competitor —
a 4-person accounting firm two suburbs from ours that targets the same
small-business hospitality clients we work with.

Analyse this post and tell me:
1. What their core offer is (what service are they promoting?)
2. What hook or angle they're using to get attention
3. Who the post seems to be aimed at based on the copy
4. One thing they're doing well and one thing I could do differently
   in {{businessName}}'s own LinkedIn content

Keep the response to dot points — I want to share it with my partner.
```
...
That structure — describe what you're sending, then state what you need — applies to every vision task. A receipt: "This is a supplier receipt from a client's FY25 expense bundle. Extract the vendor name, date, line items, and GST total." A scanned letter: "This is a letter from our client's landlord about a commercial lease renewal. Summarise the key dates and any requests they are making." A draft document: "This is a draft of our new fee proposal for the firm's hospitality clients. Tell me if the scope-and-fee section is clear — can a non-accountant reader find what's included and what's excluded quickly?"

### Common mistake → better approach

**Mistake:** Uploading a screenshot of a Xero dashboard and typing "Can you summarise this data?"

Claude returns a general description: "The dashboard shows several metrics including revenue, expenses, and net profit. It appears to be from accounting software." Technically accurate but completely useless.

**Better approach:**

```
I've uploaded a screenshot of a client's Xero P&L for Q3 FY26. This is
a 12-staff cafe group operating across three Melbourne sites. Their
typical Q3 revenue is around $620k.

I need you to:
1. Pull out the three or four headline numbers visible in the screenshot
   (revenue, COGS, gross margin, net profit, or whatever is shown)
2. Identify any line that looks significantly different from what you'd
   expect for this type of business — unusually high or low
3. Write a two-sentence plain-English summary I can paste into the file
   note before our quarterly review meeting

Note: if any numbers are unclear or you're unsure whether you've read
them correctly, flag that explicitly. I'll verify any number that
matters before it goes to the client.
```

The improved prompt does four things well: it gives client context (12-staff cafe group, three Melbourne sites, typical revenue), asks for specific outputs rather than a general summary, sets the format (two sentences), and — critically — asks Claude to flag uncertainty rather than guess silently. That last instruction is important. Claude will sometimes read a number wrong in an image. Asking it to flag uncertainty makes errors visible rather than invisible — exactly what you want before a number lands in a working paper.
