...
**Photos.** A photo of a product, a workspace, a competitor's onboarding screen, or anything you've taken on your phone. Claude can describe what's in the frame, identify text, assess layout, and respond to questions about what it sees.

**Screenshots.** A screenshot of a LinkedIn post, a competitor's pricing page, a release-note email, a Mixpanel dashboard, or a Linear board. Claude can extract text, comment on design, summarise the data it sees, or compare what you've screenshotted to something you describe.

**Scanned documents.** A scanned invoice, a handwritten note, a security questionnaire, a vendor agreement page, a signed DPA. Claude can read the text, pull out key figures, and summarise what the document says — as long as the scan is clear enough to read.
...
**Handwritten notes.** Claude can read reasonably clear handwriting. Not always perfectly — more on that shortly — but well enough to be useful for whiteboard photos, planning sketches, or annotated mockups.
...
These are the use cases that come up regularly for AU/NZ small SaaS teams.

**Reading receipts and invoices.** You've got a stack of scanned receipts from a contractor or vendor. Instead of typing each one into Xero by hand, you upload the image and ask Claude to extract the vendor, date, and total. It does this quickly and accurately for clear scans.

**Analysing a competitor's ad or landing page.** You screenshotted a competitor's LinkedIn ad or their new pricing page. You want to understand their offer, their hook, their call to action. You upload the screenshot and ask Claude to break it down. This is faster and more structured than trying to describe it in text.

**Reviewing a scanned document.** A customer sends a scanned MSA or DPA. You upload it and ask Claude to summarise the key terms, flag anything unusual, or pull out specific clauses. You still need to review the original — Claude is giving you a first pass, not a legal opinion — but that first pass saves time.

**Checking a UI or marketing layout.** You've got a draft pricing page, a launch email, or an in-app banner saved as an image. You ask Claude to comment on the layout, check whether the key message is clear, or flag anything that looks off. Claude won't replace a designer, but it will catch obvious problems before you ship.

**Extracting text from an image.** You have a photo of a whiteboard from a sprint planning session. You want the text without transcribing it yourself. Claude reads the whiteboard and returns the text in a structured format.

**Summarising a metrics dashboard screenshot.** You have a screenshot of a Mixpanel dashboard, a Stripe revenue graph, or a survey result. You ask Claude what the data shows and it gives you a plain-language summary. Useful when you want the insight without staring at the numbers yourself.

These are not edge cases. These are the kinds of tasks that come up weekly in most small SaaS teams.
...
**Complex diagrams with fine detail.** An architecture diagram, a detailed wireframe, a dense product flow with small labels — Claude can describe the overall structure but it will miss fine detail and sometimes make errors on exact figures, dimensions, or labels.
...
**Reading very small text.** Footnotes, fine print, small labels at the edge of a screenshot — Claude often misses these or reads them incorrectly.

The honest framing: Claude's vision is a capable general assistant, not a precision instrument. For most small-SaaS tasks, that is enough. For tasks where errors have real consequences, you verify.
...
**In a Project.** If you are working in a Claude Project that already has documents or context files attached, you can upload an image the same way — via the attachment button in the conversation. The image is treated as part of that conversation's context. This is useful if you are doing repeated visual work on related images — a series of invoices, a set of UI mockups, a collection of competitor screenshots.

**Via file upload on mobile.** The Claude mobile app supports image uploads from your camera roll or files. Tap the attachment icon, choose the image or document, and send. Useful when you are working from your phone and have just taken a photo of something you want to analyse — like a sticker on a customer's till, or a competitor's pricing card at a conference.

One practical note: if you are working on a document-heavy task — like analysing multiple invoices or comparing several UI mockups — consider uploading one at a time and giving Claude a separate prompt for each. Asking it to process many images at once and compare them is harder to do well.

### Prompting with images: the two-part structure

This is the most important part of the lesson, {{firstName}}.

Most founders send an image and type something like: "What do you see?" or "What do you think of this?" These prompts produce vague, general responses. Claude describes what it sees. That is rarely what you actually need.

An effective vision prompt has two parts:

**Part one: describe what you are sending.**
Tell Claude what the image is, where it came from, and any context that helps it read the image correctly. This is not because Claude cannot see the image — it can. It is because context changes interpretation. A screenshot of a competitor's pricing page means something different than a screenshot of your own pricing page that converted poorly. Telling Claude which it is shapes how it responds.
...
**Effective prompt:**
```
I've uploaded a screenshot of a LinkedIn ad from a competing SaaS — they
sell a similar quoting tool to small AU trade businesses, our exact ICP.
...
4. One thing they're doing well and one thing I could do differently in
   my own ads

Keep the response to dot points — I want to share it with my co-founder.
```
...
That structure — describe what you're sending, then state what you need — applies to every vision task. A receipt: "This is a contractor receipt from our last design-agency engagement. Extract the vendor name, date, line items, and GST total." A scanned letter: "This is a letter from our cloud provider about a planned maintenance window. Summarise the key dates and any actions they're asking us to take." A UI mockup: "This is a draft of our new pricing page. Tell me if the hierarchy is clear — can a non-technical buyer find the cheapest annual plan quickly, or does the layout make it hard?"

### Common mistake → better approach

**Mistake:** Uploading a screenshot of a metrics dashboard and typing "Can you summarise this data?"

Claude returns a general description: "The dashboard shows several metrics including monthly traffic, conversion rate, and revenue. It appears to be from a product analytics platform." Technically accurate but completely useless.

**Better approach:**

```
I've uploaded a screenshot of our Mixpanel funnel for March. This is for
{{businessName}}, our project-billing SaaS targeting AU/NZ indie
consultancies. Most signups come from a paid LinkedIn campaign; the rest
are organic word-of-mouth.

I need you to:
1. Pull out the three or four headline numbers visible in the screenshot
   (signups, trial-to-paid conversion, MRR, or whatever is shown)
2. Identify any metric that looks significantly different from what you'd
   expect — unusually high or low
3. Write a two-sentence plain-English summary I can paste into our monthly
   investor update

Note: if any numbers are unclear or you're unsure whether you've read them
correctly, flag that explicitly.
```

The improved prompt does four things well: it gives business context (project-billing SaaS, AU/NZ indie consultancies, paid LinkedIn), asks for specific outputs rather than a general summary, sets the format (two sentences), and — critically — asks Claude to flag uncertainty rather than guess silently. That last instruction is important. Claude will sometimes read a number wrong in an image. Asking it to flag uncertainty makes errors visible rather than invisible.
