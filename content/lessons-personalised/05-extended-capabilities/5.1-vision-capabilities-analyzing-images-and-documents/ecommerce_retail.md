...
Here is what that covers in practice for {{businessName}}:

**Photos.** A photo of a product, a stock arrival, a competitor's window display, or anything you've taken on your phone. Claude can describe what's in the frame, identify text, assess layout, and respond to questions about what it sees.

**Screenshots.** A screenshot of a competitor's social post, a Shopify product page, a Klaviyo report, or a Lightspeed dashboard. Claude can extract text, comment on design, summarise the data it sees, or compare what you've screenshotted to something you describe.

**Scanned documents.** A scanned supplier invoice, a handwritten supplier note, a wholesale order form, a lease page, a signed agreement. Claude can read the text, pull out key figures, and summarise what the document says — as long as the scan is clear enough to read.
...
**Handwritten notes.** Claude can read reasonably clear handwriting. Not always perfectly — more on that shortly — but well enough to be useful for stocktake notes, sketches, or annotated documents.
...
These are the use cases that come up regularly for AU/NZ small retailers.

**Reading receipts and supplier invoices.** You've got a stack of scanned receipts from a wholesale buying trip. Instead of typing out each one, you upload the image and ask Claude to extract the supplier, date, and total. It does this quickly and accurately for clear scans.
...
**Reviewing a scanned wholesale agreement.** A new stockist sends back a scanned and signed wholesale agreement or order form. You upload it and ask Claude to summarise the key terms, flag anything unusual, or pull out specific figures. You still need to review the original — Claude is giving you a first pass, not a legal opinion — but that first pass saves time.

**Checking a design layout.** You've got a draft email banner, a campaign tile, or a wholesale line sheet saved as an image. You ask Claude to comment on the layout, check whether the key message is clear, or flag anything that looks off. Claude won't replace a designer, but it will catch obvious problems before you push the campaign live.

**Extracting text from an image.** You have a photo of a whiteboard from a stocktake or planning session. You want the text without transcribing it yourself. Claude reads the whiteboard and returns the text in a structured format.

**Summarising a Shopify or Klaviyo report screenshot.** You have a screenshot of a Shopify analytics dashboard, a Klaviyo campaign performance chart, or a customer-survey result. You ask Claude what the data shows and it gives you a plain-language summary. Useful when you want the insight without reading the numbers yourself.

These are not edge cases. These are the kinds of tasks that come up weekly in most retail businesses.

### What vision is NOT good for

Claude's vision is useful but it is not infallible. You need to know where it struggles.

**Blurry or low-resolution images.** If you can barely read the text in the image yourself, Claude will struggle too. A photo taken in bad warehouse light, a screenshot at too low a resolution, or a compressed image file will produce guesses rather than accurate readings. Rubbish in, rubbish out.

**Complex diagrams with fine detail.** A detailed packaging spec, a dense pick-and-pack diagram, a sizing chart with small labels — Claude can describe the overall structure but it will miss fine detail and sometimes make errors on exact figures, dimensions, or labels.

**Tasks requiring perfect OCR accuracy.** If you need every word exactly right — for a wholesale agreement, a supplier contract, a financial record — do not rely solely on Claude. It can miss words, misread numbers, or skip lines. Use it as a first pass, then verify the original. Never use Claude's text extraction as your official record.
...
**Reading very small text.** Care-instruction footnotes, fine print on a label, small captions at the edge of an image — Claude often misses these or reads them incorrectly.

The honest framing: Claude's vision is a capable general assistant, not a precision instrument. For most retail tasks, that is enough. For tasks where errors have real consequences, you verify.
...
**In a Project.** If you are working in a Claude Project that already has documents or context files attached, you can upload an image the same way — via the attachment button in the conversation. The image is treated as part of that conversation's context. This is useful if you are doing repeated visual work on related images — a series of supplier invoices, a set of new-arrival product photos, a collection of competitor ads.

**Via file upload on mobile.** The Claude mobile app supports image uploads from your camera roll or files. Tap the attachment icon, choose the image or document, and send. Useful when you are working from your phone and have just taken a photo of a product, a label, or a stocktake whiteboard.

One practical note: if you are working on a document-heavy task — like analysing multiple supplier invoices or comparing several campaign tiles — consider uploading one at a time and giving Claude a separate prompt for each. Asking it to process many images at once and compare them is harder to do well.

### Prompting with images: the two-part structure

This is the most important part of the lesson, {{firstName}}.

Most retailers send an image and type something like: "What do you see?" or "What do you think of this?" These prompts produce vague, general responses. Claude describes what it sees. That is rarely what you actually need.

An effective vision prompt has two parts:

**Part one: describe what you are sending.**
Tell Claude what the image is, where it came from, and any context that helps it read the image correctly. This is not because Claude cannot see the image — it can. It is because context changes interpretation. A screenshot of a competitor's Instagram post means something different than a screenshot of your own post that underperformed. Telling Claude which it is shapes how it responds.
...
**Effective prompt:**
```
I've uploaded a screenshot of an Instagram post from a competing AU homewares brand — they sit at a similar price point and target the same gift-buyer audience as us.

Analyse this post and tell me:
1. What their core offer is (what are they promoting?)
2. What hook or angle they're using to stop the scroll
3. Who the post seems to be aimed at based on the copy and visuals
4. One thing they're doing well and one thing I could do differently in my own posts

Keep the response to dot points — I want to share it with my designer.
```
...
That structure — describe what you're sending, then state what you need — applies to every vision task. A receipt: "This is a supplier invoice from our last wholesale stock order. Extract the supplier name, date, line items, and GST total." A scanned letter: "This is a letter from our shopfront landlord about lease renewal. Summarise the key dates and any requests they are making." A design file: "This is a draft campaign tile for our Mother's Day email. Tell me if the hierarchy is clear — can someone find the offer in two seconds, or does the layout bury it?"

### Common mistake → better approach

**Mistake:** Uploading a screenshot of a Klaviyo dashboard and typing "Can you summarise this data?"

Claude returns a general description: "The dashboard shows several metrics including open rate, click rate, and revenue. It appears to be from an email marketing platform." Technically accurate but completely useless.

**Better approach:**

```
I've uploaded a screenshot of our Klaviyo campaign report for our April Mother's Day email. This is for a small homewares store on Shopify. We sent to 1,800 subscribers, mostly women 35–55 in AU.

I need you to:
1. Pull out the three or four headline numbers visible in the screenshot (open rate, click rate, revenue, conversion rate, or whatever is shown)
2. Identify any metric that looks significantly different from what you'd expect for a small Shopify homewares store — unusually high or low
3. Write a two-sentence plain-English summary I can paste into our monthly review

Note: if any numbers are unclear or you're unsure whether you've read them correctly, flag that explicitly.
```

The improved prompt does four things well: it gives store context (Shopify, homewares, AU women 35–55), asks for specific outputs rather than a general summary, sets the format (two sentences), and — critically — asks Claude to flag uncertainty rather than guess silently. That last instruction is important. Claude will sometimes read a number wrong in an image. Asking it to flag uncertainty makes errors visible rather than invisible.
