...
**Photos.** A photo of a whiteboard, a workspace, a competitor's office reception, or anything you've taken on your phone. Claude can describe what's in the frame, identify text, assess layout, and respond to questions about what it sees.

**Screenshots.** A screenshot of a LinkedIn post, a website, a report, a spreadsheet, or a dashboard. Claude can extract text, comment on design, summarise the data it sees, or compare what you've screenshotted to something you describe.

**Scanned documents.** A scanned invoice, a handwritten note, a form, a lease page, a signed engagement letter. Claude can read the text, pull out key figures, and summarise what the document says — as long as the scan is clear enough to read.
...
**Handwritten notes.** Claude can read reasonably clear handwriting. Not always perfectly — more on that shortly — but well enough to be useful for workshop notes, sketches, or annotated documents.
...
These are the use cases that come up regularly for AU/NZ consulting and advisory firms.

**Reading receipts and invoices.** You've got a stack of scanned receipts from a client trip. Instead of typing out each one, you upload the image and ask Claude to extract the vendor, date, and total. It does this quickly and accurately for clear scans.

**Analysing a competitor's content.** You screenshotted a competitor consultancy's LinkedIn ad or capability snippet. You want to understand their offer, their hook, their call to action. You upload the screenshot and ask Claude to break it down. This is faster and more structured than trying to describe it in text.

**Reviewing a scanned document.** A client sends a scanned engagement letter or compliance form. You upload it and ask Claude to summarise the key terms, flag anything unusual, or pull out specific figures. You still need to review the original — Claude is giving you a first pass, not a legal opinion — but that first pass saves time.

**Checking a slide layout.** You've got a draft client deck slide, a one-pager, or a discovery report saved as an image. You ask Claude to comment on the layout, check whether the key message is clear, or flag anything that looks off. Claude won't replace a designer, but it will catch obvious problems before you send something to a client.

**Extracting text from an image.** You have a photo of a whiteboard from a client workshop. You want the text without transcribing it yourself. Claude reads the whiteboard and returns the text in a structured format.

**Summarising a report chart or dashboard screenshot.** You have a screenshot of a client's analytics dashboard, a financial chart, or a survey result. You ask Claude what the data shows and it gives you a plain-language summary. Useful when you want the insight without reading the numbers yourself.

These are not edge cases. These are the kinds of tasks that come up weekly in most small firms.
...
**Complex diagrams with fine detail.** A process diagram, a detailed org chart, a dense infographic with small labels — Claude can describe the overall structure but it will miss fine detail and sometimes make errors on exact figures, headcounts, or labels.

**Tasks requiring perfect OCR accuracy.** If you need every word exactly right — for a legal document, an engagement letter, a financial record — do not rely solely on Claude. It can miss words, misread numbers, or skip lines. Use it as a first pass, then verify the original. Never use Claude's text extraction as your official record.
...
The honest framing: Claude's vision is a capable general assistant, not a precision instrument. For most small-firm tasks, that is enough. For tasks where errors have real consequences, you verify.
...
One practical note: if you are working on a document-heavy task — like analysing multiple invoices or comparing several scope drafts — consider uploading one at a time and giving Claude a separate prompt for each. Asking it to process many images at once and compare them is harder to do well.
...
Tell Claude what the image is, where it came from, and any context that helps it read the image correctly. This is not because Claude cannot see the image — it can. It is because context changes interpretation. A screenshot of a LinkedIn post from a competitor means something different than a screenshot of your own post that underperformed. Telling Claude which it is shapes how it responds.
...
**Effective prompt:**
```
I've uploaded a screenshot of a LinkedIn ad from a competitor consultancy — they're a Brisbane HR advisory targeting the same mid-market clients we work with.
...
Keep the response to dot points — I want to share it with my associate.
```
...
That structure — describe what you're sending, then state what you need — applies to every vision task. A receipt: "This is a supplier receipt from our last office order. Extract the vendor name, date, line items, and GST total." A scanned letter: "This is a letter from our landlord about the lease renewal. Summarise the key dates and any requests they are making." A draft slide: "This is a draft executive summary slide for {{businessName}}. Tell me if the key recommendation is clear — can a busy CEO pick it up in ten seconds, or does the layout bury the message?"

### Common mistake → better approach

**Mistake:** Uploading a screenshot of a client dashboard and typing "Can you summarise this data?"
...
```
I've uploaded a screenshot of our client's Google Analytics dashboard for March. The client is a national B2B services firm we're advising on lead-generation. Most traffic comes from VIC and NSW.

I need you to:
1. Pull out the three or four headline numbers visible in the screenshot (sessions, conversion rate, lead volume, or whatever is shown)
2. Identify any metric that looks significantly different from what you'd expect — unusually high or low
3. Write a two-sentence plain-English summary I can paste into our monthly status update

Note: if any numbers are unclear or you're unsure whether you've read them correctly, flag that explicitly.
```

The improved prompt does four things well: it gives engagement context (B2B services, lead-gen, national), asks for specific outputs rather than a general summary, sets the format (two sentences), and — critically — asks Claude to flag uncertainty rather than guess silently. That last instruction is important. Claude will sometimes read a number wrong in an image. Asking it to flag uncertainty makes errors visible rather than invisible.
...
Start with that habit, {{firstName}}, and you will get consistently useful responses from vision tasks.
