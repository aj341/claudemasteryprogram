...
Here is a belief that trips up a lot of founders who've worked through this course: if Claude can do something more powerful, that's always the better option. Upload a file instead of pasting text. Build a Project instead of a standalone chat. Set up an automation instead of writing a prompt manually. More advanced means more effective, right?
...
The question isn't "what's the most powerful tool available?" The question is "what's the right tool for this specific task at {{businessName}}?" Those are different questions, and confusing them wastes time and introduces friction you don't need.

### The four-tier framework

Think of Claude's capabilities in four tiers, each with a clear use case.

**Tier 1 — Basic chat.** A single conversation, no setup, no uploads, no persistent memory. You open a chat, type a prompt, get a response, close it. This is the right tier for one-off tasks, quick questions, first drafts of things you'll refine yourself, and anything where the context fits in a single prompt. Most founders underestimate how much they can get done at this tier with a well-crafted prompt.

**Tier 2 — Project + knowledge base.** A named Project with a system prompt and one or more uploaded reference documents. The right tier for recurring workflows where you always need the same context — your tone of voice, your feature list, your strategic account's details, your internal processes. The system prompt carries the standing instructions so you don't have to restate them every conversation.

**Tier 3 — File and vision processing.** Uploading documents, images, or screenshots into a conversation so Claude can read, analyse, or extract from them. The right tier when the task requires Claude to engage with content that doesn't fit neatly into a typed prompt — a competitor's PDF pricing page, a vendor invoice, a screenshot of a Mixpanel funnel.

**Tier 4 — API and automation.** Claude triggered by events rather than by you. A demo-request form arrives, an automation sends it to Claude, Claude drafts a response, the response goes somewhere — without you sitting there. The right tier for tasks you've already done manually many times, know work, and want to stop repeating.
...
**Using vision when text would do.** Someone takes a screenshot of a help-centre article, uploads it as an image, and asks Claude to read and summarise it. This works — but Claude is reading pixels and reconstructing text. If the document exists as a copyable text source, copying and pasting the relevant section is faster and produces a more accurate result. Vision is valuable when the document genuinely only exists as an image. It's overhead when it doesn't.

**Common mistake → Better approach**

A founder receives a vendor quote as a PDF. She screenshots the key pricing table and uploads the image so Claude can extract the figures. Claude reads the image, but misreads two numbers — "8" looks like "B" at that resolution.

The better approach: open the PDF, select and copy the pricing table text, paste it directly into the chat. If the PDF is scanned and not selectable, the image upload is the right call. If the text is selectable, use it. The rule, {{firstName}}: prefer machine-readable text over images when you have the choice.

**Uploading a PDF when pasting the key section is faster.** This is a subtle version of the same trap. You have a ten-page MSA, and you want Claude to review the payment terms clause. You upload the whole PDF. Claude processes ten pages to find the two paragraphs you need. The faster and more precise approach: open the contract, navigate to the payment terms section, copy those three paragraphs, paste them into the chat with your question. You get a sharper answer because Claude isn't parsing irrelevant content. You get it faster because you're not waiting on a file upload.
...
**Treating a Project as the default for everything.** Projects are useful. But they're not the right container for every task. If you're using a Project for a one-off task — writing a single press release, drafting a one-time investor update for an unusual SAFE round — the Project structure is overhead you didn't need. Open a standard chat, write a thorough prompt, get the output, close it. Save Projects for recurring workflows with consistent context requirements.
...
You won't get this perfectly right on the first pass. Most founders find at least one thing in their audit that belongs at a different tier. That's not a failure — that's the point of auditing. The value is in the correction.
