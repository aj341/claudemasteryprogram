...
Here is a belief that trips up a lot of tradies who've worked through this course: if Claude can do something more powerful, that's always the better option. Upload a file instead of pasting text. Build a Project instead of a standalone chat. Set up an automation instead of writing a prompt manually. More advanced means more effective, right?
...
**Tier 1 — Basic chat.** A single conversation, no setup, no uploads, no persistent memory. You open a chat, type a prompt, get a response, close it. This is the right tier for one-off tasks, quick questions, first drafts of things you'll refine yourself, and anything where the context fits in a single prompt. Most tradies underestimate how much they can get done at this tier with a well-crafted prompt.

**Tier 2 — Project + knowledge base.** A named Project with a system prompt and one or more uploaded reference documents. The right tier for recurring workflows where you always need the same context — your tone of voice, your service list, your homeowner brief, your internal SWMS templates. The system prompt carries the standing instructions so you don't have to restate them every conversation.

**Tier 3 — File and vision processing.** Uploading documents, images, or screenshots into a conversation so Claude can read, analyse, or extract from them. The right tier when the task requires Claude to engage with content that doesn't fit neatly into a typed prompt — a head-contractor's PDF agreement, a Reece invoice, a photo of a damaged switchboard taken on the phone.

**Tier 4 — API and automation.** Claude triggered by events rather than by you. An enquiry form lands, an automation sends it to Claude, Claude drafts a response, the response goes somewhere — without you sitting there. The right tier for tasks you've already done manually many times, know work, and want to stop repeating.
...
This is the hardest question to answer honestly. It requires you to actually try the simpler approach before concluding you need the more complex one. Many trades owners skip straight to building a Project or setting up an automation because they assume a basic prompt won't be good enough. Often a properly structured Tier 1 prompt — with a role, a clear task, a format, and a length constraint — gets you to 80% of what the complex setup produces. If 80% is enough for this task, the complex setup is overhead you don't need.
...
**Using vision when text would do.** A builder takes a screenshot of a supplier proposal, uploads it as an image, and asks Claude to read and summarise it. This works — but Claude is reading pixels and reconstructing text. If the document exists as a copyable text file, copying and pasting the relevant section is faster and produces a more accurate result. Vision is valuable when the document genuinely only exists as an image — a phone photo of a damaged switchboard, a defect snap from site. It's overhead when it doesn't.

**Common mistake → Better approach**

A builder receives a supplier quote as a PDF. He screenshots the key pricing table and uploads the image so Claude can extract the figures. Claude reads the image, but misreads two numbers — "8" looks like "B" at that resolution.
...
**Uploading a PDF when pasting the key section is faster.** This is a subtle version of the same trap. You have a ten-page head-contractor agreement, and you want Claude to review the variation clause. You upload the whole PDF. Claude processes ten pages to find the two paragraphs you need. The faster and more precise approach: open the agreement, navigate to the variation section, copy those three paragraphs, paste them into the chat with your question. You get a sharper answer because Claude isn't parsing irrelevant content. You get it faster because you're not waiting on a file upload.
...
**Treating a Project as the default for everything.** Projects are useful. But they're not the right container for every task. If you're using a Project for a one-off task — writing a single capability statement, drafting a one-time letter to council about a difficult inspection — the Project structure is overhead you didn't need. Open a standard chat, write a thorough prompt, get the output, close it. Save Projects for recurring workflows with consistent context requirements.
...
The audit goes both ways. Over-engineering (using a complex setup for a simple task) wastes setup time and adds maintenance burden. Under-engineering (typing the same business background into every chat because you haven't set up a Project) wastes repetitive effort you've already done.
...
You won't get this perfectly right on the first pass. Most tradies find at least one thing in their audit that belongs at a different tier. That's not a failure — that's the point of auditing. The value is in the correction.
