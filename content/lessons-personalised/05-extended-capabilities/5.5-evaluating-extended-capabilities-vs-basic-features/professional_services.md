...
Here is a belief that trips up a lot of consultants who've worked through this course: if Claude can do something more powerful, that's always the better option. Upload a file instead of pasting text. Build a Project instead of a standalone chat. Set up an automation instead of writing a prompt manually. More advanced means more effective, right?
...
**Tier 1 — Basic chat.** A single conversation, no setup, no uploads, no persistent memory. You open a chat, type a prompt, get a response, close it. This is the right tier for one-off tasks, quick questions, first drafts of things you'll refine yourself, and anything where the context fits in a single prompt. Most consultants underestimate how much they can get done at this tier with a well-crafted prompt.
...
This is the hardest question to answer honestly. It requires you to actually try the simpler approach before concluding you need the more complex one. Many consultants skip straight to building a Project or setting up an automation because they assume a basic prompt won't be good enough. Often a properly structured Tier 1 prompt — with a role, a clear task, a format, and a length constraint — gets you to 80% of what the complex setup produces. If 80% is enough for this task, the complex setup is overhead you don't need.
...
A consultant receives a subcontractor quote as a PDF. She screenshots the key fee table and uploads the image so Claude can extract the figures. Claude reads the image, but misreads two numbers — "8" looks like "B" at that resolution.

The better approach: open the PDF, select and copy the fee table text, paste it directly into the chat. If the PDF is scanned and not selectable, the image upload is the right call. If the text is selectable, use it. The rule: prefer machine-readable text over images when you have the choice.

**Uploading a PDF when pasting the key section is faster.** This is a subtle version of the same trap. You have a ten-page subcontractor agreement, and you want Claude to review the payment terms clause. You upload the whole PDF. Claude processes ten pages to find the two paragraphs you need. The faster and more precise approach: open the agreement, navigate to the payment terms section, copy those three paragraphs, paste them into the chat with your question. You get a sharper answer because Claude isn't parsing irrelevant content. You get it faster because you're not waiting on a file upload.
...
You won't get this perfectly right on the first pass. Most consultants find at least one thing in their audit that belongs at a different tier. That's not a failure — that's the point of auditing. The value is in the correction.
