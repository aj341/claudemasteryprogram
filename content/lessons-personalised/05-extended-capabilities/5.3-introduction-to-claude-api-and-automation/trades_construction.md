...
Think of it like this. When a homeowner rings you to book a job, you're the human in the loop. But if you've got an online booking form on your website, jobs can come in at any hour without anyone picking up a phone. The API is the online booking form. Other tools and apps can send requests to Claude and receive responses automatically — without you having to type anything, and without you having to be there at all.

That's the key shift: the API lets Claude respond to triggers — not just to your manual prompts. A new enquiry form lands, Claude drafts a response. A job is marked complete in ServiceM8, Claude generates a follow-up email. A batch of supplier invoices arrives in your inbox, Claude summarises each one. All of this happens in the background, driven by events, not by you sitting at a keyboard at 9pm.
...
**n8n** is an open-source option. You can run it yourself (free, but requires some technical setup) or use their hosted version (paid). It's the most flexible of the three but also the most technical. For most trades owners without a developer mate, n8n's self-hosted option is probably a step too far — but their cloud version is a genuine option if you want deep customisation without vendor lock-in.
...
What Claude does: Receives the form data — name, suburb, job type, problem description — and drafts a personalised response email that acknowledges the specific enquiry, mentions relevant trades or experience, and proposes a next step (usually a site visit or a quote).

Output: A draft email, either sent automatically or saved to your drafts folder for review before sending.

This automation works well because enquiry responses are high-stakes (first impression) and often repetitive (the same trade questions, the same suburbs, the same services). Once you've done this manually twenty times and know what a good response looks like, automating the first draft saves fifteen to twenty minutes per enquiry while keeping the quality consistent.

**2. Summarising incoming email threads**

Trigger: An email lands in a specific label or folder (e.g., "Builder Updates" or "Supplier Queries") in Gmail.
...
Output: The summary is either sent to you as a daily digest, added as a note in ServiceM8 or simPRO, or posted to a Slack channel your team monitors.

This works for trades owners managing a high volume of homeowner and builder communications who need a quick read on each thread without opening and scrolling through every email chain. It's especially useful when you're juggling three sites or running a residential operation alongside small commercial — anywhere a job has multiple stakeholders sending updates across a long thread.
...
What Claude does: Receives data from a Google Sheet (job completions, hours billed, callbacks logged, debtors days — whatever you track) and produces a plain-English weekly summary: what went up, what went down, what needs attention.

Output: A formatted report emailed to you, posted to Slack, or added to a shared Google Doc.

This works because pulling meaning from numbers is exactly the kind of task Claude does well — and it's a task most trades owners do manually at the end of every week, spending twenty to thirty minutes writing a summary they could generate in thirty seconds. Once you've done it manually enough times to know what a good weekly report looks like for your business, automating it is straightforward.
