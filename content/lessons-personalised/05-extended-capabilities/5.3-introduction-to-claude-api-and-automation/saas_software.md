...
That's the key shift: the API lets Claude respond to triggers — not just to your manual prompts. A new trial signup hits your Stripe webhook, Claude drafts the welcome email. A new churn-survey response lands in your spreadsheet, Claude classifies the reason. A batch of demo Gong transcripts gets uploaded, Claude summarises each one. All of this happens in the background, driven by events, not by you sitting at a keyboard.

You don't need to understand the technical side of the API to benefit from it. But you do need to understand what it makes possible — because that shapes the decisions you'll make in this lesson about where automation might fit in {{businessName}}.
...
The three most relevant for AU/NZ small SaaS teams right now are Zapier, Make, and n8n.

**Zapier** is the most widely used and has the gentlest learning curve. It works on a "trigger and action" model: something happens in one app, Zapier does something in another. It has a native Claude integration, which means you can include a Claude step in any Zap — send Claude some text, get a response back, pass that response somewhere else. Zapier is subscription-based and the pricing scales with how many automated tasks you run each month. For most small SaaS teams testing a first automation, the free tier or the Starter plan is enough to begin.

**Make** (formerly Integromat) is more visual and more flexible than Zapier. You build workflows as flowcharts — each step is a module, and you can branch, filter, and loop through data in ways Zapier doesn't support as easily. It tends to suit founders who want more control over what happens at each step. The learning curve is steeper, but the pricing is generally more cost-effective at higher volumes.

**n8n** is an open-source option. You can run it yourself (free, but requires some technical setup) or use their hosted version (paid). It's the most flexible of the three but also the most technical. For most non-technical founders, n8n's self-hosted option is probably a step too far — but their cloud version is a genuine option if you want deep customisation without vendor lock-in. If your engineer already runs n8n internally, lean on that.

The practical takeaway: if you want to test your first Claude automation without your engineer, start with Zapier. It has the most tutorials, the largest community, and the clearest interface for someone building their first workflow. Make is worth exploring once you've run a few Zaps and want more flexibility. n8n is for when you've outgrown both and want to own the infrastructure.
...
Here are three specific examples of Claude-powered automations that founders have built using no-code tools — no engineering review required.

**1. Auto-drafting responses to demo-request form submissions**

Trigger: A new demo-request form is submitted on your website (via Typeform, JotForm, or your own form on the marketing site).

What Claude does: Receives the form data — name, company, team size, use case — and drafts a personalised response email that acknowledges the specific use case, mentions relevant case studies, and proposes a next step (a 20-minute call slot, a tailored Loom).

Output: A draft email, either sent automatically or saved to your drafts folder for review before sending.

This automation works well because demo-request responses are high-stakes (first impression with a buyer) and often repetitive (the same questions, the same kind of buyer, the same use cases). Once you've done this manually twenty times and know what a good response looks like, automating the first draft saves fifteen to twenty minutes per request while keeping the quality consistent.

**2. Summarising incoming Intercom or email threads**

Trigger: A new conversation lands in a specific Intercom inbox or a Gmail label (e.g., "Customer Updates" or "Vendor Queries").

What Claude does: Reads the thread and produces a three-to-five bullet summary: what the situation is, what's been agreed, what's outstanding, and whether any action is required from you.

Output: The summary is either sent to you as a daily digest, added as a note in Pipedrive, or posted to a Slack channel your team monitors.

This works for founders managing a high volume of customer threads who need a quick read on each conversation without opening and scrolling through every reply chain. It's especially useful in customer success and account management — anywhere a thread has multiple stakeholders sending updates across a long Intercom conversation.

**3. Generating a weekly metrics report from a spreadsheet**

Trigger: A scheduled time trigger — every Monday morning at 7am, or every Friday at 5pm.

What Claude does: Receives data from a Google Sheet (MRR, signups, churn, NPS — whatever you track) and produces a plain-English weekly summary: what went up, what went down, what needs attention.

Output: A formatted report emailed to you, posted to Slack, or added to a shared Notion page.

This works because pulling meaning from numbers is exactly the kind of task Claude does well — and it's a task most small SaaS founders do manually at the end of every week, spending twenty to thirty minutes writing a summary they could generate in thirty seconds. Once you've done it manually enough times to know what a good weekly report looks like for your SaaS, automating it is straightforward.
...
Current pricing for Claude Sonnet (the model most automation workflows use, as at April 2026): approximately USD $3 per million input tokens and $15 per million output tokens. To put that in practical terms: a typical demo-request response — processing a 200-word form submission and generating a 150-word draft email — costs roughly a fraction of one cent. Running that automation a hundred times a month costs less than a dollar in API fees alone.
...
Here is the most important concept in this lesson, and the one most founders skip.

Automate only what you have already done repeatedly by hand, and know works.

If you haven't manually responded to twenty demo-request submissions and developed a sense of what a good response looks like — including the tone, the length, the specific case studies to mention, the things to avoid — you don't yet know what "good" looks like. You can't configure Claude to produce good automated outputs if you haven't defined what good means for your SaaS. You'll build an automation that produces mediocre outputs at scale, and you won't notice because you're no longer in the loop.
...
Before you automate any task, ask yourself three questions, {{firstName}}:
...
A founder reads about automating demo-request responses, thinks "great idea," spends a weekend building a Zapier workflow with Claude, and switches it on. The automation sends responses — but they're generic, they don't reflect the product's voice, they don't address the right buyer concerns, and the conversion rate from request to booked demo drops. The founder eventually turns it off, concludes "AI doesn't work for this," and goes back to writing responses by hand.

**Better approach:** Do the manual version first. Open Claude, paste in a real demo-request submission, write a prompt that produces a response you'd actually be proud to send. Do this fifteen times. Refine the prompt each time something doesn't land. Once your prompt reliably produces a response that feels like you wrote it — and you can see the quality in the booked-demo rate — then you have everything you need to build the automation. The prompt you've refined manually becomes the system prompt in your Zap. The inputs you've been pasting manually become the variables the form data fills in. The automation doesn't replace your judgment. It runs the judgment you've already developed.
