...
Think of it like this. When you call a supplier to place an order, you're the human in the loop. But if the supplier has an online ordering portal, you can place orders at any time without anyone picking up a phone. The API is the online ordering portal. Other tools and apps can send requests to Claude and receive responses automatically — without you having to type anything, and without you having to be there at all.

That's the key shift: the API lets Claude respond to triggers — not just to your manual prompts. A new wholesale enquiry comes in, Claude drafts a response. A Shopify order is flagged, Claude generates a delay note. A batch of customer-service emails arrives, Claude summarises each one. All of this happens in the background, driven by events, not by you sitting at a keyboard.
...
Connecting your retail tools to Claude's API directly would require a developer. But there's a category of software called no-code automation tools that does that connecting work for you — through a visual interface, without any programming.

The three most relevant for AU/NZ small retailers right now are Zapier, Make, and n8n.

**Zapier** is the most widely used and has the gentlest learning curve. It works on a "trigger and action" model: something happens in one app, Zapier does something in another. It has a native Claude integration, which means you can include a Claude step in any Zap — send Claude some text, get a response back, pass that response somewhere else (Shopify, Klaviyo, your helpdesk, Gmail). Zapier is subscription-based and the pricing scales with how many automated tasks you run each month. For most small retailers testing a first automation, the free tier or the Starter plan is enough to begin.

**Make** (formerly Integromat) is more visual and more flexible than Zapier. You build workflows as flowcharts — each step is a module, and you can branch, filter, and loop through data in ways Zapier doesn't support as easily. It tends to suit retailers who want more control over what happens at each step. The learning curve is steeper, but the pricing is generally more cost-effective at higher volumes.

**n8n** is an open-source option. You can run it yourself (free, but requires some technical setup) or use their hosted version (paid). It's the most flexible of the three but also the most technical. For most small retailers without a developer, n8n's self-hosted option is probably a step too far — but their cloud version is a genuine option if you want deep customisation without vendor lock-in.
...
Here are three specific examples of Claude-powered automations that small retailers have built using no-code tools — no developer required.

**1. Auto-drafting responses to wholesale enquiry forms**

Trigger: A new wholesale enquiry form is submitted on your website (via Typeform, JotForm, Gravity Forms, or your Shopify wholesale page).

What Claude does: Receives the form data — name, business type, store location, what they want to stock — and drafts a personalised response email that acknowledges the specific enquiry, mentions the most relevant SKUs from your range for their store profile, and proposes a next step (usually a sample box or a 20-minute call).

Output: A draft email, either sent automatically or saved to your Gmail drafts folder for review before sending.

This automation works well because wholesale responses are high-stakes (first impression with a potential stockist) and often repetitive (the same questions, the same store profiles, the same range overview). Once you've done this manually twenty times and know what a good wholesale-first-reply looks like, automating the first draft saves fifteen to twenty minutes per enquiry while keeping the quality consistent.

**2. Summarising incoming customer-service email threads**

Trigger: An email lands in a specific label or folder (e.g., "Damaged Stock" or "Wholesale Queries") in Gmail.
...
Output: The summary is either sent to you as a daily digest, added as a note in your helpdesk, or posted to a Slack channel your team monitors.

This works for retailers managing a high volume of customer comms who need a quick read on each thread without opening and scrolling through every email chain. It's especially useful for multi-channel brands and Etsy sellers scaling to wholesale — anywhere a customer or stockist relationship has multiple touchpoints across a long thread.

**3. Generating a weekly retail report from a Shopify export**

Trigger: A scheduled time trigger — every Monday morning at 7am, or every Friday at 5pm.

What Claude does: Receives data from a Google Sheet (Shopify orders, top SKUs, returns, traffic) and produces a plain-English weekly summary: what went up, what went down, what needs attention.

Output: A formatted report emailed to you, posted to Slack, or added to a shared Google Doc.

This works because pulling meaning from numbers is exactly the kind of task Claude does well — and it's a task most retailers do manually at the end of every week, spending twenty to thirty minutes writing a summary they could generate in thirty seconds. Once you've done it manually enough times to know what a good weekly report looks like for your store, automating it is straightforward.
...
Current pricing for Claude Sonnet (the model most automation workflows use, as at April 2026): approximately USD $3 per million input tokens and $15 per million output tokens. To put that in practical terms: a typical wholesale-enquiry response — processing a 200-word form submission and generating a 150-word draft email — costs roughly a fraction of one cent. Running that automation a hundred times a month costs less than a dollar in API fees alone.
...
Here is the most important concept in this lesson, {{firstName}}, and the one most people skip.

Automate only what you have already done repeatedly by hand, and know works.

If you haven't manually responded to twenty wholesale enquiry forms and developed a sense of what a good response looks like — including the tone, the length, the specific SKUs to recommend, the things to avoid — you don't yet know what "good" looks like. You can't configure Claude to produce good automated outputs if you haven't defined what good means for your store. You'll build an automation that produces mediocre outputs at scale, and you won't notice because you're no longer in the loop.
...
A retailer reads about automating wholesale enquiry responses, thinks "great idea," spends a weekend building a Zapier workflow with Claude, and switches it on. The automation sends responses — but they're generic, they don't reflect the brand voice, they don't address the right buyer concerns, and the conversion rate from enquiry to first sample order drops. The retailer eventually turns it off, concludes "AI doesn't work for wholesale," and goes back to writing responses by hand.

**Better approach:** Do the manual version first. Open Claude, paste in a real wholesale enquiry, write a prompt that produces a response you'd actually be proud to send. Do this fifteen times. Refine the prompt each time something doesn't land. Once your prompt reliably produces a response that feels like you wrote it — and you can see the quality in the conversion rate — then you have everything you need to build the automation. The prompt you've refined manually becomes the system prompt in your Zap. The inputs you've been pasting manually become the variables the form data fills in. The automation doesn't replace your judgment. It runs the judgment you've already developed.
