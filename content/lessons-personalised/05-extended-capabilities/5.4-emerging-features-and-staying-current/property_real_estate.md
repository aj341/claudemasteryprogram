...
Think about the last time your CRM or trust accounting platform changed. There was probably a banner notification, a few updated menu items, maybe a new report format. You adapted in an afternoon. The core of the tool — where the rent roll lives, how you log a maintenance request, what an end-of-month statement produces — stayed the same.

Claude changes differently. New capabilities arrive without warning. The same prompt {{firstName}} has been using for vendor updates for three months might produce a noticeably different output today — sometimes better, sometimes just different, occasionally worse. The model you're talking to is updated regularly, and Anthropic doesn't always announce every change when it happens.
...
But "mostly good news" isn't the same as "no adjustments required." A few times a year, something you've been relying on will shift. A vendor-update prompt that used to produce clean dot-pointed output will suddenly add extra explanation you didn't ask for. A tone you'd dialled in for owner reports will skew slightly formal. An output format you'd standardised across your sales reps will change shape. When that happens, you need to know how to recognise it and what to do.
...
**Capability upgrades** mean Claude can now do something it couldn't do before, or do something it did poorly much better. Examples: the ability to process large documents like full strata reports, improved reasoning on multi-step tasks, better performance on structured data, the addition of vision capabilities for floor plans and condition reports. These updates expand what's possible. They don't break anything you're already doing. The right response is to notice them and file them away: is there a task in {{businessName}} where this new capability would actually help?
...
Knowing which type of change just happened stops you overreacting or underreacting. You don't need to rebuild your vendor-update workflow when a menu moves. You don't need to ignore a new capability just because it's not in today's task list.
...
**One practical newsletter**, not twenty. The AI space produces a large volume of content, most of it aimed at investors, developers, or people who want to be generally informed about the industry. For your purposes — using Claude in a real estate business — one curated newsletter is enough. Look for newsletters that write about practical applications of AI tools rather than news about AI as an industry. "Claude added a new feature that does X, here's how to use it for client work" is useful. "AI is going to change everything about work" is not, for your purposes.
...
The `wait for the use case` approach has a second benefit: it filters out features that sound exciting but don't fit your business. Not every Claude capability is relevant to every agency. A feature that helps developers process code at scale is not a feature you need to understand deeply if you're a buyer's agent. A capability that improves Claude's performance on long financial documents is extremely relevant to a strata manager and mostly irrelevant to a sales rep doing CMAs. Waiting for your own use case to surface means you spend learning time on the features that will actually change how you work.

### When Claude gets something wrong that it used to get right

This is the situation most people aren't prepared for: you've built a reliable workflow around a Claude prompt — say, your weekly post-OFI vendor update — and one day the output is off. Not wrong in a factual sense — just different from what you expected. The tone shifted. The format changed. The level of detail changed. Something that used to be reliable is now inconsistent.
...
3. Add an explicit instruction for the thing that changed. If Claude is now adding explanation you didn't ask for, add: `Provide no preamble or explanation. Output only the requested content.` If the tone has shifted to too-casual for a vendor email, add: `Write in a direct, professional tone — no warmth, no hedging.`
...
A principal reads about Claude's ability to analyse documents — contracts, building inspections, strata reports. She uploads a PDF of a contract of sale and types: "What does this say?" Claude summarises the contract. She thinks: "That's fine, but I could just read it myself." She doesn't use the feature again.

The test failed not because the feature is limited but because the task wasn't specific. "What does this say?" is a reading task. The feature's value is in analysis tasks — comparison, extraction, identification of specific clauses, flagging anomalies. A better test would be: "Read this contract of sale. Identify the settlement period, the cooling-off provisions, the special conditions, and any clauses that differ from our standard NSW agency template." That prompt extracts information the principal would otherwise have to search for manually, and it would have revealed what the feature actually does.
