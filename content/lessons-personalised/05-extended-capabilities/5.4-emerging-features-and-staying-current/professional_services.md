...
Think about the last time your practice management software changed. There was probably a banner notification, a few updated menu items, maybe a new report format. You adapted in an afternoon. The core of the tool — where the data lives, how you navigate it, what it produces — stayed the same.
...
The `wait for the use case` approach has a second benefit: it filters out features that sound exciting but don't fit your firm. Not every Claude capability is relevant to every consultant. A feature that helps developers process code at scale is not a feature you need to understand deeply if you run a 4-person HR consultancy. A capability that improves Claude's performance on long financial documents is extremely relevant to a business advisor and mostly irrelevant to a brand strategist. Waiting for your own use case to surface means you spend learning time on the features that will actually change how you work.

### When Claude gets something wrong that it used to get right

This is the situation most consultants aren't prepared for: you've built a reliable workflow around a Claude prompt, and one day the output is off. Not wrong in a factual sense — just different from what you expected. The tone shifted. The format changed. The level of detail changed. Something that used to be reliable is now inconsistent.
...
A consultant reads about Claude's ability to analyse documents — invoices, engagement letters, reports. She uploads a PDF of a subcontractor agreement and types: "What does this say?" Claude summarises the agreement. She thinks: "That's fine, but I could just read it myself." She doesn't use the feature again.

The test failed not because the feature is limited but because the task wasn't specific. "What does this say?" is a reading task. The feature's value is in analysis tasks — comparison, extraction, identification of specific clauses, flagging anomalies. A better test would be: "Read this agreement. Identify the payment terms, the notice period for cancellation, and any clauses that differ from our standard 30-day payment arrangement." That prompt extracts information the principal would otherwise have to search for manually, and it would have revealed what the feature actually does.
