...
Think about the last time your job-management software changed. There was probably a banner notification in ServiceM8 or simPRO, a few updated menu items, maybe a new report format. You adapted in an afternoon. The core of the tool — where the jobs live, how you navigate it, what it produces — stayed the same.
...
**Anthropic's blog** at anthropic.com/news. When a major update ships — a new model, a significant new capability — Anthropic publishes an announcement here. It's infrequent (a few times a year for major releases), written for a general audience, and doesn't require a technical background. Set a reminder to check it once a month. Five minutes — easy to fit in over smoko.
...
**One practical newsletter**, not twenty. The AI space produces a large volume of content, most of it aimed at investors, developers, or people who want to be generally informed about the industry. For your purposes — using Claude in a trades business — one curated newsletter is enough. Look for newsletters that write about practical applications of AI tools rather than news about AI as an industry. "Claude added a new feature that does X, here's how to use it" is useful. "AI is going to change everything about work" is not, for your purposes.
...
The `wait for the use case` approach has a second benefit: it filters out features that sound exciting but don't fit your business. Not every Claude capability is relevant to every trades owner. A feature that helps developers process code at scale is not a feature you need to understand deeply if you run a landscaping crew. A capability that improves Claude's performance on long financial documents is extremely relevant to a builder reviewing head-contractor agreements and mostly irrelevant to a sole-trader tiler. Waiting for your own use case to surface means you spend learning time on the features that will actually change how you work.

### When Claude gets something wrong that it used to get right

This is the situation most tradies aren't prepared for: you've built a reliable workflow around a Claude prompt — say, drafting variation notices to homeowners — and one day the output is off. Not wrong in a factual sense — just different from what you expected. The tone shifted. The format changed. The level of detail changed. Something that used to be reliable is now inconsistent.
...
A builder reads about Claude's ability to analyse documents — invoices, contracts, reports. He uploads a PDF of a head-contractor agreement and types: "What does this say?" Claude summarises the agreement. He thinks: "That's fine, but I could just read it myself." He doesn't use the feature again.

The test failed not because the feature is limited but because the task wasn't specific. "What does this say?" is a reading task. The feature's value is in analysis tasks — comparison, extraction, identification of specific clauses, flagging anomalies. A better test would be: "Read this agreement. Identify the payment terms, the defects liability period, and any clauses that differ from our standard 30-day progress claim arrangement." That prompt extracts information the builder would otherwise have to search for manually, and it would have revealed what the feature actually does.
