...
Most accountants hear "research" and think: go out and find information. That's one kind of research. It's not what Claude is for.

Claude is a text processor, not a search engine. It doesn't browse the internet, doesn't check live ATO databases, and doesn't know what happened last week. If you ask Claude to research current super contribution caps, find today's foreign exchange rate, or tell you what the ATO ruled in last month's tax determination — you'll get either a refusal or, worse, a confident-sounding answer that's out of date.

But there's a second kind of research that Claude handles extremely well: analysing material you already have.

You have fee proposals from three accounting software vendors. You have a contract section in a client's lease that you don't fully understand. You have fifty rows of client survey feedback. You have an engagement letter that's forty pages long. You have your file notes from a discovery call. Any of these can go into Claude, and Claude can work with all of them — compare them, summarise them, extract patterns, flag risks, and tell you what they mean for a decision you need to make.
...
**Synthesising information you provide.** Paste in multiple working papers, client emails, or datasets and ask Claude to draw them together into a coherent picture. Claude holds everything in the conversation in working memory and can cross-reference it. This is where its large context window earns its keep.

**Comparing options against criteria.** Give Claude a shortlist — three software vendors, two payroll tools, four candidate responses to a client complaint about a fee — along with the criteria that matter to you, and ask it to compare them. Claude produces structured comparisons consistently when you give it structured inputs.

**Analysing documents you paste in.** Engagement letters, fee proposals, terms and conditions, ATO determinations, survey results. Ask Claude to read and work with the document — extract key terms, flag unusual clauses, summarise the main obligations, note anything missing relative to what you'd expect to see.

**Summarising long material.** A long client email thread, a partner board paper, a software implementation deliverable. Paste it in and ask for a summary shaped around what you need from it — the decisions, the risks, the action items. Claude reads the whole thing and produces what you ask for.

**Asking analytical questions about material you provide.** "Given everything I've pasted in, what's the strongest argument for restructuring from sole trader to Pty Ltd?" "What's missing from this fee proposal relative to what we usually include?" "What does this client feedback pattern suggest about our onboarding process?" These are questions Claude handles well — because the material is in front of it and it can reason across it.

### What Claude is not good at for research and analysis

This matters as much as the previous section, so read it carefully.

**Finding live information.** Claude cannot check current super caps, verify whether a software vendor is still operating, look up today's exchange rates, or confirm that a tax ruling is still current. If you ask and Claude answers, it's drawing on training data — which has a cutoff date. Anything time-sensitive needs a current source (the ATO website, your software vendor's release notes, your professional body's bulletins).

**Knowing your specific market's current pricing or rates.** "What should a registered tax agent charge per hour in Adelaide?" is a question Claude will attempt — but its answer is based on training data, not the current market. Use Claude to structure the analysis once you have the numbers. Get the numbers from a current source like CPA Australia or your benchmarking data.

**Anything requiring real-time data.** Live ledger balances, today's interest rate movements, current SMSF investment values, this morning's ATO portal status. Not Claude's territory.

**Verifying facts that weren't in the material you provided.** If you paste in a client's lease and ask whether the indemnity clause is standard in your industry, Claude can comment on what it knows generally. But "generally" is not "currently in your specific market." For anything with legal or tax consequences, the analysis Claude provides is a starting point, not a conclusion. You verify with a current source.

The practical rule: if accuracy matters and the information is time-sensitive or highly specific to your client's situation, use Claude to structure and analyse — but verify the facts from a current, authoritative source.

### The "bring your own material" workflow

This is the most valuable pattern in this lesson. It has four steps.

**Step 1: Gather your raw material.** This is the part Claude can't do for you. Find the proposals, the feedback, the contract, the working papers, the emails. If the material is confidential, you can summarise or paraphrase it before pasting — Claude doesn't require verbatim text, only enough detail to work with.

**Step 2: Decide what question you need answered.** Before you open Claude, be clear on the decision or output you need. "Which software should I roll out across my client base?" "What are the key risks in this client's commercial lease?" "What patterns appear in this client survey feedback?" The question shapes the prompt.
...
```
Below are fee proposals from three external bookkeepers I'm considering
sub-contracting to for overflow BAS work. I need to choose one. My priorities,
in order, are: reliability (they meet quarterly BAS deadlines), communication
(they respond same-day when a client query lands with them), and price
(lowest cost is not the goal — value for money is).

For each bookkeeper, summarise what the proposal tells me about each of
my three priorities. Then produce a comparison table: rows are bookkeepers,
columns are my three priorities, cells are brief assessments. Finish with
a short paragraph identifying the strongest option based on this
information only.

[Three proposals pasted below]
```
...
Comparison prompts are one of the highest-value uses of Claude in a practice context. Here's what makes them work.

**Provide the options as parallel information.** If you're comparing three vendors, include the same categories of information for each — or note explicitly what's missing. Claude compares what you give it. If Vendor A's proposal covers five pages and Vendor B's covers half a page, the comparison will reflect that asymmetry.

**State your evaluation criteria explicitly.** Don't make Claude guess what matters to you. "I care about price, turnaround time, and whether they've worked with hospitality businesses on Single Touch Payroll" is far more useful than "help me compare these." Claude will weight the criteria you name — so name the right ones.
...
Mistake: "Compare these two software options for me." Claude produces two paragraphs of balanced description that don't help you decide anything for {{businessName}}.

Better approach: "I need to choose between Xero and MYOB for a 6-person tradies business in Queensland that I'm onboarding next month. My priorities are: ease of use for a team with no bookkeeping background, mobile access for field staff doing receipts capture, and integration with their existing quoting tool ServiceM8. Compare these two options against those three criteria in a table. Then give me a direct recommendation — not a list of pros and cons, an actual call on which one to put this client on."

The second prompt gives Claude the context (6-person tradies business, Queensland), the explicit criteria (ease of use, mobile access, ServiceM8 integration), the format (comparison table), and a clear instruction about the output type (recommendation, not pros and cons list). Claude responds in kind.

### Turning analysis into a decision

The difference between a summary and a useful analysis is a recommendation.

Claude defaults to balanced. Ask it to compare two things and it will tell you what's good and bad about both. This is useful — but often not enough. You need a call. You need to know what to actually do for the client in front of you.
...
One more technique: tell Claude the decision has to be made. "I need to choose one of these external bookkeepers by end of week. Based only on what I've pasted in, which one gives me the strongest case?" The time pressure isn't information Claude uses technically, but framing the task as a real decision with stakes focuses the output in a useful way.

After Claude gives a recommendation, push on it. Ask: "What's the strongest argument against this recommendation?" "What information would change your call?" "What am I most likely to regret about this choice in six months — say, if FY26 BAS volume doubles?" These follow-up questions surface the limits of the analysis and give you a more complete picture before you decide.

### A word on confidential material

If the material you need to analyse is commercially sensitive — a client's engagement letter, a client's management accounts, staff performance notes — you have two options.

First, check your firm's policy on pasting confidential material into any AI tool. This is not unique to Claude. The TPB and your professional body (CPA, CA ANZ, IPA) all expect you to handle client information appropriately. If there's no policy at {{businessName}}, consider making one before you rely on this workflow.

Second, know that you can paraphrase or anonymise the material before pasting. Replace client names with "Client A" and "Client B." Replace individual names with roles. Replace specific dollar amounts with ranges. Replace ABNs and TFNs with placeholders. Claude can still do meaningful analysis on paraphrased material — the analytical patterns don't disappear when you remove identifying details.
