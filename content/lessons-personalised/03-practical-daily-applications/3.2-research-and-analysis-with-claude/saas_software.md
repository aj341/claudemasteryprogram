...
**Synthesising information you provide.** Paste in multiple documents, emails, or datasets and ask Claude to draw them together into a coherent picture. Claude holds everything in the conversation in working memory and can cross-reference it. This is where its large context window earns its keep — a Gong transcript plus a churn survey plus a Linear feature request, side by side.

**Comparing options against criteria.** Give Claude a shortlist — three SaaS vendors, two analytics tools, four candidate replies to a sensitive customer escalation — along with the criteria that matter to you, and ask it to compare them. Claude produces structured comparisons consistently when you give it structured inputs.

**Analysing documents you paste in.** Master service agreements, vendor proposals, terms and conditions, security questionnaires, survey results. Ask Claude to read and work with the document — extract key terms, flag unusual clauses, summarise the main obligations, note anything missing relative to what you'd expect to see.

**Summarising long material.** A long email thread, a 60-minute Gong transcript, a consultant's audit deliverable. Paste it in and ask for a summary shaped around what you need from it — the decisions, the risks, the action items. Claude reads the whole thing and produces what you ask for.

**Asking analytical questions about material you provide.** "Given everything I've pasted in, what's the strongest argument for choosing Vendor A?" "What's missing from this proposal relative to what we asked for?" "What does this churn-survey pattern suggest about our onboarding flow?" These are questions Claude handles well — because the material is in front of it and it can reason across it.

### What Claude is not good at for research and analysis

This matters as much as the previous section, so read it carefully.

**Finding live information.** Claude cannot check current prices, verify whether a SaaS competitor is still operating, look up today's exchange rates, or confirm that a regulation is still current. If you ask and Claude answers, it's drawing on training data — which has a cutoff date. Anything time-sensitive needs a current source.

**Knowing your specific category's current pricing or market conditions.** "What should an indie B2B tool charge per seat in AU right now?" is a question Claude will attempt — but its answer is based on training data, not the current market. Use Claude to structure the analysis once you have the numbers. Get the numbers from a current source — ProfitWell, OpenView's benchmarks.

**Anything requiring real-time data.** Today's MRR, live signup count, current support-queue depth, today's status-page incidents, this morning's churn. Not Claude's territory.

**Verifying facts that weren't in the material you provided.** If you paste in a partner agreement and ask whether the indemnity clause is standard for AU SaaS contracts, Claude can comment on what it knows generally. But "generally" is not "currently in your specific market." For anything with legal or financial consequences, the analysis Claude provides is a starting point, not a conclusion.

The practical rule, {{firstName}}: if accuracy matters and the information is time-sensitive or highly specific to your market, use Claude to structure and analyse — but verify the facts from a current, authoritative source.

### The "bring your own material" workflow

This is the most valuable pattern in this lesson. It has four steps.

**Step 1: Gather your raw material.** This is the part Claude can't do for you. Find the proposals, the survey responses, the contract, the emails, the Gong transcripts. If the material is confidential, you can summarise or paraphrase it before pasting — Claude doesn't require verbatim text, only enough detail to work with.

**Step 2: Decide what question you need answered.** Before you open Claude, be clear on the decision or output you need. "Which vendor should I choose?" "What are the key risks in this DPA?" "What patterns appear in these churn responses?" The question shapes the prompt.
...
```
Below are proposals from three customer-support tools we're evaluating to
replace our current helpdesk. I need to choose one. My priorities, in order,
are: response-time on inbound (we're a 14-person SaaS, support is one
person plus on-call), depth of integration with our product (webhooks, our
session data), and price (lowest cost is not the goal — value for money is).

For each tool, summarise what the proposal tells me about each of my three
priorities. Then produce a comparison table: rows are tools, columns are my
three priorities, cells are brief assessments. Finish with a short paragraph
identifying the strongest option based on this information only.

[Vendor proposals pasted below]
```
...
Comparison prompts are one of the highest-value uses of Claude in a small SaaS context. Here's what makes them work.
...
**State your evaluation criteria explicitly.** Don't make Claude guess what matters to you. "I care about price, time-to-value, and whether they've worked with B2B SaaS at our scale" is far more useful than "help me compare these." Claude will weight the criteria you name — so name the right ones.
...
Mistake: "Compare these two analytics tools for me." Claude produces two paragraphs of balanced description that don't help you decide anything.

Better approach: "I need to choose between Mixpanel and Amplitude for our 12-person SaaS in Sydney. My priorities are: ease of use for a non-data-trained PM team, time-to-first-funnel, and a free tier that fits our current 18,000 MAU. Compare these two against those three criteria in a table. Then give me a direct recommendation — not a list of pros and cons, an actual call on which one to choose for this situation."

The second prompt gives Claude the context (12-person SaaS, Sydney), the explicit criteria (ease of use, time-to-first-funnel, free-tier fit), the format (comparison table), and a clear instruction about the output type (recommendation, not pros and cons list). Claude responds in kind.
...
If the material you need to analyse is commercially sensitive — a contract with a strategic account, internal MRR figures, support-ticket content tied to a named user — you have two options.

First, check your team's policy on pasting confidential material into any AI tool. This is not unique to Claude. If there's no policy, consider making one before you rely on this workflow.
