...
```
Below are quotes from three commercial laundry suppliers for {{businessName}}'s
clinic linen and towel service. I need to choose one supplier. My priorities, in
order, are: reliability (they collect and return on schedule), communication
(they respond quickly when something is wrong), and price (lowest cost is not
the goal — value for money is, and infection-control standards are non-negotiable).

For each supplier, summarise what the quote tells me about each of my three
priorities. Then produce a comparison table: rows are suppliers, columns are
...
**State your evaluation criteria explicitly.** Don't make Claude guess what matters to you. "I care about price, turnaround time, and whether they've worked with allied health practices before" is far more useful than "help me compare these." Claude will weight the criteria you name — so name the right ones.
...
Mistake: "Compare these two practice management options for me." Claude produces two paragraphs of balanced description that don't help you decide anything.

Better approach: "I need to choose between Cliniko and Nookal for {{businessName}}, a 6-clinician allied health practice in Queensland. My priorities are: ease of use for staff with no software background, online booking and SMS reminders, and integration with HICAPS for on-the-spot health-fund claiming. Compare these two options against those three criteria in a table. Then give me a direct recommendation — not a list of pros and cons, an actual call on which one to choose for this situation."

The second prompt gives Claude the context (6-clinician allied health practice, Queensland), the explicit criteria (ease of use, online booking and SMS, HICAPS integration), the format (comparison table), and a clear instruction about the output type (recommendation, not pros and cons list). Claude responds in kind.
...
If the material you need to analyse is commercially sensitive — a service agreement with a corporate-wellness client, internal financials, staff performance notes — you have two options.
