...
**Knowing your specific industry's current pricing or market conditions.** "What should a Year 11 maths tutor charge per session in Adelaide?" is a question Claude will attempt — but its answer is based on training data, not the current market. Use Claude to structure the analysis once you have the numbers. Get the numbers from a current source.
...
```
Below are proposals from three relief-tutor agencies we could use to cover
sick days at our service. I need to choose one. My priorities, in order, are:
reliability (they show up when scheduled), communication (they respond
quickly when something is wrong), and price (lowest cost is not the goal —
value for money is).

For each agency, summarise what the proposal tells me about each of my three
priorities. Then produce a comparison table: rows are agencies, columns are
...
Better approach: "I need to choose between Cliniko and SimplyBook for a 6-tutor education service in Queensland. My priorities are: ease of use for a team with no booking-software background, mobile access for tutors on the road, and integration with our payment processor, Stripe. Compare these two options against those three criteria in a table. Then give me a direct recommendation — not a list of pros and cons, an actual call on which one to choose for this situation."

The second prompt gives Claude the context (6-tutor education service, Queensland), the explicit criteria (ease of use, mobile access, Stripe integration), the format (comparison table), and a clear instruction about the output type (recommendation, not pros and cons list). Claude responds in kind.
