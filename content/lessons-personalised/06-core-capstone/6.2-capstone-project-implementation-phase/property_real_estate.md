...
In 6.1 you wrote a design document. It described the business problem you were solving inside {{businessName}}, the Project you would build, the knowledge documents you would upload, and the prompts you planned to write. That document is your brief for this lesson.
...
That said, not all designs survive contact with reality. You may find that the listing template you planned to upload is out of date and reflects last year's compliance wording. You may find that the prompt structure you sketched in 6.1 produces vendor updates that don't match how your principal actually writes. You may realise the problem you identified is slightly different from the problem that surfaces when you start building. All of this is normal, and none of it is a problem with your design.
...
Open Claude, create a new Project, and name it clearly. Then write the custom instructions. These instructions are the system-level behaviour rules — who Claude is in this context, what it knows about the agency, how it should respond, and what it should never do. Instructions are not a welcome message. They are constraints.
...
A useful instruction set for a capstone solution will typically include: the agency name and what it does, the specific context Claude should always have (vendor profile, sales versus property management focus, tone expectations), the format Claude should default to in this Project, and at least one explicit constraint — something Claude should not do, or should always check before doing.

```
Business: Hartley Property, a 10-person regional NSW agency handling
sales and property management across two coastal towns. Average rent roll: 220 properties.
Context: All outputs in this Project are for vendor communications,
buyer follow-up, or owner reporting. Vendors are mostly retirees
selling family homes; owners are mostly mum-and-dad investors with
one or two properties.
Tone: Plain, specific, no real estate cliches. Write to a vendor at
their kitchen table, not a property investor magazine. No "stunning",
"prestigious", or "must inspect".
Format: Use bullet points for owner reports and OFI summaries.
Use plain paragraphs for vendor emails. Never use tables in vendor
communications unless explicitly asked.
Constraint: Never state specific price guides, days on market, or
buyer numbers without noting they should be verified against the
CRM and current campaign notes before sending.
```
...
With instructions in place, add the knowledge your Project needs. These might be documents you already have — your standard agency agreement template, three strong past listings, a one-page tone document, the agency's standard owner statement layout — or short documents you write now to give Claude the context it can't get anywhere else.
...
When you test, you are checking two things: output quality and system integration. Output quality means: does this prompt produce the kind of vendor update or owner report I need? System integration means: does this prompt produce the right response when running inside the Project context — with the custom instructions active and the knowledge documents loaded?

Those are different tests. A prompt that passes output quality in a fresh conversation can still fail system integration. You'll know when this happens because the output in the Project feels subtly wrong — not broken, but off. The tone doesn't match, the format ignores an instruction, or Claude draws on knowledge from the wrong domain — for example, writing a buyer's agent shortlist memo in the voice of a selling agent's listing presentation.
...
Things will break. Not dramatically — usually quietly. The vendor update is plausible but generic. The format on the owner report is not quite right. Claude is helpful but not specific enough about the actual buyer feedback you captured at the OFI. These are the mid-build failures most students encounter, and they are all diagnosable.

**Failure type 1: The prompt is too short for the system context.**

When you write a short prompt inside a Project, Claude fills the remaining space with its instructions. Sometimes that's useful. Sometimes the instruction set is too broad for the specific task, and Claude produces an output that is technically compliant with the instructions but not right for this particular prompt. The fix: lengthen the prompt. Add context that's specific to this task — the suburb, the campaign week, the vendor's main concern — not already covered in the instructions. The more specific the prompt, the less room Claude has to fill with general instruction-following behaviour.

**Failure type 2: The knowledge document is too long or unstructured.**

Claude reads knowledge documents in context. If a document is 4,000 words of unformatted prose — a strata management agreement copy-pasted in full, for example — Claude will extract from it inconsistently. What it surfaces depends on what the prompt asks, and sometimes the relevant clause gets buried. The fix: either restructure the document (break it into headed sections so Claude can navigate it more reliably) or split it into multiple shorter, focused documents. A short, well-structured document retrieves better than a long, unfocused one.

**Failure type 3: The instructions contain an ambiguity that surfaces only in a specific prompt.**

Instructions are written in advance and can't anticipate every prompt scenario. You'll occasionally find that a prompt triggers an instruction in a way that produces an unintended result — for example, your "no tables in vendor communications" rule fires inside a CMA where a comparable sales table is genuinely the best format. The fix: add a specific clause to the instructions that covers this case, or add an override instruction directly in the prompt ("For this CMA, use a comparable sales table for the recent settlements section only — keep the rest as paragraphs"). Prompt-level instructions override system instructions for that response.
...
- The version of something you replaced (even if it's just "first version of Prompt 1 — too generic, vendor update read like portal copy")
...
The evaluation phase in 6.3 will ask you to assess whether your solution works. That assessment is only credible if you have records of how it behaved during the build. "It worked" is not an evaluation. "I tested the post-OFI vendor update prompt 4 times, adjusted the format instruction twice, and the fourth output was the one I'd send to a vendor without editing" is an evaluation.

### You are 60 minutes from a working solution

This is the most demanding lesson in the course. It is not demanding because the concepts are complicated — you have everything you need from Modules 1 to 5. It is demanding because you are doing real work. You are building something that has to work in a live agency, on real campaigns, not a practice exercise.

That matters. The difference between a practice exercise and a real build is exactly the pressure that surfaces the problems worth solving. A practice exercise works the first time because it is designed to. A real build requires you to adapt — to the vendor who reads every word twice, to the owner who only checks the maintenance section, to the buyer who emails at 9pm on a Sunday. The adaptation is the point.

When you finish this lesson, {{firstName}}, you will have a working Claude solution configured for a specific problem inside your business. That solution will have custom instructions you wrote, a knowledge base you assembled, and prompts you tested and refined. It will not be perfect. It will be real.
