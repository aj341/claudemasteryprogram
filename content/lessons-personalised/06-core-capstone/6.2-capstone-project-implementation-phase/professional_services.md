...
Open Claude, create a new Project, and name it clearly. Then write the custom instructions. These instructions are the system-level behaviour rules — who Claude is in this context, what it knows about your firm, how it should respond, and what it should never do. Instructions are not a welcome message. They are constraints.
...
```
Practice: Hartley Advisory, a 4-person operations consultancy serving 18 mid-sized AU/NZ clients across professional services and logistics.
Context: All outputs in this Project are for client communications or internal engagement reviews.
Tone: Plain, specific, no jargon. Write to a managing director, not a Big-4 partner.
Format: Use bullet points for summaries. Use plain paragraphs for client emails. Never use tables unless explicitly asked.
Constraint: Never state specific fee figures or deliverable dates without noting that they should be verified against the engagement letter before sending.
```
...
With instructions in place, add the knowledge your Project needs. These might be documents you already have — a service description, past client examples, a fee guide, a tone document — or short documents you write now to give Claude the context it can't get anywhere else.
...
This is the most demanding lesson in the course. It is not demanding because the concepts are complicated — you have everything you need from Modules 1–5. It is demanding because you are doing real work. You are building something that has to work in your actual practice, not a practice exercise.

That matters, {{firstName}}. The difference between a practice exercise and a real build is exactly the pressure that surfaces the problems worth solving. A practice exercise works the first time because it is designed to. A real build requires you to adapt. The adaptation is the point.

When you finish this lesson, you will have a working Claude solution configured for a specific problem you identified yourself at {{businessName}}. That solution will have custom instructions you wrote, a knowledge base you assembled, and prompts you tested and refined. It will not be perfect. It will be real.
