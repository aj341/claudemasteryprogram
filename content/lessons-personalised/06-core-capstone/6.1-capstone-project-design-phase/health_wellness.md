...
{{firstName}}, you have completed thirty lessons. You have built a prompt engineering practice, a Projects infrastructure, and a set of extended capabilities. Every lesson up to this point gave you a bounded exercise — a specific skill to practise, a specific output to produce. The curriculum decided what you worked on.
...
**Is it specific?** Can you name exactly what is broken or slow or inconsistent? Not "my quoting process is inefficient" but "I spend 45 minutes per treatment-plan summary manually pulling findings from intake notes, calculating session counts and gap fees, and formatting the output — and I send 6–8 plans a week from {{businessName}}." The second version names the task, the time cost, the frequency, and the friction point. That is specific.
...
- Treatment-plan drafting time reduced from 45 minutes to under 15 minutes
- Client summary emails score 4 out of 5 or higher in a self-review against the checklist I defined
- All weekly referrer-update letters produced with a consistent structure — zero missing sections — over four weeks
...
Use: Upload intake-form PDFs at the start of each plan-writing session.
Claude reads the document and extracts the relevant findings (presenting
condition, prior history, work demands, goals) before I write the prompt
for the treatment-plan summary. This replaces the manual reading and
note-taking step that currently takes 15–20 minutes per plan.
```
...
Step 1: Create a Project for treatment plans
Step 2: Add prompts
Step 3: Test with a real plan
Step 4: Refine

SPECIFIC PLAN (aim for this):
Step 1: Create a Treatment-Plan Project in Claude. Upload the two
  intake-form PDFs I use most often. Write the Project instructions
  to define Claude's role as a clinical drafting assistant who knows
  {{businessName}}'s standard episode-of-care structure (4–8 sessions)
  and gap-fee defaults. (~15 mins)

Step 2: Write the extraction prompt template — the prompt I will use
  to pull findings from a newly uploaded intake form at the start of
  each plan-writing session. (~20 mins)

Step 3: Write the plan-summary drafting prompt template — the prompt
  that takes the extracted findings plus the client's goals and produces
  a formatted plan draft in our standard layout. (~20 mins)

Step 4: Run the full workflow on a real plan from this week.
