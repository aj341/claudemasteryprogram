...
**Is it specific?** Can you name exactly what is broken or slow or inconsistent? Not "my enrolment process is inefficient" but "I spend 45 minutes per enrolment proposal manually pulling student details from intake forms, calculating term packages, and formatting the output — and I send 6–8 proposals a week." The second version names the task, the time cost, the frequency, and the friction point. That is specific.
...
- Enrolment proposal drafting time reduced from 45 minutes to under 15 minutes
- Parent summary emails score 4 out of 5 or higher in a self-review against the checklist I defined
...
Use: Upload parent intake forms

GOOD ENTRY:
Capability: File uploads — Module 5, Lesson 5.2
Use: Upload parent intake form PDFs at the start of each enrolment proposal
session. Claude reads the document and extracts the details I need
(year level, target subjects, prior tutoring history, parent priorities)
before I write the prompt for the proposal. This replaces the manual
reading and note-taking step that currently takes 15–20 minutes per
proposal.
```
...
Step 1: Create a Project for enrolments
Step 2: Add prompts
Step 3: Test with a real proposal
Step 4: Refine

SPECIFIC PLAN (aim for this):
Step 1: Create an Enrolment Project in Claude. Upload the two intake
  form PDFs I use most often. Write the Project instructions to define
  Claude's role as an enrolment assistant who knows my term package
  prices and standard terms. (~15 mins)

Step 2: Write the extraction prompt template — the prompt I will use
  to pull student details from a newly uploaded intake form at the
  start of each enrolment session. (~20 mins)

Step 3: Write the proposal drafting prompt template — the prompt that
  takes the extracted details plus parent requirements and produces
  a formatted proposal draft in my standard layout. (~20 mins)

Step 4: Run the full workflow on a real enrolment from this week.
