...
You have completed thirty lessons, {{firstName}}. You have built a prompt engineering practice, a Projects infrastructure, and a set of extended capabilities. Every lesson up to this point gave you a bounded exercise — a specific skill to practise, a specific output to produce. The curriculum decided what you worked on.

Module 6 is different. You decide what you work on.

The capstone is a four-part project, spread across Lessons 6.1 through 6.4. You pick one real challenge in {{businessName}} and build a complete Claude-assisted solution for it. Not a lesson exercise — an actual piece of work that matters on the tools and in the office. This lesson, 6.1, is the design phase. You map the problem, define the solution approach, name the tools you will use, and plan the implementation before touching Claude.
...
Most students arrive at Lesson 6.1 with one of two problems. The first is choosing something too vague: "I want to tighten up the way we handle homeowner comms" or "I want to use Claude more on the admin side." These are directions, not challenges. You cannot build a solution for a direction. You cannot measure whether it worked.

The second is choosing something too large: "I want to handle every piece of post-job admin without touching it" or "I want Claude to run our entire quoting workflow." These are projects that would take months with a full office team. A capstone that cannot be completed in the time you have is a capstone that will fail.
...
**Is it specific?** Can you name exactly what is broken or slow or inconsistent? Not "our quoting is a mess" but "I spend 40 minutes per quote pulling specs from supplier PDFs, working out a margin, and writing it up in our format — and we send 8–10 quotes a week." The second version names the task, the time cost, the frequency, and the friction point. That is specific.

**Is it significant?** Would solving this make a real difference — in time saved, quality improved, or consistency increased? A challenge that saves five minutes once a month is not worth a four-lesson capstone. A challenge that saves two hours a week, or removes a job you have been avoiding after dinner, or produces consistently better quotes for homeowners — that is worth the work.

**Is it achievable with your current toolkit?** The Foundation tier is prompts, Projects, file processing, and vision. If your solution requires the Claude API, third-party automation through ServiceM8 or simPRO, or code, it is outside scope for this capstone. That does not mean those things are impossible — it means they belong in a future project, after this course. Keep the scope inside what you have built.
...
- Quote drafting time reduced from 40 minutes to under 15 minutes
- End-of-job wrap-up emails score 4 out of 5 or higher in a self-review against the checklist I defined
- All weekly site updates produced with a consistent structure — zero missing sections — over four weeks

Poor success criteria:
- Homeowners are happier (not measurable without a survey or system)
...
Use: Upload supplier spec sheet PDFs at the start of each quote session.
Claude reads the document and pulls the specs I need (model, capacity,
warranty, lead time) before I write the prompt for the quote. This
replaces the manual reading and note-taking step that currently takes
15–20 minutes per quote.
```

The good entry names the specific document type, the specific output Claude pulls out, and the specific step it replaces. That is a capability map entry that tells you exactly how the build should run when you get to Lesson 6.2.
...
Step 1: Create a Quoting Project in Claude. Upload the two supplier
  spec PDFs we use most often. Write the Project instructions to
  define Claude's role as a quoting assistant who knows our margin
  targets (28%) and standard inclusions. (~15 mins)

Step 2: Write the extraction prompt template — the prompt I will use
  to pull specs from a newly uploaded supplier PDF at the start of
  each quoting session. (~20 mins)

Step 3: Write the quote drafting prompt template — the prompt that
  takes the extracted specs plus the homeowner's scope and produces
  a quote draft in our standard layout. (~20 mins)

Step 4: Run the full workflow on a real quote from this week.
  Document what works and what needs adjustment. (~30 mins)

Step 5: Revise both prompts based on the test. Lock the workflow
...
The strongest capstone projects in this course share one thing: specificity from the start. The students who produce the sharpest 6.3 evaluations are the ones who wrote the sharpest design documents in 6.1. The challenge is named precisely. The success criteria are measurable. The capability map explains why, not just what. The implementation plan is specific enough to follow.
