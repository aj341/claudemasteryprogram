...
Most founders arrive at Lesson 6.1 with one of two problems. The first is choosing something too vague: "I want to improve my customer communications" or "I want to use Claude more in my SaaS." These are directions, not challenges. You cannot build a solution for a direction. You cannot measure whether it worked.

The second is choosing something too large: "I want to automate my entire onboarding flow" or "I want Claude to handle my support queue." These are projects that would take months with a team. A capstone that cannot be completed in the time you have is a capstone that will fail.

The right challenge sits in a specific middle ground. It is concrete enough to describe in two or three sentences. It is significant enough that solving it produces a real result for your SaaS — not a nice-to-have, but a genuine improvement in time, quality, or consistency. And it is achievable with the Foundation tier toolkit: prompts, Projects, and extended capabilities. No APIs, no external automations, no engineer dependencies.

Here is the test to apply to any challenge you are considering. Answer all three:

**Is it specific?** Can you name exactly what is broken or slow or inconsistent? Not "my release-note process is inefficient" but "I spend 45 minutes per release manually pulling change descriptions from Linear, summarising them, and formatting the customer-facing post — and I ship 6–8 releases a quarter." The second version names the task, the time cost, the frequency, and the friction point. That is specific.

**Is it significant?** Would solving this make a real difference — in time saved, quality improved, or consistency increased? A challenge that saves five minutes once a month is not worth a four-lesson capstone. A challenge that saves two hours a week, or removes a task you have been avoiding, or produces consistently better outputs for customers — that is worth the work.

**Is it achievable with your current toolkit?** The Foundation tier is prompts, Projects, file processing, and vision. If your solution requires the Claude API, third-party automation, or production code, it is outside scope for this capstone. That does not mean those things are impossible — it means they belong in a future project, after this course. Keep the scope inside what you have built.

### Defining your objectives and success criteria

Before you decide how to build anything, define what success looks like. This sounds obvious. Almost no founder does it before they start.

Success criteria serve two functions in this capstone. First, they constrain your solution design — if you know you need to reduce release-note time from 45 minutes to under 15, you design a solution aimed at that specific target rather than building something vague and hoping it helps. Second, they give you a clear answer in Lesson 6.3 when you evaluate whether the solution worked. Without criteria, evaluation is just an opinion.
...
- Release-note drafting time reduced from 45 minutes to under 15 minutes
- Customer-success summary emails score 4 out of 5 or higher in a self-review against the checklist I defined
- All weekly investor updates produced with a consistent structure — zero missing sections — over four weeks

Poor success criteria:
- Customers are more satisfied (not measurable without a survey or NPS pull)
...
More importantly, the capability map should explain the logic. Not "I will use a Project from Module 4" but "I will use a Project from Module 4 to hold the feature changelog and customer-pain-point notes that Claude needs to reference in every release-note session — without it, I would have to re-paste this context each time." The capability is justified by the problem, not just listed.
...
Use: Upload Linear exports

GOOD ENTRY:
Capability: File uploads — Module 5, Lesson 5.2
Use: Upload the fortnight's Linear ticket export at the start of each
release-note session. Claude reads the export and extracts the items
I need (shipped feature, who asked for it, customer-facing impact)
before I write the prompt for the customer post. This replaces the
manual reading and note-taking step that currently takes 15–20
minutes per release.
```
...
Step 1: Create a Project for release notes
Step 2: Add prompts
Step 3: Test with a real release
Step 4: Refine

SPECIFIC PLAN (aim for this):
Step 1: Create a Release Notes Project in Claude. Upload the last
  three release notes I was happy with as voice-and-format examples.
  Write the Project instructions to define Claude's role as a release-
  note assistant who knows our pricing tiers and standard tone. (~15 mins)

Step 2: Write the extraction prompt template — the prompt I will use
  to pull shipped items from a newly uploaded Linear export at the
  start of each release-note session. (~20 mins)

Step 3: Write the release-note drafting prompt template — the prompt
  that takes the extracted items plus customer-pain context and
  produces a formatted release note in our standard layout. (~20 mins)

Step 4: Run the full workflow on a real release from this fortnight.
...
That four-part structure is the closest this course gets to real project work, because it is real project work. Not a simulation, not a case study — an actual solution you designed and built for your actual SaaS, using a toolkit you spent five modules developing.

The best capstone projects in this course share one characteristic: specificity from the start. The founders who produce the strongest 6.3 evaluations are the ones who wrote the sharpest design documents in 6.1. The challenge is named precisely. The success criteria are measurable. The capability map explains why, not just what. The implementation plan is specific enough to follow.
