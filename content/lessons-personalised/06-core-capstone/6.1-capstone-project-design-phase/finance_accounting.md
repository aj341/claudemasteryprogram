...
Most accountants arrive at Lesson 6.1 with one of two problems. The first is choosing something too vague: "I want to improve my client communications" or "I want to use Claude more in my practice." These are directions, not challenges. You cannot build a solution for a direction. You cannot measure whether it worked.

The second is choosing something too large: "I want to automate my entire onboarding process" or "I want Claude to handle my year-end client comms." These are projects that would take months with a team. A capstone that cannot be completed in the time you have is a capstone that will fail.

The right challenge sits in a specific middle ground. It is concrete enough to describe in two or three sentences. It is significant enough that solving it produces a real result for your practice — not a nice-to-have, but a genuine improvement in time, quality, or consistency. And it is achievable with the Foundation tier toolkit: prompts, Projects, and extended capabilities. No APIs, no external automations, no developer dependencies.

Here is the test to apply to any challenge you are considering. Answer all three:

**Is it specific?** Can you name exactly what is broken or slow or inconsistent? Not "my fee proposal process is inefficient" but "I spend 45 minutes per fee proposal manually pulling service descriptions from our master document, calculating fees against our tier table, and formatting the output — and I send 6–8 proposals a week." The second version names the task, the time cost, the frequency, and the friction point. That is specific.
...
Before you decide how to build anything, define what success looks like. This sounds obvious. Almost no accountants do it before they start.
...
- All quarterly client reports produced with a consistent structure — zero missing sections — over four quarters
...
More importantly, the capability map should explain the logic. Not "I will use a Project from Module 4" but "I will use a Project from Module 4 to hold the service descriptions and fee tier table that Claude needs to reference in every fee-proposal session — without it, I would have to re-paste this context each time." The capability is justified by the problem, not just listed.
...
Use: Upload a client's most recent management accounts (PDF) at the
start of each fee-proposal session. Claude reads the document and
extracts the indicators I need (turnover, employee count, transaction
volume, current accounting platform) before I write the prompt for
the proposal. This replaces the manual reading and note-taking step
that currently takes 15–20 minutes per proposal.
```
...
Step 1: Create a Project for fee proposals
Step 2: Add prompts
Step 3: Test with a real proposal
Step 4: Refine

SPECIFIC PLAN (aim for this):
Step 1: Create a Fee Proposal Project in Claude. Upload our service
  descriptions document and our 3-tier fee table. Write the Project
  instructions to define Claude's role as a proposal-drafting assistant
  who knows our scope inclusions and our standard engagement terms. (~15 mins)

Step 2: Write the extraction prompt template — the prompt I will use
  to pull client indicators from a newly uploaded set of management
  accounts at the start of each proposal session. (~20 mins)

Step 3: Write the proposal drafting prompt template — the prompt that
  takes the extracted indicators plus the discovery-call notes and
  produces a formatted fee proposal draft in our standard layout. (~20 mins)
...
That four-part structure is the closest this course gets to real practice work, because it is real practice work. Not a simulation, not a case study — an actual solution you designed and built for your actual practice, using a toolkit you spent five modules developing.

The best capstone projects in this course share one characteristic: specificity from the start. The accountants who produce the strongest 6.3 evaluations are the ones who wrote the sharpest design documents in 6.1. The challenge is named precisely. The success criteria are measurable. The capability map explains why, not just what. The implementation plan is specific enough to follow.
