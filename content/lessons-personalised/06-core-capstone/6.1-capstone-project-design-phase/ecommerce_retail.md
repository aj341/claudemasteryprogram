...
Module 6 is different. You decide what you work on at {{businessName}}.

The capstone is a four-part project, spread across Lessons 6.1 through 6.4. You pick one real retail challenge and build a complete Claude-assisted solution for it. Not a lesson exercise — an actual piece of work that matters to your store. This lesson, 6.1, is the design phase. You map the problem, define the solution approach, name the tools you will use, and plan the implementation before touching Claude.
...
Most retailers arrive at Lesson 6.1 with one of two problems. The first is choosing something too vague: "I want to improve my customer comms" or "I want to use Claude more in the store." These are directions, not challenges. You cannot build a solution for a direction. You cannot measure whether it worked.

The second is choosing something too large: "I want to automate my entire customer-service inbox" or "I want Claude to handle all my product copy." These are projects that would take months with a team. A capstone that cannot be completed in the time you have is a capstone that will fail.

The right challenge sits in a specific middle ground. It is concrete enough to describe in two or three sentences. It is significant enough that solving it produces a real result for your store — not a nice-to-have, but a genuine improvement in time, quality, or consistency. And it is achievable with the Foundation tier toolkit: prompts, Projects, and extended capabilities. No APIs, no external automations, no developer dependencies.

Here is the test to apply to any challenge you are considering, {{firstName}}. Answer all three:

**Is it specific?** Can you name exactly what is broken or slow or inconsistent? Not "my customer-service replies are inefficient" but "I spend 45 minutes a day replying to 'where's my order' and 'when will it ship' tickets that all need a similar tone, the right tracking information, and a check on the freight ETA — and we get 12–18 of these a day during normal weeks." The second version names the task, the time cost, the frequency, and the friction point. That is specific.
...
- Customer-service drafting time reduced from 45 minutes to under 15 minutes per day
- Restock-alert emails score 4 out of 5 or higher in a self-review against the brand-voice checklist I defined
- All weekly returns-summary notes produced with a consistent structure — zero missing sections — over four weeks

Poor success criteria:
- Customers are happier (not measurable without a survey or system)
...
More importantly, the capability map should explain the logic. Not "I will use a Project from Module 4" but "I will use a Project from Module 4 to hold the brand voice guide, returns policy, and standard shipping rules that Claude needs to reference in every customer-service session — without it, I would have to re-paste this context each time." The capability is justified by the problem, not just listed.
...
Use: Upload supplier specification PDFs at the start of each new-SKU
listing session. Claude reads the document and extracts the specifications
I need (material, dimensions, care instructions, country of origin) before
I write the prompt for the product description. This replaces the manual
reading and note-taking step that currently takes 15–20 minutes per SKU.
```
...
Step 1: Create a Project for customer service
Step 2: Add prompts
Step 3: Test with a real ticket
Step 4: Refine

SPECIFIC PLAN (aim for this):
Step 1: Create a Customer Service Project in Claude. Upload my returns
  policy, shipping FAQ, and top-20-SKU details document. Write the
  Project instructions to define Claude's role as a CS assistant who
  knows our warm-but-direct tone and never invents dispatch dates or
  tracking statuses. (~15 mins)

Step 2: Write the "where's my order" reply prompt template — the prompt
  I will use when a customer chases an order. Includes the order details
  the casual will paste in. (~20 mins)

Step 3: Write the returns-acknowledgement prompt template — the prompt
  that takes the return reason plus order details and produces a warm
  reply that confirms next steps and remedies under Australian Consumer
  Law. (~20 mins)

Step 4: Run the full workflow on five real customer tickets from this
  week. Document what works and what needs adjustment. (~30 mins)
...
That four-part structure is the closest this course gets to real project work, because it is real project work. Not a simulation, not a case study — an actual solution you designed and built for your actual store, using a toolkit you spent five modules developing.

The best capstone projects in this course share one characteristic: specificity from the start. The retailers who produce the strongest 6.3 evaluations are the ones who wrote the sharpest design documents in 6.1. The challenge is named precisely. The success criteria are measurable. The capability map explains why, not just what. The implementation plan is specific enough to follow.
