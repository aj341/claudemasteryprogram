...
I run a 6-person accounting firm in regional Victoria called Riverbend
Accounting. We work with small business clients across hospitality,
trades, and primary production — sole traders through to 15-staff Pty Ltds.
Our core services are FY accounts, tax returns, BAS, and quarterly
advisory meetings. Two of our six staff are registered tax agents.

My ideal clients are owner-operators with $500k+ turnover who want a
trusted adviser, not a once-a-year tax processor. We don't compete on
price. Our voice is warm, plain-language, and direct — not Big-4 audit.
Australian English always.

Current focus: tightening scope on existing engagements before FY26
starts and onboarding two new hospitality clients sourced via referral.

---

Message 2 — your task prompt:

Draft an email to a regional cafe group that has just opened their
third location. We've worked with them since 2023. The email should
congratulate them on the new site, gently flag that the new site changes
their BAS workings (we'll need to talk through GST grouping options),
and ask for a 20-minute call — not a full meeting. Tone: warm, plain,
not formal. Under 180 words.
```
...
Notice what the context document in Message 1 does that the task prompt in Message 2 doesn't have to. It establishes the voice ("warm, plain-language, and direct — not Big-4 audit"). It names the ideal client type. It states a current business focus. When Message 2 arrives asking for an email with a "warm, plain, not formal" tone, Claude isn't guessing at what that means for this particular firm — it already has the frame. That's the practical value of the two-message structure: the context message does the briefing work, the task message does the directing work, and neither has to carry both at once.
...
When Claude responds to your task prompt, read it before you do anything else. Don't skim it. Read it as the intended recipient would read it — your client, your team member, the ATO contact officer, whoever the output is for.
...
3. Is it accurate? Are there any facts, figures, section numbers, or statements that need to be corrected?
...
from any accountant. Rewrite just the opening two sentences to reference
their third site opening specifically and our four-year working relationship.
...
Most accountants skip this step. They get a good output, use it, and move on. Then two weeks later they try the same kind of task and start from scratch — because they didn't note what worked. Two minutes of capture is the difference between a one-time win and a repeatable system.

After the session, note three things:

**What the prompt pattern was.** Jot down the structure you used — context first, then task, specific constraint on tone, word count specified. If it worked, you want to be able to reproduce it for the next similar BAS reminder or scope clarification. Keep a simple document — a notes file, a Google Doc, anything you'll actually open again — and paste the structure in. Not the full prompt; just the pattern.

**What was missing from your context document.** If you had to add information in a revision prompt that really should have been there from the start — your preferred tone, a detail about your client base, a TPB-related constraint you always want applied — add it to the context document now, while it's fresh. One good session can improve your context document in ways that make the next ten sessions faster.

**What the next task is.** You have a task map from Lesson 1.4. After a session that worked, it's obvious which task to try next. Name it before you close the session. This keeps momentum and stops the task map from becoming a document you filed and forgot until end-of-FY.
...
If yes — if you would have written that BAS reminder just as quickly, or if the draft Claude produced needs so much editing that you're really rewriting it from scratch — then the prompt needs work. Something was missing that made the output too generic or too far from what you needed. That's a useful diagnosis: the session didn't fail, it told you what to add to the prompt next time.

If no — if Claude produced something you couldn't have drafted that quickly, or something you would have procrastinated on, or something that freed up mental energy for a tax-position memo or a tricky client call — then the session worked. Claude saved you meaningful time.
...
The merged message looks like this: three paragraphs about the practice, then a paragraph about the task, all sent at once. Claude responds with something that partially uses the context and partially ignores it. The accountant rewrites the prompt, adds more context to the task, and sends it again — now with a second copy of the context mixed into the task. By the third attempt they're debugging a tangled message instead of working on the actual lodgement letter.
