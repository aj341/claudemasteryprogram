...
The first five lessons were about building the parts. You learned what Claude is and what it isn't (1.1). You learned to write a prompt that actually specifies what you want (1.2). You audited what you've been doing with AI and where the gaps are in your practice (1.3). You mapped the tasks in your week that Claude is best suited to (1.4). And you wrote a context document — a structured description of {{businessName}} that gives Claude the background it needs to produce useful, relevant output (1.5).

This lesson is about putting all of those parts together in one real session, {{firstName}}.
...
I run a 12-person IT support firm in Geelong called Foreshore IT. We
work with small-business clients across professional services, light
manufacturing, and not-for-profits — 10-staff teams through to 80-staff
operations. Our core services are managed support, cloud migration, and
cyber-uplift work. Two of our twelve staff hold senior security certs.

My ideal clients are owner-operators or operations managers who want a
trusted partner, not a break-fix vendor. We don't compete on hourly
rate. Our voice is warm, plain-language, and direct — not enterprise
sales-speak. Australian English always.

Current focus: tightening scope on existing managed-service agreements
before EOFY and onboarding two new not-for-profit clients sourced via
referral.
...
Draft an email to a regional accounting client that has just opened
their third office. We've supported them since 2023. The email should
congratulate them on the new site, gently flag that the new site changes
their network design (we'll need to walk through VPN and identity
options), and ask for a 20-minute call — not a full meeting. Tone: warm,
plain, not formal. Under 180 words.
```
...
Notice what the context document in Message 1 does that the task prompt in Message 2 doesn't have to. It establishes the voice ("warm, plain-language, and direct — not enterprise sales-speak"). It names the ideal client type. It states a current business focus. When Message 2 arrives asking for an email with a "warm, plain, not formal" tone, Claude isn't guessing at what that means for this particular firm — it already has the frame. That's the practical value of the two-message structure: the context message does the briefing work, the task message does the directing work, and neither has to carry both at once.
...
When Claude responds to your task prompt, read it before you do anything else. Don't skim it. Read it as the intended recipient would read it — your client, your team member, the supplier contact, whoever the output is for.

Then ask yourself three questions:

1. Does this match the task? Did Claude do what I actually asked?
2. Does this sound like my practice? Does the voice, the level of detail, the framing match how we communicate?
3. Is it accurate? Are there any facts, figures, dates, or statements that need to be corrected?
...
from any IT firm. Rewrite just the opening two sentences to reference
their third office opening specifically and our four-year working
relationship. Keep everything else the same.
```
...
Most consultants skip this step. They get a good output, use it, and move on. Then two weeks later they try the same kind of task and start from scratch — because they didn't note what worked. Two minutes of capture is the difference between a one-time win and a repeatable system.

After the session, note three things:

**What the prompt pattern was.** Jot down the structure you used — context first, then task, specific constraint on tone, word count specified. If it worked, you want to be able to reproduce it for the next similar status update or scope clarification. Keep a simple document — a notes file, a Google Doc, anything you'll actually open again — and paste the structure in. Not the full prompt; just the pattern.

**What was missing from your context document.** If you had to add information in a revision prompt that really should have been there from the start — your preferred tone, a detail about your client base, a regulatory constraint you always want applied — add it to the context document now, while it's fresh. One good session can improve your context document in ways that make the next ten sessions faster.

**What the next task is.** You have a task map from Lesson 1.4. After a session that worked, it's obvious which task to try next. Name it before you close the session. This keeps momentum and stops the task map from becoming a document you filed and forgot until end-of-quarter.
...
If yes — if you would have written that status update just as quickly, or if the draft Claude produced needs so much editing that you're really rewriting it from scratch — then the prompt needs work. Something was missing that made the output too generic or too far from what you needed. That's a useful diagnosis: the session didn't fail, it told you what to add to the prompt next time.

If no — if Claude produced something you couldn't have drafted that quickly, or something you would have procrastinated on, or something that freed up mental energy for a recommendation memo or a tricky client call — then the session worked. Claude saved you meaningful time.
...
If the test produces a "probably no difference," that's useful information too. Some tasks genuinely aren't better with Claude. Others are, but only if you bring a specific kind of prompt. The task map from 1.4 was your first pass at identifying which tasks belong where. The sessions you run now will confirm or update that map. By the end of Module 2, you'll have a much clearer picture of which task types consistently save you time in the practice and which ones aren't worth the setup.
...
The merged message looks like this: three paragraphs about the practice, then a paragraph about the task, all sent at once. Claude responds with something that partially uses the context and partially ignores it. The consultant rewrites the prompt, adds more context to the task, and sends it again — now with a second copy of the context mixed into the task. By the third attempt they're debugging a tangled message instead of working on the actual scope letter.
