...
You're not practising here. You're producing an output you can actually send. By the end of this lesson you'll have run a complete Claude session on a meaningful task from your task map at {{businessName}}, and you'll have something tangible to show for it — not a worked example, not a sample, but your own work.
...
I run a 4-person residential electrical business in outer Brisbane called
Stormline Electrical. We work mostly on older homes — switchboard upgrades,
full rewires, EV charger installs, and standard service work. Most jobs
come from repeat customers and referrals.

My typical customer is a homeowner, often older or doing a renovation,
who wants the work explained in plain English without electrical jargon.
We charge fairly and we don't undercut. Our voice is direct, plain, and
never corporate.

Current focus: lifting our quote-acceptance rate by writing better quote
follow-ups. We have eight outstanding quotes this week and the closing
ratio has been sitting around 50%.

---

Message 2 — your task prompt:

Draft a quote follow-up email to a homeowner who hasn't replied in two
weeks. The quote was for a full switchboard upgrade including RCDs and
a meter relocation, around $3,400. The email should reference the
specific job, ask one clear question that makes it easy to reply, and
not pressure her. Tone: direct, friendly, not salesy.
Under 160 words.
```
...
Notice what the context document in Message 1 does that the task prompt in Message 2 doesn't have to. It establishes the voice ("direct, plain, and never corporate"). It names the typical customer. It states a current business focus. When Message 2 arrives asking for a quote follow-up with a "direct, friendly, not salesy" tone, Claude isn't guessing at what that means for this particular electrical business — it already has the frame. That's the practical value of the two-message structure: the context message does the briefing work, the task message does the directing work, and neither has to carry both at once.
...
When Claude responds to your task prompt, read it before you do anything else. Don't skim it. Read it as the intended recipient would read it — the homeowner, the builder, the supplier, whoever the output is for.
...
If the answer to all three is yes, you're done. Send it.
...
from any sparkie. Rewrite just the opening two sentences so they
reference Stormline's focus on switchboard upgrades on older Brisbane
homes. Keep everything else the same.
```
...
Most tradies skip this step. They get a good output, send it, and move on. Then two weeks later they try the same kind of task and start from scratch — because they didn't note what worked. Two minutes of capture is the difference between a one-time win and a repeatable system.

After the session, note three things:

**What the prompt pattern was.** Jot down the structure you used — context first, then task, specific constraint on tone, word count specified. If it worked, you want to be able to reproduce it for the next similar task. Keep a simple document — a notes file on your phone, a Google Doc, anything you'll actually open again — and paste the structure in. Not the full prompt; just the pattern.
...
If yes — if you would have written that quote follow-up just as quickly, or if the draft Claude produced needs so much editing that you're really rewriting it from scratch — then the prompt needs work. Something was missing that made the output too generic or too far from what you needed. That's a useful diagnosis: the session didn't fail, it told you what to add to the prompt next time.

If no — if Claude produced something you couldn't have drafted that quickly, or something you would have left until the weekend, or something that freed up mental energy for a harder task — then the session worked. Claude saved you meaningful time.

That's the target. Not perfection. Not a response you send without reading. Meaningful time saved on a real task. When that happens consistently for the same type of task, you have a working pattern. It belongs in your prompt library and your task map.

Notice that this test isn't about quality in the abstract. A Claude session that produces a mediocre output in three minutes might still be worth running if that output would have taken you fifteen minutes to write and twenty minutes of avoiding the task at the end of a long day on site. The comparison isn't Claude versus perfect — it's Claude versus what you would actually have done without Claude, including the time you'd spend avoiding the task.
