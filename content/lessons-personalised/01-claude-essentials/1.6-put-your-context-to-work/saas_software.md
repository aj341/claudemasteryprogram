...
I'm {{firstName}}, technical co-founder of an 11-person micro-SaaS in
Brisbane called Pacework. We sell a project-billing tool to AU/NZ
indie consultancies — about 380 paying customers on plans from $19 to
$99/month. Stripe billing, Linear product, Pipedrive sales, Intercom
support.

My ideal customers run 2–10 person consulting shops, want clean billing
without setting up Xero from scratch, and value a fast onboarding over
a long feature list. We don't compete on price. Our voice is direct,
warm, and a bit dry — not corporate, not hype.

Current focus: lifting trial-to-paid conversion from 11% to 18% by EOFY,
and shipping a long-promised Xero export. Three feature requests dominate
support: bulk-invoice export, multi-currency, two-way calendar sync.

---

Message 2 — your task prompt:

Draft an outbound email to a 6-person Sydney UX consultancy I saw on
LinkedIn announcing they just hired their first ops manager. The email
should introduce Pacework, reference the new ops hire specifically, and
ask for a 20-minute conversation — not a full demo. Tone: direct, warm,
not salesy. Under 180 words.
```
...
Notice what the context document in Message 1 does that the task prompt in Message 2 doesn't have to. It establishes the voice ("direct, warm, and a bit dry — not corporate, not hype"). It names the ideal customer type. It states a current business focus. When Message 2 arrives asking for an outbound with a "direct, warm, not salesy" tone, Claude isn't guessing at what that means for this particular SaaS — it already has the frame. That's the practical value of the two-message structure: the context message does the briefing work, the task message does the directing work, and neither has to carry both at once.
...
When Claude responds to your task prompt, read it before you do anything else. Don't skim it. Read it as the intended recipient would read it — your customer, your teammate, the prospect, whoever the output is for.

Then ask yourself three questions:

1. Does this match the task? Did Claude do what I actually asked?
2. Does this sound like my product? Does the voice, the level of detail, the framing match how we communicate?
3. Is it accurate? Are there any facts, figures, or feature claims that need to be corrected?

If the answer to all three is yes, you're done. Ship it.
...
from any billing tool. Rewrite just the opening two sentences to make
them specific to Pacework's focus on small consultancies. Keep everything
else the same.
```
...
Most people skip this step. They get a good output, ship it, and move on. Then two weeks later they try the same kind of task and start from scratch — because they didn't note what worked. Two minutes of capture is the difference between a one-time win and a repeatable system.

After the session, note three things:

**What the prompt pattern was.** Jot down the structure you used — context first, then task, specific constraint on tone, word count specified. If it worked, you want to be able to reproduce it for the next similar task. Keep a simple document — a Notion page, a pinned Slack message to yourself, anything you'll actually open again — and paste the structure in. Not the full prompt; just the pattern.
...
That's the target. Not perfection. Not a response you ship without reading. Meaningful time saved on a real task. When that happens consistently for the same type of task, you have a working pattern. It belongs in your prompt library and your task map.
...
The merged message looks like this: three paragraphs about the SaaS, then a paragraph about the task, all sent at once. Claude responds with something that partially uses the context and partially ignores it. The founder rewrites the prompt, adds more context to the task, and sends it again — now with a second copy of the context mixed into the task. By the third attempt they're debugging a tangled message instead of working on the actual task.
