...
The first five lessons were about building the parts. You learned what Claude is and what it isn't (1.1). You learned to write a prompt that actually specifies what you want (1.2). You audited what you've been doing with AI and where the gaps are (1.3). You mapped the tasks in your week that Claude is best suited to (1.4). And you wrote a context document — a structured description of your agency that gives Claude the background it needs to produce useful, relevant output (1.5).

This lesson is about putting all of those parts together in one real session.

You're not practising here. You're producing an output you can actually use — a vendor update, a CMA narrative, an owner report, a prospecting letter. By the end of this lesson you'll have run a complete Claude session on a meaningful task from your task map, and you'll have something tangible to show for it — not a worked example, not a sample, but your own work.
...
I run a 5-person buyer's agency in inner Melbourne called Hartwell Buyers
Advocates. We work exclusively with home buyers in the $1.2M–$2.5M range,
mostly upgraders moving from a first home to a long-term family home in
Hawthorn, Camberwell, and Glen Iris. We don't take vendor clients.

My ideal clients have $100K–$300K equity, a clear timeframe (next 6 months),
and value off-market opportunities and auction strategy over property tours.
We charge a fixed fee, not a percentage. Our language is calm, considered,
and analytical — not high-energy or salesy.

Current focus: growing referrals from mortgage brokers and conveyancers in
our patch. We have four active retainers and two scoping conversations open.

---

Message 2 — your task prompt:

Draft an outreach email to a Hawthorn-based mortgage broker who has just
been mentioned in a podcast about first-home upgraders. The email should
introduce Hartwell, reference the podcast specifically, and ask for a 20-minute
conversation — not a full meeting. Tone: considered, curious, not salesy.
Under 180 words.
```
...
Notice what the context document in Message 1 does that the task prompt in Message 2 doesn't have to. It establishes the voice ("calm, considered, and analytical — not high-energy or salesy"). It names the ideal client type. It states a current business focus. When Message 2 arrives asking for an outreach email with a "considered, curious, not salesy" tone, Claude isn't guessing at what that means for this particular buyer's agency — it already has the frame. That's the practical value of the two-message structure: the context message does the briefing work, the task message does the directing work, and neither has to carry both at once.
...
When Claude responds to your task prompt, read it before you do anything else. Don't skim it. Read it as the intended recipient would read it — your vendor, your landlord, the buyer, the broker, whoever the output is for.

Then ask yourself three questions:

1. Does this match the task? Did Claude do what I actually asked?
2. Does this sound like {{businessName}}? Does the voice, the level of detail, the framing match how we communicate?
3. Is it accurate? Are there any facts, figures, or statements — sale prices, days on market, rental rates — that need to be corrected?
...
```
The structure is good but the opening line is too generic — it could be from
any buyer's agency in Melbourne. Rewrite just the opening two sentences to
make them specific to Hartwell's focus on upgraders in the inner-east. Keep
...
Most agency principals skip this step. They get a good post-open-home update, send it, and move on. Then two weeks later they try the same kind of task and start from scratch — because they didn't note what worked. Two minutes of capture is the difference between a one-time win and a repeatable system.

After the session, note three things:

**What the prompt pattern was.** Jot down the structure you used — context first, then task, specific constraint on tone, word count specified. If it worked, you want to be able to reproduce it for the next vendor update or owner report. Keep a simple document — a notes file, a Google Doc, a notebook in your CRM, anything you'll actually open again — and paste the structure in. Not the full prompt; just the pattern.

**What was missing from your context document.** If you had to add information in a revision prompt that really should have been there from the start — your preferred tone, a detail about your patch, a constraint you always want applied — add it to the context document now, while it's fresh. One good session can improve your context document in ways that make the next ten sessions faster.
...
If yes — if you would have written that vendor update just as quickly, or if the draft Claude produced needs so much editing that you're really rewriting it from scratch — then the prompt needs work. Something was missing that made the output too generic or too far from what you needed. That's a useful diagnosis: the session didn't fail, it told you what to add to the prompt next time.

If no — if Claude produced something you couldn't have drafted that quickly, or something you would have procrastinated on (the late-rent letter, the difficult price-reduction email), or something that freed up mental energy for a harder task — then the session worked. Claude saved you meaningful time.

That's the target. Not perfection. Not a response you publish without reading. Meaningful time saved on a real task. When that happens consistently for the same type of task — say, the post-open-home update — you have a working pattern. It belongs in your prompt library and your task map.

Notice that this test isn't about quality in the abstract. A Claude session that produces a mediocre vendor update in three minutes might still be worth running if that update would have taken you fifteen minutes to write and twenty minutes of procrastination before you started. The comparison isn't Claude versus perfect — it's Claude versus what you would actually have done without Claude, including the time you'd spend avoiding the task.
...
The merged message looks like this: three paragraphs about the agency, then a paragraph about the task — say, drafting a post-open-home update — all sent at once. Claude responds with something that partially uses the context and partially ignores it. The principal rewrites the prompt, adds more context to the task, and sends it again — now with a second copy of the context mixed into the task. By the third attempt they're debugging a tangled message instead of working on the actual vendor update.
