...
export function capstonePassedSubject(): string {
  return "Capstone passed — you've finished Claude Mastery";
}
...
export function capstonePassedHtml({ firstName, cohortNumber, finalScore, certificateUrl, dashboardUrl }: CapstonePassedProps): string {
  const inner = `
    ...
    ${p(`You've passed the Module 6 capstone and finished the Claude Mastery <strong style="color:${tokens.text};">Cohort ${cohortNumber}</strong> programme.`)}
    ...
    <div style="margin:0 0 16px;">
      ${primaryButton(certificateUrl, "Download your certificate →")}
    </div>
    <div style="margin:0 0 24px;">
      ${outlineButton(dashboardUrl, "Open your dashboard")}
    </div>
    ${divider()}
    <p style="margin:0 0 12px;font-size:14px;font-weight:600;color:${tokens.text};font-family:${tokens.font};">What's next</p>
    <ul style="margin:0;padding-left:20px;color:${tokens.textBody};font-size:14px;line-height:1.7;font-family:${tokens.font};">
      <li>Your dashboard stays open — revisit any lesson, anytime</li>
      <li>Share your win on LinkedIn (we'll cheer you on)</li>
