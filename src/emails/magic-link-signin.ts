...
export function magicLinkSigninHtml({ url }: MagicLinkSigninProps): string {
  const inner = `
    ...
    <div style="margin:28px 0 0;">
      ${primaryButton(url, "Sign in to Claude Mastery →")}
    </div>
    ${pSmall("If the button doesn't work, copy and paste this URL into your browser:<br><span style=\"color:" + tokens.blueDark + ";word-break:break-all;\">" + url + "</span>")}
    <p style="margin:24px 0 0;font-size:13px;color:${tokens.textFaint};font-family:${tokens.font};">If you didn't request this, you can safely ignore this email.</p>
  `;
